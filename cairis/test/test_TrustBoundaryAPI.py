...
class TrustBoundaryAPITests(CairisDaemonTestCase):
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            url, content_type="application/json", data=self.prepare_json()
        )
        if sys.version_info > (3,):
    ...
    def test_get_by_name(self):
        ...
        rv = self.app.post(
            "/api/trust_boundaries?session_id=test",
            content_type="application/json",
            data=self.prepare_json(),
        )
        ...
        self.assertEqual(
            tb["theEnvironmentProperties"][0]["theComponents"][0]["theName"],
            "Authenticate Researcher",
        )
        self.assertEqual(
            tb["theEnvironmentProperties"][0]["theComponents"][0]["theType"], "process"
        )
        rv = self.app.delete("/api/trust_boundaries/name/Shibboleth?session_id=test")
    ...
    def test_put(self):
        ...
        upd_body = self.prepare_json(
            trust_boundary=self.prepare_updated_trust_boundary()
        )
        rv = self.app.put(
            "/api/trust_boundaries/name/Shibboleth?session_id=test",
            data=upd_body,
            content_type="application/json",
        )
        self.assertIsNotNone(rv.data, "No response")
        ...
        self.assertEqual(
            upd_tb["theEnvironmentProperties"][0]["theComponents"][0]["theName"],
            "Authenticate Researcher",
        )
        self.assertEqual(
            upd_tb["theEnvironmentProperties"][0]["theComponents"][0]["theType"],
            "process",
        )
        rv = self.app.delete("/api/trust_boundaries/name/Shibboleth?session_id=test")
