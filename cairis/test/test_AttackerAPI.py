...
class AttackerAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.existing_capabilities = [
            {"name": "Resources/Equipment", "value": "Low"},
            {"name": "Knowledge/Methods", "value": "High"},
        ]
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            attackers, dict, "The result is not a dictionary as expected"
        )
    ...
    def test_get_all_summary(self):
        ...
        self.logger.info(
            "[%s] First attacker summary: %s [%s]\n", method, ats[0]["theName"]
        )

    def test_get_by_name(self):
        method = "test_get_by_name"
        url = "/api/attackers/name/%s?session_id=test" % quote(
            self.existing_attacker_name
        )
        rv = self.app.get(url)
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/attackers/name/%s?session_id=test" % quote(
            self.prepare_new_attacker().theName
        )
        ...
        self.app.post(
            "/api/attackers", content_type="application/json", data=new_attacker_body
        )
    ...
    def test_post(self):
        ...
        self.app.delete(
            "/api/attackers/name/%s?session_id=test"
            % quote(self.prepare_new_attacker().theName)
        )
    ...
    def test_put(self):
        ...
        self.assertGreater(
            message.find("updated"), -1, "The attacker was not successfully updated"
        )

        rv = self.app.get(
            "/api/attackers/name/%s?session_id=test" % quote(attacker_to_update.theName)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
