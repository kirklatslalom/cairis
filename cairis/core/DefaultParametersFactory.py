...
def addDefaultAsset(objtName, envName, session_id, assetType="Information"):
    ...
    envProps = [
        AssetEnvironmentProperties(
            envName,
            [0, 0, 1, 0, 0, 0, 0, 0],
            ["None", "None", "To be defined", "None", "None", "None", "None", "None"],
        )
    ]
    p = AssetParameters(
        objtName,
...
def addDefaultUseCase(objtName, envName, session_id):
    ...
    if roleId == -1:
        roleId = dbProxy.addRole(
            RoleParameters("Unknown", "Stakeholder", "UNKNOWN", "Unknown role", None)
        )

    s = Steps()
    s.append(Step("Undefined"))
    envProps = [
        UseCaseEnvironmentProperties(envName, "To be defined", s, "To be defined")
    ]
    p = UseCaseParameters(
        objtName, "Unknown", "TBC", ["Unknown"], "To be defined", [], envProps
    )
