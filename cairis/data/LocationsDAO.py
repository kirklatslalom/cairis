...
class LocationsDAO(CairisDAO):
    ...
    def convert_loc_list(self, real_loc_list=None, fake_loc_list=None):
        ...
        if real_loc_list is not None:
            if len(real_loc_list) > 0:
                for real_loc in real_loc_list:
                    ...
                    for ai in real_loc.assetInstances():
                        loc_dict["theAssetInstances"].append(
                            {"theName": ai[0], "theAsset": ai[1]}
                        )
                    loc_dict["thePersonaInstances"] = []
                    for pi in real_loc.personaInstances():
                        loc_dict["thePersonaInstances"].append(
                            {"theName": pi[0], "thePersona": pi[1]}
                        )
            ...
            raise MissingParameterHTTPError(
                param_names=["real_loc_list", "fake_loc_list"]
            )
        return new_loc_list
    ...
    def get_locations_model(self, locations_name, environment_name, pathValues=[]):
        ...
        try:
            riskOverlay = self.db_proxy.locationsRiskModel(
                locations_name, environment_name
            )
            lModel = GraphicalLocationModel(
                locations_name,
                environment_name,
                riskOverlay,
