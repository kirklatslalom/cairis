...
from cairis.tools.ModelDefinitions import (
    RiskModel,
    MisuseCaseModel,
    MisuseCaseEnvironmentPropertiesModel,
)
...
class RiskDAO(CairisDAO):
    ...
    def get_objects(self, constraint_id=-1, simplify=True, skip_misuse=False):
        ...
        for key in riskKeys:
            ...
            if value.theMisuseCase:
                value.theMisuseCase = self.get_misuse_case_by_risk_name(
    ...
    def get_object_by_name(self, name, simplify=True, skip_misuse=False):
        ...
        if found_risk.theMisuseCase and not skip_misuse:
            found_risk.theMisuseCase = self.get_misuse_case_by_risk_name(
                found_risk.theName, simplify=False
            )
        if simplify:
    ...
    def get_misuse_cases(self, pathValues=[]):
        ...
        for key in mcKeys:
            ...
            for mcep in misuse_cases[key].environmentProperties():
                envName = mcep.name()
                misuse_cases[key].theObjective = self.get_misuse_case_obj_and_assets(
                    threat_name, vuln_name, envName
                )
    ...
    def get_misuse_case_by_threat_vulnerability(
        self, threat_name, vulnerability_name, pathValues=[]
    ):
        try:
            misuse_case = self.db_proxy.riskMisuseCase(
                -1, threat_name, vulnerability_name
            )
            if misuse_case != None:
                return self.simplify(misuse_case)
            else:
                return self.simplify(
                    cairis.core.MisuseCaseFactory.build(
                        threat_name, vulnerability_name, self.db_proxy
                    )
                )
        except DatabaseProxyException as ex:
            self.close()
    ...
    def get_misuse_case_assets(self, threat_name, environment_name):
        ...
        try:
            ...
            environment_id = self.db_proxy.getDimensionId(
    ...
    def get_misuse_case_obj_and_assets(
        self, threat_name, vulnerability_name, environment_name
    ):
        """
    ...
    def get_misuse_case_severity(self, vulnerability_name, environment_name):
        severity_name = "N/A"
        try:
            vulnerability_id = self.db_proxy.getDimensionId(
                vulnerability_name, "vulnerability"
            )
            environment_id = self.db_proxy.getDimensionId(
                environment_name, "environment"
            )
            severity_name = self.db_proxy.vulnerabilitySeverity(
    ...
    def expand_mc_props(self, misuse_case):
        ...
        for idx in range(0, len(misuse_case.theEnvironmentProperties)):
            ...
            (
                env_prop.theObjective,
                env_prop.theAssets,
            ) = self.get_misuse_case_obj_and_assets(
    ...
    def get_scores_by_rvte(
        ...
        return self.get_scores_by_rtve(
            risk_name, vulnerability_name, threat_name, environment_name, pathValues
        )

    def get_scores_by_rtve(
        self,
        risk_name,
        threat_name,
        vulnerability_name,
        ...
        try:
            scores = self.db_proxy.riskScore(
                threat_name, vulnerability_name, environment_name, risk_name
            )
            if len(scores) > 0:
    ...
    def get_risk_rating_by_vte(
        self, vulnerability_name, threat_name, environment_name, pathValues=[]
    ):
        return self.get_risk_rating_by_tve(
            threat_name, vulnerability_name, environment_name, pathValues
        )

    def get_risk_rating_by_tve(
        self, threat_name, vulnerability_name, environment_name, pathValues=[]
    ):
        try:
            rating = self.db_proxy.riskRating(
                -1, threat_name, vulnerability_name, environment_name
            )
            risk_rating = RiskRating(
                threat_name, vulnerability_name, environment_name, rating
            )
            return risk_rating
        except DatabaseProxyException as ex:
            self.close()
    ...
    def from_json(self, request):
        ...
        json_dict["__python_obj__"] = (
            RiskParameters.__module__ + "." + RiskParameters.__name__
        )

        if json_dict["theMisuseCase"]:
            mc_dict = json_dict["theMisuseCase"]
            check_required_keys(mc_dict, MisuseCaseModel.required)
            mc_dict["__python_obj__"] = (
                MisuseCaseParameters.__module__ + "." + MisuseCaseParameters.__name__
            )
            for idx in range(0, len(mc_dict["theEnvironmentProperties"])):
                ...
                mcep_dict["__python_obj__"] = (
                    MisuseCaseEnvironmentProperties.__module__
                    + "."
                    + MisuseCaseEnvironmentProperties.__name__
                )
