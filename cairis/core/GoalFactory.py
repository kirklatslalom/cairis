...
def preventDeterText(response, dbProxy):
    ...
    return [
        (riskGoalName, riskGoalDef, goalCat),
...
def detectText(response, dbProxy):
    ...
    return [
        (riskGoalName, riskGoalDef, goalCategory),
...
def reactText(response, dbProxy):
    ...
    riskGoalDef = (
        "The system shall react to an occurrence of " + riskName + " detected by "
    )
    thrGoalDef = (
        "The system shall react to an occurrence of " + thrName + " detected by "
    )
    vulGoalDef = (
        "The system shall react to an occurrence of " + vulName + " detected by "
    )
    if len(detMechs) == 1:
...
def transferText(response):
    ...
    goalDef = (
        "The roles of "
        + ",".join(response.roleNames("", "Maximise", ""))
        + " shall be accountable for mitigating "
        + response.risk()
        + "."
    )
    ...
    for p in environmentProperties:
        ...
        if currentType != firstMitType:
            exceptionText = (
                "Mitigation "
                + response.name()
                + " does not have the same mitigation type in all of its situated environments."
            )
            ...
            if p.detectionPoint() != firstDetPoint:
                exceptionText = (
                    ...
                    + response.name()
                    + " needs to detect at the same point in all of its situated environments."
                )
                raise ConflictingType(exceptionText)
            ...
            if p.detectionMechanisms() != firstDetMechs:
                exceptionText = (
                    "React mitigation "
                    + response.name()
                    + " needs to use the same countermeasure in each of its situated environments."
                )
                raise ConflictingType(exceptionText)
...
def build(response, dbProxy=None):
    ...
    if goalCategory == "Transfer":
        ...
        threatParameters = GoalParameters(
        ...
        vulnerabilityParameters = GoalParameters(
            vulGoalName, goalOriginator, [], vulGoalEnvProperties
        )
