...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Computer Aided Integration of Requirements and Information Security - Model Export"
    )
    ...
    parser.add_argument(
        "--type",
        dest="modelFormat",
        help="model type to export.  One of all, package, requirements, scenarios, usecases, architecture, attackpatterns, securitypatterns or GRL",
    )
    parser.add_argument(
        "--persona",
        nargs="+",
        help="Persona name or ALL (relevant for GRL export only)",
    )
    parser.add_argument(
        "--task", nargs="+", help="Task name or ALL (relevant for GRL export only)"
    )
    parser.add_argument(
        "--environment",
        dest="envName",
        help="Environment name (relevant for GRL export only)",
    )
    args = parser.parse_args()
    cairis.core.BorgFactory.initialise(user=args.userName, db=args.dbName)
    file_export(
        args.modelFormat, args.outputFile, args.persona, args.task, args.envName
    )
...
def file_export(
    ...
    if modelFormat == "all":
        ...
        if len(personaNames) == 0:
            ...
            msgStr += exportGRL(
                outputFile, personaNames, taskNames, envName, session_id
            )
    else:
