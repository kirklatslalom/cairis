...
class ModelExportTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        importTVTypeFile(
            ...
            + "/../examples/threat_vulnerability_types/cwecapec_tv_types.xml",
            0,
        )
        importDirectoryFile(
            os.environ["CAIRIS_SRC"] + "/../examples/directories/owasp_directory.xml", 0
        )
        importDirectoryFile(
            os.environ["CAIRIS_SRC"]
            + "/../examples/directories/cwecapec_directory.xml",
            0,
        )
    ...
    def testExportGRL(self):
        ...
        self.assertEqual(
            exportGRL(outFile, "all", "Unsafe application install", "Complete"),
            "Exported GRL for all in tasks Unsafe application install situated in environment Complete",
        )
        self.assertEqual(os.path.isfile(outFile), True)
