...
class UserGoalModel:
    def __init__(
        self, contributions, envName, db_proxy=None, font_name=None, font_size=None
    ):
    ...
    def buildNode(self, dimName, objtName, personaName):
        ...
        if dimName == "goal" or dimName == "softgoal" or dimName == "belief":
            ...
            satScore = self.dbProxy.goalSatisfactionScore(
    ...
    def graph(self):
        try:
            ...
            for pName in set(
                filter(
                    lambda x: x != "",
                    list(map(lambda x: x[3], self.theContributions))
                    + list(map(lambda x: x[7], self.theContributions)),
                )
            ):
                c = pydot.Cluster(
                    pName,
