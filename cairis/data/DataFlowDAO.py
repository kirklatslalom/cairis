...
class DataFlowDAO(CairisDAO):
    ...
    def get_objects(
        ...
        try:
            dfs = self.db_proxy.getDataFlows(
                dataflow_name, from_name, from_type, to_name, to_type, environment_name
            )
            dfsOut = []
            for df in dfs:
    ...
    def realToFake(self, df):
        ...
        for dfo in df.obstacles():
            fakeDfos.append(
                {"theObstacleName": dfo[0], "theKeyword": dfo[1], "theContext": dfo[2]}
            )
    ...
    def check_existing_dataflow(
        self, dataflow_name, from_type, from_name, to_type, to_name, environment_name
    ):
        try:
            self.db_proxy.checkDataFlowExists(
                dataflow_name, from_type, from_name, to_type, to_name, environment_name
            )
            return False
        except DatabaseProxyException as ex:
    ...
    def get_dataflow_diagram(
        self, environment_name, filter_type, filter_element, pathValues=[]
    ):
        fontName, fontSize, apFontName = get_fonts(session_id=self.session_id)
        if filter_element == "all":
            filter_element = ""
        try:
            dfdRows = self.db_proxy.dataFlowDiagram(
                environment_name, filter_type, filter_element
            )
            associations = DataFlowDiagram(
                dfdRows,
                environment_name,
                self.db_proxy,
                font_name=fontName,
