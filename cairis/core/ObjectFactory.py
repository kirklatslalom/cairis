...
def build(objtId, p):
    ...
    if p.__class__.__name__ == "PersonaParameters":
        return Persona(objtId, p.name(), p.activities(), p.attitudes(), p.aptitudes(), p.motivations(), p.skills(),
            p.intrinsic(), p.contextual(), p.image(), p.assumption(), p.type(), p.tags(), p.environmentProperties(),
            p.codes(), )
    if p.__class__.__name__ == "AssetParameters":
        return Asset(objtId, p.name(), p.shortCode(), p.description(), p.significance(), p.type(), p.critical(),
            p.criticalRationale(), p.tags(), p.interfaces(), p.environmentProperties(), )
    if p.__class__.__name__ == "TemplateAssetParameters":
        return TemplateAsset(objtId, p.name(), p.shortCode(), p.description(), p.significance(), p.type(),
            p.surfaceType(), p.accessRight(), p.properties(), p.rationale(), p.tags(), p.interfaces(), )
    if p.__class__.__name__ == "TemplateRequirementParameters":
        return TemplateRequirement(objtId, p.name(), p.asset(), p.type(), p.description(), p.rationale(),
            p.fitCriterion(), )
    ...
    if p.__class__.__name__ == "SecurityPatternParameters":
        return SecurityPattern(objtId, p.name(), p.context(), p.problem(), p.solution(), p.requirements(),
            p.associations(), )
    ...
    if p.__class__.__name__ == "ClassAssociationParameters":
        return ClassAssociation(objtId, p.environment(), p.headAsset(), p.headDimension(), p.headNavigation(),
            p.headType(), p.headMultiplicity(), p.headRole(), p.tailRole(), p.tailMultiplicity(), p.tailType(),
            p.tailNavigation(), p.tailDimension(), p.tailAsset(), p.rationale(), )
    if p.__class__.__name__ == "GoalAssociationParameters":
        return GoalAssociation(objtId, p.environment(), p.goal(), p.goalDimension(), p.type(), p.subGoal(),
            p.subGoalDimension(), p.alternative(), p.rationale(), )
    if p.__class__.__name__ == "DependencyParameters":
        return Dependency(objtId, p.environment(), p.depender(), p.dependee(), p.dependencyType(), p.dependency(),
            p.rationale(), )
    ...
    if p.__class__.__name__ == "TaskParameters":
        return Task(objtId, p.name(), p.shortCode(), p.objective(), p.assumption(), p.author(), p.tags(),
            p.environmentProperties(), )
    if p.__class__.__name__ == "UseCaseParameters":
        return UseCase(objtId, p.name(), p.author(), p.code(), p.actors(), p.description(), p.tags(),
            p.environmentProperties(), )
    ...
    if p.__class__.__name__ == "EnvironmentParameters":
        return Environment(objtId, p.name(), p.shortCode(), p.description(), p.environments(), p.duplicateProperty(),
    ...
    if p.__class__.__name__ == "PersonaCharacteristicParameters":
        return PersonaCharacteristic(objtId, p.persona(), p.qualifier(), p.behaviouralVariable(), p.characteristic(),
            p.grounds(), p.warrant(), p.backing(), p.rebuttal(), )
    if p.__class__.__name__ == "TaskCharacteristicParameters":
        return TaskCharacteristic(objtId, p.task(), p.qualifier(), p.characteristic(), p.grounds(), p.warrant(),
    ...
    if p.__class__.__name__ == "DataFlowParameters":
        return DataFlow(p.name(), p.type(), p.environment(), p.fromName(), p.fromType(), p.toName(), p.toType(),
            p.assets(), p.obstacles(), p.tags(), )
    else:
