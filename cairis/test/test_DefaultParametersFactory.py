...
class DefaultParametersFactoryTest(unittest.TestCase):
    ...
    def testAddDefaultAsset(self):
        assetId = cairis.core.DefaultParametersFactory.build(
            "TestAsset", "Default", "asset", None, "Information"
        )
        b = Borg()
    ...
    def testAddDefaultUseCase(self):
        ucId = cairis.core.DefaultParametersFactory.build(
            "TestUseCase", "Default", "usecase", None
        )
        b = Borg()
        ...
        self.assertEqual(objt.author(), "Unknown")
        self.assertEqual(
            objt.environmentProperties()[0].preconditions(), "To be defined"
        )
        self.assertEqual(str(objt.environmentProperties()[0].steps()[0]), "Undefined")
        self.assertEqual(
            objt.environmentProperties()[0].postconditions(), "To be defined"
        )
