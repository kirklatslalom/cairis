...
class RiskAPITests(CairisDaemonTestCase):
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/risks/name/%s?session_id=test" % quote(
            self.prepare_new_risk().name()
        )
    ...
    def test_post(self):
        ...
        rv = self.app.delete(
            "/api/risks/name/%s?session_id=test" % quote(self.prepare_new_risk().name())
        )
    ...
    def test_put(self):
        ...
        rv = self.app.put(
            "/api/risks/name/%s?session_id=test"
            % quote(self.prepare_new_risk().name()),
            data=upd_env_body,
        ...
        self.assertGreater(
            message.find("updated"), -1, "The risk was not successfully updated"
        )

        rv = self.app.get(
            "/api/risks/name/%s?session_id=test" % quote(risk_to_update.theName)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_get_scoring(self):
        ...
        url = (
            "/api/risks/name/%s/threat/%s/vulnerability/%s/environment/%s?session_id=test"
            % (
        ...
        url = (
            "/api/risks/name/%s/vulnerability/%s/threat/%s/environment/%s?session_id=test"
            % (
                quote(self.existing_risk_name),
                quote(self.existing_vulnerability),
                quote(self.existing_threat_name),
