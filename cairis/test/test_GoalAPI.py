...
class GoalAPITests(CairisDaemonTestCase):
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/goals/name/%s?session_id=test" % quote(
            self.prepare_new_goal().theName
        )
    ...
    def test_post(self):
        ...
        self.app.delete(
            "/api/goals/name/%s?session_id=test"
            % quote(self.prepare_new_goal().theName)
        )
    ...
    def test_put(self):
        ...
        self.assertGreater(
            message.find("updated"), -1, "The goal was not successfully updated"
        )

        rv = self.app.get(
            "/api/goals/name/%s?session_id=test" % quote(goal_to_update.theName)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_get_goal_concerns(self):
        ...
        rv = self.app.get(
            "/api/goals/name/Active%20Directory%20Network%20Services/environment/Day/concerns?session_id=test"
        )
    ...
    def prepare_new_goal(self):
        new_goal_refinements = [
            RefinementModel(
                "PreventUnauthorised Certificate Access", "goal", "or", "No", "None"
            )
        ]
