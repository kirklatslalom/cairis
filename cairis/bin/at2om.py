...
def dotToObstacleModel(graph, contextName, originatorName):
    ...
    for g in goals:
        xmlBuf += (
            ...
            + ' originator="'
            + originatorName
            + '">\n    <goal_environment name="'
            + contextName
            + '" category="Maintain" priority="Medium">\n      <definition>'
            + g.replace('"', "")
    ...
    for e in graph.get_edge_list():
        ...
        if fromName in acs:
            ...
            if fromName in goalNames:
                assocs.append(
                    '  <goal_association environment="'
                    + contextName
                    + '" goal_name='
                    + fromName
                    + ' goal_dim="goal" ref_type="obstruct" subgoal_name='
                    + toName
                    + ' subgoal_dim="obstacle" alternative_id="0">\n    <rationale>None</rationale>\n  </goal_association>\n'
                )
            else:
                assocs.append(
                    '  <goal_association environment="'
                    ...
                    + '" goal_name='
                    + fromName
                    + ' goal_dim="obstacle" ref_type="resolve" subgoal_name='
                    + toName
                    + ' subgoal_dim="goal" alternative_id="0">\n    <rationale>None</rationale>\n  </goal_association>\n'
                )
...
def main(args=None):
    parser = argparse.ArgumentParser(
        description="Attack Tree to CAIRIS Model converter"
    )
