...
class TemplateRequirementAPITests(CairisDaemonTestCase):
    ...
    def test_get_all(self):
        ...
        tr = trs[0]
        self.logger.info(
            "[%s] First template requirement: %s \n", method, tr["theName"]
        )

    def test_get_by_name(self):
        method = "test_get_by_name"
        url = "/api/template_requirements/name/%s?session_id=test" % quote(
            self.existing_tr_name
        )
        rv = self.app.get(url)
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            ...
            data=jsonpickle.encode(self.new_tr_dict),
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        self.assertGreater(
            ackMsg.find("created"), -1, "Template requirement not created"
        )
    ...
    def test_put(self):
        ...
        rv = self.app.put(
            url, content_type="application/json", data=jsonpickle.encode(updDict)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        ackMsg = json_resp.get("message", None)
        self.assertGreater(
            ackMsg.find("updated"), -1, "Template requirement not updated"
        )
    ...
    def test_delete(self):
        ...
        ackMsg = json_resp.get("message", None)
        self.assertGreater(
            ackMsg.find("deleted"), -1, "Template requirement not deleted"
        )
