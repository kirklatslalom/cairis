...
class UserStoryTest(unittest.TestCase):
    def setUp(self):
        ...
        ied = ExternalDocumentParameters(
            edData["theName"],
            edData["theVersion"],
            edData["thePublicationDate"],
            edData["theAuthors"],
            edData["theDescription"],
        )
        b.dbProxy.addExternalDocument(ied)

        drData = d["document_references"][0]
        idr = DocumentReferenceParameters(
            drData["theName"],
            drData["theDocName"],
            drData["theContributor"],
            drData["theExcerpt"],
        )
        b.dbProxy.addDocumentReference(idr)

        ugData = d["user_goals"][0]
        iug = ReferenceSynopsis(
            -1,
            ugData["theReference"],
            ugData["theSynopsis"],
            ugData["theDimension"],
