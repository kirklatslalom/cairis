...
class GoalTest(unittest.TestCase):
    def setUp(self):
        ...
        iaeps2 = AssetEnvironmentProperties(
            iassets[1]["theEnvironmentProperties"][1][0],
            iassets[1]["theEnvironmentProperties"][1][1],
            iassets[1]["theEnvironmentProperties"][1][2],
        )
    ...
    def testGoal(self):
        ...
        igep1d = GoalEnvironmentProperties(
            ...
            [],
            self.iGoals[0]["theEnvironmentProperties"][0]["thePolicy"],
        )
        ...
        igp1 = GoalParameters(
            self.iGoals[0]["theName"],
            self.iGoals[0]["theOriginator"],
            [],
            [igep1d, igep1n],
        )
        igp2 = GoalParameters(
            self.iGoals[1]["theName"],
            self.iGoals[1]["theOriginator"],
            [],
            [igep2d, igep2n],
        )
        igp3 = GoalParameters(
            self.iGoals[2]["theName"],
        ...
        self.assertEqual(
            igep1d.definition() + " [Day].  " + igep1d.definition() + " [Night].  ",
            og1.definition("", "Maximise"),
        )
        self.assertEqual(igep1d.category(), ogep1.category())
        self.assertEqual(
            igep1d.category() + " [Day].  " + igep1d.category() + " [Night].  ",
            og1.category("", "Maximise"),
        )
        self.assertEqual(igep1d.priority(), ogep1.priority())
        self.assertEqual(
            igep1d.priority() + " [Day].  " + igep1d.priority() + " [Night].  ",
            og1.priority("", "Maximise"),
        )
        self.assertEqual(igep1d.fitCriterion(), ogep1.fitCriterion())
        self.assertEqual(
            igep1d.fitCriterion() + " [Day].  " + igep1d.fitCriterion() + " [Night].  ",
            og1.fitCriterion("", "Maximise"),
        )
        ...
        igop1 = GoalAssociationParameters(
            igep1d.name(), igp1.name(), "goal", "and", igp2.name(), "goal", 0, "None"
        )
        igop2 = GoalAssociationParameters(
            igep1d.name(), igp1.name(), "goal", "and", igp3.name(), "goal", 0, "None"
        )
