...
class CharacteristicReferenceSynopsis(object):
    ...
    def __getitem__(self, varName):
        if varName == "theSynopsis":
            return self.theSynopsis
        elif varName == "theDimension":
            return self.theDimension
        elif varName == "theActorType":
            return self.theActorType
        elif varName == "theActor":
            return self.theActor
        elif varName == "theInitialSatisfaction":
...
class CharacteristicReferenceContribution(object):
    ...
    def __getitem__(self, varName):
        if varName == "theMeansEnd":
            return self.theMeansEnd
        elif varName == "theContribution":
...
class CharacteristicReference(object):
    resource_fields = {
        ...
        "theReferenceSynopsis": fields.Nested(
...
class EnvironmentTensionModel(object):
    ...
    attr_dictionary = OrderedDict(
        sorted(list(attr_dictionary.items()), key=lambda t: t[1])
    )
    # endregion
    ...
    def __init__(
        self, base_attr_id=-1, attr_id=-1, value=0, rationale="None", key=None
    ):
        """
    ...
    def __init__(self, pSettings=None, pDict=None, contributors=None, revisions=None):
        ...
        for contributor in contributors or []:
            if isinstance(contributor, tuple):
                ...
                logger.warning(
                    "Item does not meet typical contributor structure. Passing this one."
                )
...
class PersonaTaskCharacteristics(object):
    ...
    def persona(self):
        return self.thePersona

    def duration(self):
        return self.theDuration

    def frequency(self):
        return self.theFrequency

    def demands(self):
        return self.theDemands

    def goalConflict(self):
        return self.theGoalConflict
...
class StepAttributes(object):
    resource_fields = {
        ...
        "theActor": fields.String,
        "theActorType": fields.String,
        "theExceptions": fields.List(
            fields.Nested(ExceptionAttributes.resource_fields)
        ),
    }
    required = list(resource_fields.keys())
    required.remove(obj_id_field)
