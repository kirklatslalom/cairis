...
class ObstacleDAO(CairisDAO):
    ...
    def add_object(self, obstacle):
        obsParams = ObstacleParameters(
            obsName=obstacle.theName,
            obsOrig=obstacle.theOriginator,
            tags=obstacle.theTags,
            properties=obstacle.theEnvironmentProperties,
        )
    ...
    def get_obstacle_types(self, environment_name=""):
        try:
            obstacle_types = self.db_proxy.getValueTypes(
                "obstacle_type", environment_name
            )
    ...
    def add_obstacle_type(self, obstacle_type, environment_name=""):
        ...
        type_exists = self.check_existing_obstacle_type(
            obstacle_type.theName, environment_name=environment_name
        )
    ...
    def get_obstacle_values(self, environment_name=""):
        try:
            obstacle_values = self.db_proxy.getValueTypes(
                "obstacle_value", environment_name
            )
    ...
    def simplify(self, obstacle):
        obstacle.theEnvironmentProperties = self.convert_properties(
            real_props=obstacle.theEnvironmentProperties
        )
    ...
    def generate_vulnerability(self, name, pathValues=[]):
        ...
        for op in obs.environmentProperties():
            vps.append(
                VulnerabilityEnvironmentProperties(
                    op.name(), "Negligible", op.concerns()
                )
            )
            gaps.append(
        ...
        v = VulnerabilityParameters(
            obs.name() + "(V)",
            obs.name(),
            self.db_proxy.defaultValue("vulnerability_type"),
            [],
            vps,
        )
