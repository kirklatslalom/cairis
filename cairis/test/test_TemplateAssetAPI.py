...
def addTestData():
    ...
    iar2 = ValueTypeParameters(
        iAccessRights[1]["theName"],
...
class TemplateAssetAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.new_ta_dict = {"session_id": "test", "object": self.new_ta}
        self.existing_ta_name = "Application Data"
        self.upd_ta = d["template_assets"][0]
        self.upd_ta_dict = {"session_id": "test", "object": self.new_ta}
        self.existing_ta_name = "Application Data"
    ...
    def test_get_by_name(self):
        ...
        url = "/api/template_assets/name/%s?session_id=test" % quote(
            self.existing_ta_name
        )
        rv = self.app.get(url)
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            ...
            data=jsonpickle.encode(self.new_ta_dict),
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_put(self):
        ...
        rv = self.app.put(
            url,
            content_type="application/json",
            data=jsonpickle.encode(self.upd_ta_dict),
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
