...
class ConceptMapModel:
    ...
    def buildNode(self, objtName, envName):
        ...
        n = pydot.Node(
            objtName, color=str(tScore), fontcolor=str(fontColour), URL=objtUrl
        )
        if self.cfSet == False:
    ...
    def graph(self):
        ...
        reqNodes = self.dbProxy.getDimensionNames(
            "requirement", self.theEnvironmentName
        )
