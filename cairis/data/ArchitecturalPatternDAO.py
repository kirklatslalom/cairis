...
class ArchitecturalPatternDAO(CairisDAO):
    ...
    def realToFakeAP(self, cv):
        ...
        ap["theAttackSurfaceMetric"] = {
            "theInterfacesDER": asm[0],
            "theChannelsDER": asm[1],
            "theUntrustedSurfaceDER": asm[2],
        }
        return ap
    ...
    def fakeToRealAp(self, ap):
        ...
        for c in ap["theComponents"]:
            ...
            for i in c["theInterfaces"]:
                cInts.append(
                    (i["theName"], i["theType"], i["theAccessRight"], i["thePrivilege"])
                )
            cStructs = []
            ...
            theComponents.append(
                ComponentParameters(
                    cName, cDesc, cInts, cStructs, cReqs, cGoals, cGoalAssocs
                )
            )
        theConnectors = []
        for conn in ap["theConnectors"]:
            theConnectors.append(
                ConnectorParameters(
                    conn["theConnectorName"],
                    apName,
                    conn["theFromComponent"],
                    conn["theFromRole"],
                    conn["theFromInterface"],
                    conn["theToComponent"],
                    conn["theToInterface"],
                    conn["theToRole"],
                    conn["theAssetName"],
        ...
        cvParams = ComponentViewParameters(
            apName, apSyn, [], [], [], [], [], theComponents, theConnectors
        )
        return cvParams
    ...
    def get_component_model(self, cvName, pathValues=[]):
        ...
        try:
            ...
            associations = GraphicalComponentModel(
                interfaces,
                connectors,
                db_proxy=self.db_proxy,
                font_name=fontName,
    ...
    def get_weakness_analysis(self, cvName, envName, pathValues=[]):
        try:
            ...
            for goalName, obsName in self.db_proxy.candidateGoalObstacles(
                cvName, envName
            ):
                walm.theCandidateGoals.append(
                    CandidateGoalObstacleModel(goalName, obsName)
                )
            return walm
        except DatabaseProxyException as ex:
            self.close()
        ...
        for assetName, componentName in self.db_proxy.componentAssets(cvName):
            assetParametersList.append(
                cairis.core.AssetParametersFactory.buildFromTemplate(
                    assetName, [envName], self.db_proxy
                )
            )
            if assetName not in acDict:
                acDict[assetName] = []
            acDict[assetName].append(componentName)
        self.db_proxy.situateComponentView(
            cvName, envName, acDict, assetParametersList, [], []
        )
