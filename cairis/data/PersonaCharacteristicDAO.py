...
class PersonaCharacteristicDAO(CairisDAO):
        ...
        if simplify:
            for key, value in list(pcs.items()):
                ...
                crs = CharacteristicReferenceSynopsis(
                    cs.synopsis(), cs.dimension(), cs.actorType(), cs.actor()
                )
    ...
    def get_object_by_name(self, persona_characteristic_name):
        pcId = self.db_proxy.getDimensionId(
            persona_characteristic_name, "persona_characteristic"
        )
        ...
        raise ObjectNotFoundHTTPError(
            'Persona characteristic:"' + persona_characteristic_name + '"'
        )
    ...
    def from_json(self, request, to_props=False):
        ...
        json_dict["__python_obj__"] = (
            PersonaCharacteristic.__module__ + "." + PersonaCharacteristic.__name__
        )
        ...
        if real_pc is not None:
            ...
            if len(real_pc.theGrounds) > 0:
                for real_pcr in real_pc.theGrounds:
                    ...
                    crs = CharacteristicReferenceSynopsis(
                        rs.synopsis(), rs.dimension(), rs.actorType(), rs.actor()
                    )
                    ...
                    pcr_list.append(
                        CharacteristicReference(
                            real_pcr[0], "grounds", real_pcr[1], real_pcr[2], crs, frc
                        )
                    )
            ...
            if len(real_pc.theWarrant) > 0:
                for real_pcr in real_pc.theWarrant:
                    ...
                    pcr_list.append(
                        CharacteristicReference(
                            real_pcr[0], "warrant", real_pcr[1], real_pcr[2], crs, frc
                        )
                    )
            ...
            if len(real_pc.theRebuttal) > 0:
                for real_pcr in real_pc.theRebuttal:
                    ...
                    pcr_list.append(
                        CharacteristicReference(
                            real_pcr[0], "rebuttal", real_pcr[1], real_pcr[2], crs, frc
                        )
                    )
    ...
    def assignIntentionalElements(self, pcSyn, rSyns, rConts):
        ...
        for rCont in rConts:
            if self.db_proxy.hasContribution(
                "reference", rCont.source(), rCont.destination()
            ):
