...
class RequirementDAO(CairisDAO):
    ...
    def get_requirements_by_asset(self, name, pathValues):
        ...
        return self.get_requirements(
            [pathValues[0], constraint_id, assetName, environmentName]
        )
    ...
    def get_requirements(self, pathValues):
        ...
        try:
            ...
            if ordered:
                requirements = self.simplifyList(
                    self.db_proxy.getOrderedRequirements(constraint_id, is_asset)
                )
            else:
                requirements = self.simplifyList(
                    (self.db_proxy.getRequirements(constraint_id, is_asset)).values()
                )
            return requirements
    ...
    def add_requirement(self, requirement, pathValues=[]):
        ...
        try:
            self.db_proxy.addRequirement(
                requirement, assetName=requirement.domain(), isAsset=isAsset
            )
        except ARMException as ex:
            self.close()
            handle_exception(ex)
    ...
    def from_json(self, request, domain_name=None):
        ...
        json_dict["__python_obj__"] = (
            Requirement.__module__ + "." + Requirement.__name__
        )
        json_dict["attrs"] = {}
    ...
    def get_concept_map_model(self, environment_name, requirement_name, pathValues):
        ...
        try:
            ...
            associations = GraphicalConceptMapModel(
