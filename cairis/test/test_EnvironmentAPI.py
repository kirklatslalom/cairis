...
class EnvironmentAPITests(CairisDaemonTestCase):
    ...
    def test_get_environment_names_by_threat_vulnerability(self):
        ...
        rv = self.app.get(
            "/api/environments/threat/Trojan%20Horse/vulnerability/Workflow%20channel/names?session_id=test"
        )
    ...
    def test_get_environment_names_by_vulnerability_threat(self):
        method = "test_get_environment_names_by_threat_vulnerability"
        rv = self.app.get(
            "/api/environments/vulnerability/Workflow%20channel/threat/Trojan%20Horse/names?session_id=test"
        )
        responseData = rv.data.decode("utf-8")
        names = jsonpickle.decode(responseData)
        self.assertIsNotNone(names, "No results after deserialization")
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            environments, list, "The result is not a list as expected"
        )
    ...
    def test_get_by_name(self):
        ...
        url = "/api/environments/name/%s?session_id=test" % quote(
            self.existing_environment_name
        )
        rv = self.app.get(url)
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/environments/name/%s?session_id=test" % quote(
            self.prepare_new_environment().theName
        )
    ...
    def test_post(self):
        ...
        self.app.delete(
            "/api/environments/name/%s?session_id=test"
            % quote(self.prepare_new_environment().theName)
        )
        rv = self.app.post(
            url, content_type="application/json", data=new_environment_body
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_put(self):
        ...
        self.assertGreater(
            message.find("updated"), -1, "The environment was not successfully updated"
        )

        rv = self.app.get(
            "/api/environments/name/%s?session_id=test"
            % quote(environment_to_update.theName)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_put_composite(self):
        ...
        rv = self.app.put(
            "/api/environments/name/%s?session_id=test"
            % quote(self.prepare_comp_environment().theName),
            data=upd_env_body,
            content_type="application/json",
        )
        self.assertIsNotNone(rv.data, "No response")
        ...
        for idx1 in range(0, 4):
            for idx2 in range(4, 8):
                tension = EnvironmentTensionModel(
                    base_attr_id=idx1, attr_id=idx2, value=0, rationale="None"
                )
