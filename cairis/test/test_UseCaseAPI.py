...
class UseCaseAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.existing_steps = []
        anException = ExceptionAttributes(
            "anException",
            "requirement",
        ...
        self.existing_steps.append(
            StepAttributes("Researcher does something", "", "", "", [anException])
        )
        self.existing_steps.append(
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            usecases, dict, "The result is not a dictionary as expected"
        )
    ...
    def test_get_all_summary(self):
        ...
        self.logger.info(
            "[%s] First use case summary: %s \n", method, ucs[0]["theName"]
        )

    def test_get_by_name(self):
        method = "test_get_by_name"
        url = "/api/usecases/name/%s?session_id=test" % quote(
    ...
    def test_delete(self):
        ...
        rv = self.app.get(
            "/api/persona_characteristics/name/Managers%20delegate%20security%20decisions?session_id=test"
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        pc["theCharacteristicSynopsis"] = {
            "theActor": "Claire",
            "theActorType": "persona",
            "theSynopsis": "Security delegated",
            "theDimension": "goal",
        }
        ...
        rv = self.app.put(
            "/api/persona_characteristics/name/Managers%20delegate%20security%20decisions?session_id=test",
            content_type="application/json",
            data=jsonpickle.encode(pcDict),
        )
        url = "/api/usecases/name/%s?session_id=test" % quote(
            self.prepare_new_usecase().name()
        )
        ...
        self.app.post(
            "/api/usecases", content_type="application/json", data=new_usecase_body
        )
    ...
    def test_post(self):
        ...
        self.app.delete(
            "/api/usecases/name/%s?session_id=test"
            % quote(self.prepare_new_usecase().name())
        )
    ...
    def test_put(self):
        ...
        self.assertGreater(
            message.find("updated"), -1, "The usecase was not successfully updated"
        )

        rv = self.app.get(
            "/api/usecases/name/%s?session_id=test" % quote(usecase_to_update.name())
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        rv = self.app.delete(
            "/api/usecases/name/%s?session_id=test" % quote(usecase_to_update.theName)
        )
