...
def main(args=None):
    parser = argparse.ArgumentParser(
        description="Computer Aided Integration of Requirements and Information Security - Grounded Theory to Persona Case converter"
    )
    ...
    parser.add_argument(
        "--concepts", dest="conceptsFile", help="grounded theory model concepts"
    )
    parser.add_argument(
        "--propositions",
        dest="propositionsFile",
        help="Propositions associated with grounded theory model quotations",
    )
    parser.add_argument(
        "--characteristics",
        dest="characteristicsFile",
        help="Persona characteristics associated with grounded theory model associations",
    )
    parser.add_argument(
        "--narratives", dest="narrativesFile", help="Persona narratives"
    )
    ...
    xmlHdr += (
        ...
        + args.contextName
        + '">\n    <contributors>\n      <contributor first_name="None" surname="None" affiliation="'
        + args.originatorName
        + '" role="Scribe" />\n    </contributors>\n  </project_settings>\n  <environment name="'
        + args.contextName
        + '" short_code="'
    ...
    for li in pcf.readlines():
        ...
        xmlBuf += (
            '<persona_characteristic persona="'
            + pName
            + '" behavioural_variable="'
            + bvName
            + '" modal_qualifier="'
            + modQual
            + '" >\n  <definition>'
    ...
    for personaName in personaNames:
        pHdr += (
            '<persona name="'
            + personaName
            + '" type="Primary" assumption_persona="FALSE" image="" >\n <activities>'
            + pnDict[(personaName, "ACT")]
            + "</activities>\n  <attitudes>"
            + pnDict[(personaName, "ATT")]
            + "</attitudes>\n  <aptitudes>"
            + pnDict[(personaName, "APT")]
            + "</aptitudes>\n  <motivations>"
            + pnDict[(personaName, "MOT")]
            + "</motivations>\n  <skills>"
            + pnDict[(personaName, "SKI")]
            + "</skills>\n  <intrinsic>"
            + pnDict[(personaName, "INT")]
            + "</intrinsic>\n  <contextual>"
            + pnDict[(personaName, "CON")]
            + '</contextual>\n<persona_environment name="'
            + args.contextName
