...
class RiskAnalysisContentHandler(ContentHandler, EntityResolver):
    ...
    def startElement(self, name, attrs):
        ...
        if name == "role":
            ...
            self.theInterfaces.append(
                (attrs["name"], it2Id(attrs["type"]), arName, privName)
            )
    ...
    def endElement(self, name):
        if name == "role":
            ...
            self.theSpDict[self.thePropertyName] = (
                self.thePropertyValue,
                unescape(self.theRationale),
            )
            self.resetThreatenedPropertyAttributes()
        elif name == "vulnerability":
            p = VulnerabilityParameters(
                self.theName,
                unescape(self.theDescription),
                self.theType,
                self.theTags,
            ...
            p = VulnerabilityEnvironmentProperties(
                self.theEnvironmentName, self.theSeverity, self.theAssets
            )
            ...
            p = AttackerEnvironmentProperties(
                self.theEnvironmentName,
                self.theRoles,
                self.theMotivations,
                self.theCapabilities,
            ...
            mc = MisuseCase(
                -1,
                "Exploit " + self.theName,
                self.theEnvironmentProperties,
                self.theName,
            )
            p = RiskParameters(
                self.theName, self.theThreat, self.theVulnerability, mc, self.theTags
            )
            self.theRisks.append(p)
            self.resetRiskAttributes()
        elif name == "misusecase":
            p = MisuseCaseEnvironmentProperties(
                self.theEnvironmentName, unescape(self.theDescription)
            )
            self.theEnvironmentProperties.append(p)
            self.resetRiskEnvironmentAttributes()
        elif name == "response":
            p = ResponseParameters(
                self.theType + " " + self.theRisk,
                self.theRisk,
                self.theTags,
                self.theEnvironmentProperties,
                self.theType,
            ...
            p = AcceptEnvironmentProperties(
                self.theEnvironmentName, self.theCost, unescape(self.theDescription)
            )
            self.theEnvironmentProperties.append(p)
            self.resetResponseEnvironmentAttributes()
        elif name == "transfer_environment":
            p = TransferEnvironmentProperties(
                self.theEnvironmentName,
                unescape(self.theDescription),
                self.theResponseRoles,
            )
            ...
            p = MitigateEnvironmentProperties(
                self.theEnvironmentName, "Detect", self.theDetectionPoint
            )
            self.theEnvironmentProperties.append(p)
            self.resetResponseEnvironmentAttributes()
        elif name == "react_environment":
            p = MitigateEnvironmentProperties(
                self.theEnvironmentName, "React", "", self.theDetectionMechanisms
            )
            self.theEnvironmentProperties.append(p)
            self.resetResponseEnvironmentAttributes()
        elif name == "asset_association":
