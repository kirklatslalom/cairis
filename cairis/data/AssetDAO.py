...
class AssetDAO(CairisDAO):
    ...
    def get_asset_model(self, environment_name, asset_name, pathValues):
        ...
        try:
            ...
            associations = GraphicalAssetModel(
                list(associationDictionary.values()),
                environment_name,
                asset_name,
        ...
        if real_ifs is not None:
            ...
            if len(fake_ifs) > 0:
                for fIf in fake_ifs:
                    new_ifs.append(
                        (
                            fIf["theInterfaceName"],
                            fIf["theInterfaceType"],
                            fIf["theAccessRight"],
                            fIf["thePrivilege"],
                        )
                    )
        ...
        if real_props is not None:
            if len(real_props) > 0:
                for real_prop in real_props:
                    ...
                    asset_values = self.db_proxy.getValueTypes(
                        "asset_value", real_prop.theEnvironmentName
                    )
                    prop_dict = {}
                    ...
                    for idx in range(0, len(real_prop.theAssociations)):
                        ...
                        fakeAssoc = {
                            "theHeadNav": realAssoc[0],
                            "theHeadType": realAssoc[1],
                            "theHeadMultiplicity": realAssoc[2],
                            "theHeadRole": realAssoc[3],
                            "theTailRole": realAssoc[4],
                            "theTailMultiplicity": realAssoc[5],
                            "theTailType": realAssoc[6],
                            "theTailNav": realAssoc[7],
                            "theTailName": realAssoc[8],
                        }
            ...
            if len(fake_props) > 0:
                for fake_prop in fake_props:
                    ...
                    asset_values = self.db_proxy.getValueTypes(
                        "asset_value", fake_prop["theEnvironmentName"]
                    )
                    rev_prop_dict = {}
                    for asset_value in asset_values:
                    ...
                    for idx in range(0, len(fake_prop["theAssociations"])):
                        ...
                        fake_prop["theAssociations"][idx] = (
                            fakeAssoc["theHeadNav"],
                            fakeAssoc["theHeadType"],
                            fakeAssoc["theHeadMultiplicity"],
                            fakeAssoc["theHeadRole"],
                            fakeAssoc["theTailRole"],
                            fakeAssoc["theTailMultiplicity"],
                            fakeAssoc["theTailType"],
                            fakeAssoc["theTailNav"],
                            fakeAssoc["theTailName"],
                        )
    ...
    def simplify(self, asset):
        ...
        asset.theEnvironmentProperties = self.convert_props(
            real_props=asset.theEnvironmentProperties
        )
    ...
    def get_asset_association(self, environment_name, head_name, tail_name):
        ...
        for key in assocs:
            ...
            if (envName == environment_name) and (
                ((headName == head_name) and (tailName == tail_name))
                or ((headName == tail_name) and (tailName == head_name))
            ):
                assoc = assocs[key]
                self.close()
                return assoc
