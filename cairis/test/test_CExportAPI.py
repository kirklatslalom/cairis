...
from cairis.mio.ArchitecturalPatternContentHandler import (
    ArchitecturalPatternContentHandler,
)
...
class CExportTests(CairisDaemonTestCase):
    ...
    def setUpClass(cls):
        ...
        importSecurityPatternsFile(
            os.environ["CAIRIS_SRC"] + "/test/test_security_patterns.xml", "test"
        )
    ...
    def test_cexport_architecturalpattern_get(self):
        ...
        self.assertIsNotNone(rv.data, "No response")
        self.assertIsNone(
            xml.sax.parseString(rv.data, ArchitecturalPatternContentHandler())
        )
