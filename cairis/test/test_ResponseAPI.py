...
class ResponseAPITests(CairisDaemonTestCase):
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            responses, list, "The result is not a dictionary as expected"
        )
    ...
    def test_get_by_name(self):
        ...
        url = "/api/responses/name/%s?session_id=test" % quote(
            self.existing_response_name
        )
        rv = self.app.get(url)
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/responses/name/%s?session_id=test" % quote(
            self.prepare_new_response().theName
        )
        ...
        self.app.post(
            "/api/responses", content_type="application/json", data=new_response_body
        )
    ...
    def test_new_accept_post(self):
        ...
        self.app.delete(
            "/api/responses/name/%s?session_id=test"
            % quote(self.prepare_new_response().theName)
        )
    ...
    def test_new_mitigate_post(self):
        ...
        rv = self.app.post(
            "/api/responses/name/%s/generate_goal?session_id=test"
            % quote(self.prepare_new_response().theName)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
    ...
    def test_put(self):
        ...
        self.assertGreater(
            message.find("updated"), -1, "The response was not successfully updated"
        )

        rv = self.app.get(
            "/api/responses/name/%s?session_id=test" % quote(response_to_update.theName)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
            ...
            new_response_props = {
                "transfer": [
                    TransferEnvironmentProperties(
                        environmentName=self.existing_environment_name,
                        rationale="Test",
                        roles=[(self.existing_role_name, "Low")],
                    )
                ],
                "accept": [],
                "mitigate": [],
