...
def main():
    parser = argparse.ArgumentParser(
        description="Computer Aided Integration of Requirements and Information Security - Restore server"
    )
    ...
    with tarfile.open(args.archiveFile, "r") as archive:
        ...
        for modelFile in modelFiles:
            if modelFile.name != "hashes.txt":
                ...
                try:
                    ...
                    logger.info(
                        "Error re-creating account " + accountName + " : " + str(ex)
                    )
            else:
                hashes = list(
                    map(
                        lambda x: x.decode().rstrip().split(","),
                        archive.extractfile(modelFile).readlines(),
                    )
                )
        for accountName, model in models:
            try:
                ...
                logger.info(
                    "Error re-importing default model for account "
                    + accountName
                    + " : "
                    + str(ex)
                )
