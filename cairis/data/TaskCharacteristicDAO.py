...
class TaskCharacteristicDAO(CairisDAO):
    ...
    def get_object_by_name(self, task_characteristic_name):
        ...
        raise ObjectNotFoundHTTPError(
            'Task characteristic:"' + task_characteristic_name + '"'
        )
    ...
    def from_json(self, request, to_props=False):
        ...
        json_dict["__python_obj__"] = (
            TaskCharacteristic.__module__ + "." + TaskCharacteristic.__name__
        )
        ...
        if real_tc is not None:
            ...
            if len(real_tc.theGrounds) > 0:
                for real_tcr in real_tc.theGrounds:
                    tcr_list.append(
                        CharacteristicReference(
                            real_tcr[0], "grounds", real_tcr[1], real_tcr[2]
                        )
                    )
                ...
                for real_tcr in real_tc.theWarrant:
                    tcr_list.append(
                        CharacteristicReference(
                            real_tcr[0], "warrant", real_tcr[1], real_tcr[2]
                        )
                    )
                ...
                for real_tcr in real_tc.theRebuttal:
                    tcr_list.append(
                        CharacteristicReference(
                            real_tcr[0], "rebuttal", real_tcr[1], real_tcr[2]
                        )
                    )
            ...
            if len(fake_tc.theGrounds) > 0:
                for tcr in fake_tc.theGrounds:
                    tcr_list.append(
                        (
                            tcr["theReferenceName"],
                            tcr["theReferenceDescription"],
                            tcr["theDimensionName"],
                        )
                    )
