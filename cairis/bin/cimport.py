...
def main(args=None):
    parser = argparse.ArgumentParser(
        description="Computer Aided Integration of Requirements and Information Security - Model Import"
    )
    ...
    parser.add_argument(
        "--type",
        dest="modelFormat",
        help="model type to import.  One of securitypattern, attackpattern, tvtypes, directory, requirements, riskanalysis, usability, misusability, project, domainvalues, architecturalpattern, associations, synopses, processes, assets, locations, dataflows, stories, package, or all",
        default="all",
    )
    parser.add_argument(
        "--overwrite",
        dest="isOverwrite",
        help="Where appropriate, overwrite an existing CAIRIS model with this model",
        default=1,
    )
    parser.add_argument(
        "--image_dir",
        dest="imageDir",
        help="Where appropriate, directory for model images (overwrites default_image_dir value in cairis.cnf)",
    )
    ...
    for fileName in fileList:
        ...
        if fType == "xml":
            ...
            if modelType == "cairis_model":
                if models[modelType] != "":
                    raise ARMException(
                        "Cannot have more than one CAIRIS model file in the package file"
                    )
...
def file_import(importFile, mFormat, overwriteFlag, session_id=None):
    ...
    from cairis.mio.ModelImport import (
        ...
        importRiskAnalysisFile,
        importUsabilityFile,
        importAssociationsFile,
        importProjectFile,
