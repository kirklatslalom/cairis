...
class TaskCharacteristicAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.new_tc = TaskCharacteristic(
            ...
            pcGrounds=[
                {
                    "theReferenceName": "Acknowledge target device",
                    "theDimensionName": "usecase",
                    "theCharacteristicType": "grounds",
                    "theReferenceDescription": "A test grounds description",
                }
            ],
            pcWarrant=[],
            pcRebuttal=[],
        ...
        self.new_tc_dict = {"session_id": "test", "object": self.new_tc}
    ...
    def test_get_by_name(self):
        ...
        url = "/api/task_characteristics/name/%s?session_id=test" % quote(
            self.existing_tc_name
        )
        rv = self.app.get(url)
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            ...
            data=jsonpickle.encode(self.new_tc_dict),
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
