...
def buildConceptMap(p, envName, graphName):
    ...
    model = ConceptMapModel(
        list(p.conceptMapModel(envName).values()), envName, "", True
    )
    if model.size() == 0:
        return False
...
def exportUserGoalWorkbook(outFile, session_id=None):
    ...
    hFormat = wb.add_format(
        {"border": 1, "bg_color": "#C6EFCE", "bold": True, "text_wrap": True}
    )
    unlocked = wb.add_format(
        {"locked": False, "text_wrap": True, "font_color": "green"}
    )
    ...
    for objt in pcs + list(drSet):
        ...
        ugSheet.data_validation(
            "E" + str(ugRow),
            {"validate": "list", "source": ["goal", "softgoal", "belief"]},
        )
    ...
    for pc in pcs:
        for e in pc.grounds() + pc.warrant() + pc.rebuttal():
            ...
            contSheet.write_formula(
                contRow, 1, "=UserGoal!" + cellDict[pc.characteristic()], tWrap
            )
            contSheet.data_validation(
                "C" + str(contRow + 1), {"validate": "list", "source": ["means", "end"]}
            )
...
def exportPersonaCharacteristicsWorkbook(outFile, session_id=None):
    ...
    for dr in drs:
        ...
        drSheet.data_validation(
            "B" + str(drRow + 1),
            {"validate": "list", "source": "='External Documents'!$A$2:$A$5000"},
        )
    ...
    for pc in pcs:
        ...
        pcSheet.write(
            "E" + str(pcRow + 1),
            ",".join(list(map(lambda x: x[0], pc.grounds()))),
            wrapped,
        )
        pcSheet.write(
            "F" + str(pcRow + 1),
            ",".join(list(map(lambda x: x[0], pc.warrant()))),
            wrapped,
        )
        pcSheet.write(
            "G" + str(pcRow + 1),
            ",".join(list(map(lambda x: x[0], pc.rebuttal()))),
            wrapped,
        )
        pcRow += 1
