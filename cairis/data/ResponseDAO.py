...
from cairis.tools.ModelDefinitions import (
    ResponseModel,
    ResponseEnvironmentPropertiesModel,
    AcceptEnvironmentPropertiesModel,
    MitigateEnvironmentPropertiesModel,
    TransferEnvironmentPropertiesModel,
)
...
class ResponseDAO(CairisDAO):
    ...
    def from_json(self, request):
        ...
        try:
            real_props = self.convert_props(
                fake_props=property_dict, response_type=json_dict["theResponseType"]
            )
    ...
    def convert_props(self, real_props=None, fake_props=None, response_type=None):
        ...
        if real_props:
            ...
            for idx in range(0, len(real_props)):
                ...
                if (
                    isinstance(real_prop, AcceptEnvironmentProperties)
                    and response_type == "accept"
                ):
                    new_props_list.append(real_prop)
                elif (
                    isinstance(real_prop, MitigateEnvironmentProperties)
                    and response_type == "mitigate"
                ):
                    new_props_list.append(real_prop)
                elif (
                    isinstance(real_prop, TransferEnvironmentProperties)
            ...
            for fake_prop_key in fake_props:
                ...
                if len(fake_prop_list) > 0:
                    for fake_prop in fake_prop_list:
                        ...
                        fake_prop["__python_obj__"] = (
                            target_class.__module__ + "." + target_class.__name__
                        )
    ...
    def simplify(self, obj):
        ...
        del obj.costLookup
        try:
            obj.theEnvironmentProperties = self.convert_props(
                real_props=obj.theEnvironmentProperties,
                response_type=obj.theResponseType,
            )
        except MalformedJSONHTTPError as ex:
            self.close()
            raise ex
    ...
    def generate_goal(self, responseName, pathValues=[]):
        try:
            ...
            responses = self.db_proxy.getResponses(respId)
            goalParameters = cairis.core.GoalFactory.build(
                responses[responseName], self.db_proxy
            )
            if goalParameters == None:
                raise ARMException(
                    "Error generating a goal. Cannot generate goals from Accept responses"
                )
