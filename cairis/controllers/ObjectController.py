...
class ObjectsAPI(Resource):
    def __init__(self, **kwargs):
        self.DAOModule = getattr(
            import_module("cairis.data." + kwargs["dao"]), kwargs["dao"]
        )
        ...
        if self.DAOModule.__name__ == "ExportDAO":
            ...
            if self.theGetMethod == "file_export":
                resp.headers["Content-Disposition"] = (
                    "Attachment; filename=" + pathValues[0] + "." + pathValues[1]
                )
            elif (
                self.theGetMethod == "user_goals_export"
                or self.theGetMethod == "persona_characteristics_export"
            ):
                resp.headers["Content-Disposition"] = (
                    "Attachment; filename=" + pathValues[0] + ".xlsx"
                )
            else:
                resp.headers["Content-Disposition"] = (
                    "Attachment; filename=" + pathValues[0]
                )
        else:
            resp = make_response(json_serialize(objts, session_id=session_id), OK)
            resp.contenttype = "application/json"
...
class ObjectsByMethodAndThreeParametersAPI(Resource):
    ...
    def get(self, p1, p2, p3):
        ...
        if self.DAOModule.__name__ == "ExportDAO":
            ...
            resp.headers["Content-Disposition"] = (
                "Attachment; filename=" + fileName + ".grl"
            )
        else:
            resp = make_response(json_serialize(objts, session_id=session_id), OK)
            resp.contenttype = "application/json"
...
class ObjectsByMethodAndParameterAPI(Resource):
    ...
    def get(self, parameter_string):
        ...
        pathValues = []
        for parameterName, defaultValue in self.thePathParameters:
            if self.DAOModule.__name__ == "CountermeasureDAO" and parameterName in [
                "requirement",
                "role",
            ]:
        ...
        if self.DAOModule.__name__ == "ExportDAO":
            ...
            resp.headers["Content-Type"] = "application/xml"
            resp.headers["Content-Disposition"] = (
                "Attachment; filename=" + pathValues[0] + ".xml"
            )
    ...
    def get(self, parameter_string):
        ...
        resp = make_response(
            model_generator.generate(
                dot_code, model_type=self.theModelType, renderer=self.theRenderer
            ),
            OK,
        )
    ...
    def post(self):
        ...
        try:
            ...
            raise CairisHTTPError(
                status_code=CONFLICT, message=str(ex), status="Unknown error"
            )
...
class ObjectsByMethodAndFiveParametersAPI(Resource):
    ...
    def put(self, p1, p2, p3, p4, p5):
        ...
        resp_dict = {
            "message": p1 + "/" + p2 + "/" + p3 + "/" + p4 + "/" + p5 + " updated"
        }
        resp = make_response(json_serialize(resp_dict, session_id=session_id), OK)
        resp.contenttype = "application/json"
        return resp
    ...
    def delete(self, p1, p2, p3, p4, p5):
        ...
        resp_dict = {
            "message": p1 + "/" + p2 + "/" + p3 + "/" + p4 + "/" + p5 + " deleted"
        }
        resp = make_response(json_serialize(resp_dict, session_id=session_id), OK)
        resp.contenttype = "application/json"
        return resp
