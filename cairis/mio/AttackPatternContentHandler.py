...
class AttackPatternContentHandler(ContentHandler, EntityResolver):
    ...
    def obstacles(self):
        return self.theObstacleParameters

    def obstacleAssociations(self):
        return self.theObstacleAssociationParameters
    ...
    def vulnerability(self):
        return self.theVulnerabilityParameters

    def threat(self):
        return self.theThreatParameters

    def risk(self):
        return self.theRiskParameters
    ...
    def endElement(self, name):
        if name == "intent":
            ...
            self.theParticipants.append(
                (self.theParticipant, self.theMotives, self.theResponsibilities)
            )
            self.resetParticipantElements()
        elif name == "description":
            self.inDescription = 0
            ...
            for assetName in assetList:
                ...
                if assetId == -1:
                    ...
                    if taId == -1:
                        raise ARMException(
                            "Cannot import attack pattern: no asset or template asset for "
                            + assetName
                        )
                    else:
            ...
            for tObs in self.theObstacles:
                ...
                self.theObstacleParameters.append(
                    ObstacleParameters(tObs.name(), self.thePatternName, [], [ep])
                )
            ...
            vp = VulnerabilityEnvironmentProperties(
                self.theEnvironment, self.theSeverity, self.theExploits
            )
            ...
            if len(vulRows) == 0:
                vulData = [
                    ...
                    "",
                    self.theExploit,
                    self.dbProxy.defaultValue("vulnerability_type"),
                ]
            else:
                vulData = vulRows[0]
            ...
            if len(thrRows) == 0:
                thrData = [
                    ...
                    "",
                    self.theExploit,
                    self.dbProxy.defaultValue("threat_type"),
                ]
            else:
                thrData = thrRows[0]
            ...
            rep = MisuseCaseEnvironmentProperties(
                self.theEnvironment, self.theImplementation
            )
            mc = MisuseCase(
                -1, "Exploit " + self.thePatternName, [rep], self.thePatternName
            )
