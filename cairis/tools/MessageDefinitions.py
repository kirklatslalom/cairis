...
class AssetEnvironmentPropertiesMessage(DefaultMessage):
    resource_fields = gen_message_multival_fields(
        ModelDefinitions.AssetEnvironmentPropertiesModel
    )
...
class CountermeasureTaskMessage(DefaultMessage):
    resource_fields = fields.List(
        fields.Nested(ModelDefinitions.CountermeasureTask.resource_fields)
    )
    required = DefaultMessage.required
