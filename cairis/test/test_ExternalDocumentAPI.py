...
class ExternalDocumentAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.new_edoc_dict = {"session_id": "test", "object": self.new_edoc}
    ...
    def test_get_by_name(self):
        ...
        url = "/api/external_documents/name/%s?session_id=test" % quote(
            self.existing_edoc_name
        )
        rv = self.app.get(url)
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            ...
            data=jsonpickle.encode(self.new_edoc_dict),
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_put(self):
        ...
        rv = self.app.put(
            url, content_type="application/json", data=jsonpickle.encode(upd_dict)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
