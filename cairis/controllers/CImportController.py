...
from cairis.tools.SessionValidator import (
    validate_proxy,
    check_required_keys,
    get_session_id,
)
...
class CImportPackageAPI(Resource):
    def post(self):
        ...
        try:
            ...
            raise CairisHTTPError(
                status_code=500, message=str(ex), status="Unknown error"
            )
    ...
    def post(self):
        ...
        if file_contents.startswith("<?xml"):
            ...
            try:
                ...
                dao = ImportDAO(session_id)
                result = dao.import_attack_tree(
                    file_contents, environment_name, contributor_name
                )
                dao.close()
                message = str(result)
                if result == 0:
