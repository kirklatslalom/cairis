...
class AssetAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.new_asset_sec_attr = [
            SecurityAttribute("Accountability", "Low", "None"),
            SecurityAttribute("Confidentiality", "Medium", "None"),
        ]
        ...
        self.new_asset_dict = {"session_id": "test", "object": self.new_asset}
        # endregion
    ...
    def test_post(self):
        ...
        url = (
            "/api/assets/name/"
            + quote(self.new_asset_dict["object"].theName)
            + "?session_id=test"
        )
        rv = self.app.get(url)
        if sys.version_info > (3,):
    ...
    def test_types_get(self):
        ...
        self.assertIsInstance(
            assets, list, "The result is not a dictionary as expected"
        )
    ...
    def test_types_delete(self):
        ...
        url = "/api/assets/types/name/%s?session_id=test" % quote(
            self.prepare_new_asset_type().theName
        )
    ...
    def test_types_post(self):
        ...
        rv = self.app.post(
            url, content_type="application/json", data=new_asset_type_body
        )
        if sys.version_info > (3,):
            postResponse = rv.data.decode("utf-8")
        else:
        ...
        rv = self.app.delete(
            "/api/assets/types/name/%s?session_id=test"
            % quote(self.prepare_new_asset_type().theName)
        )
        delResponse = rv.data.decode("utf-8")
        self.assertIsNotNone(delResponse, "No response")
        json_resp = jsonpickle.decode(delResponse)
