...
class ComponentModel:
    def __init__(
        self, interfaces, connectors, db_proxy=None, font_name=None, font_size=None
    ):
        ...
        self.theGraph.set_node_defaults(
            shape="rectangle",
            pencolor="black",
            style="filled",
            colorscheme="reds3",
        ...
        self.theGraph.set_edge_defaults(
            dir="both", arrowhead="obox", arrowtail="obox", weight="1"
        )
        ...
        if componentName not in self.theComponentNames:
            ...
            self.theGraph.add_node(
                pydot.Node(
                    ...
                    margin=0,
                    fillcolor=resolveColour(
                        self.dbProxy.componentAttackSurface(componentName)
                    ),
                    URL=componentUrl,
                )
            )
    ...
    def buildConnector(self, cnName, fromName, toName, protocolName, arName):
        ...
        self.theGraph.add_edge(
            pydot.Edge(fromObjtName, toObjtName, label=lbl, URL=urlName)
        )
