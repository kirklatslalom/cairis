...
class GoalAssociationDAO(CairisDAO):
    ...
    def get_goal_association(
        self, environment_name, goal_name, subgoal_name, pathValues=[], deleteId=True
    ):
        try:
            assoc = self.db_proxy.getGoalAssociation(
                environment_name, goal_name, subgoal_name
            )
            if deleteId == True:
                del assoc.theId
            return assoc
    ...
    def add_goal_association(self, assoc, pathValues=[]):
        if assoc.theGoal == assoc.theSubGoal:
            raise CairisHTTPError(
                ...
                message="Cannot self-refine " + assoc.theGoal,
            )
    ...
    def update_goal_association(
        self, assoc, environment_name, goal_name, subgoal_name, pathValues=[]
    ):
        if assoc.theGoal == assoc.theSubGoal:
            raise CairisHTTPError(
                status_code=http.client.BAD_REQUEST,
        ...
        old_assoc = self.get_goal_association(
            environment_name, goal_name, subgoal_name, [], False
        )
    ...
    def delete_goal_association(
        self, environment_name, goal_name, subgoal_name, pathValues=[]
    ):
        assoc = self.get_goal_association(
            environment_name, goal_name, subgoal_name, [], False
        )
        try:
            self.db_proxy.deleteGoalAssociation(
                assoc.theId, assoc.theGoalDimension, assoc.theSubGoalDimension
            )
        except ARMException as ex:
            self.close()
            raise ARMHTTPError(ex)
    ...
    def from_json(self, request, to_props=False):
        ...
        json_dict["__python_obj__"] = (
            GoalAssociation.__module__ + "." + GoalAssociation.__name__
        )
