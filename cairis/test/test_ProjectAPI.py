...
class ProjectAPITests(CairisDaemonTestCase):
    ...
    def setUpClass(cls):
        ...
        createDatabaseAccount(
            b.rPasswd, b.dbHost, b.dbPort, "cairis_test", "cairis_test"
        )
        createDatabaseAndPrivileges(
            b.rPasswd,
            b.dbHost,
    ...
    def test_settings_get(self):
        ...
        self.assertIsInstance(
            json_dict, dict, "Response is not a valid JSON dictionary"
        )
        has_all_keys = all(k in json_dict for k in ProjectSettings.required)
        self.assertTrue(
            has_all_keys, "The response is not a valid ProjectSettings object"
        )
    ...
    def test_settings_put(self):
        ...
        json_dict = jsonpickle.decode(responseData)
        self.logger.info(
            "[%s] Current project name: %s", method, json_dict["projectName"]
        )
        ...
        self.assertIsInstance(
            new_json_dict, dict, "Response is not a valid JSON dictionary"
        )
    ...
    def test_clear_project(self):
        ...
        data = {"session_id": "test", "file": (buf, "import.xml")}
        ...
        self.logger.info(
            "[%s] Successfully created new project and restored the example project\n",
            method,
        )
    ...
    def test_create_new_database(self):
        ...
        self.logger.info("[%s] Message: %s", method, message)
        self.assertGreater(
            message.find("successfully"), -1, "Failed to create new database"
        )
    ...
    def test_database_invalidname_exception(self):
        ...
        self.logger.info("[%s] Message: %s", method, message)
        self.assertGreater(
            message.find("nodb does not exist"), -1, "Error expected but not raised"
        )
        url = "/api/settings/database/default/open?session_id=test"
        rv = self.app.post(url)
    ...
    def test_database_nullname_exception(self):
        ...
        self.logger.info("[%s] Message: %s", method, message)
        self.assertGreater(
            message.find("No database name defined"),
            -1,
            "Error expected but not raised",
        )
        url = "/api/settings/database/default/open?session_id=test"
        rv = self.app.post(url)
    ...
    def test_open_database(self):
        ...
        self.logger.info("[%s] Message: %s", method, message)
        self.assertGreater(
            message.find("successfully"), -1, "Failed to create testopendb"
        )
    ...
    def test_show_databases(self):
        ...
        message = str(json_dict["message"])
        self.assertGreater(
            message.find("successfully"), -1, "Failed to create testshowdb"
        )
        ...
        message = str(json_dict["message"])
        self.assertGreater(
            message.find("successfully"), -1, "Failed to open default databases"
        )
    ...
    def test_delete_database(self):
        ...
        message = str(json_dict["message"])
        self.assertGreater(
            message.find("successfully"), -1, "Failed to create testdeldb"
        )
        ...
        message = str(json_dict["message"])
        self.assertGreater(
            message.find("successfully"), -1, "Failed to open cairis_default"
        )
        ...
        message = str(json_dict["message"])
        self.assertGreater(
            message.find("successfully"), -1, "Failed to delete testdeldb"
        )
