...
class PolicyStatementDAO(CairisDAO):
    ...
    def get_object_by_name(
        self, goal_name, env_name, subject, access_type, resource, pathValues=[]
    ):
        psId = self.db_proxy.getDimensionId(
            goal_name
            + "/"
            + env_name
            + "/"
    ...
    def update_object(
        self, ps, goal_name, env_name, subject, access_type, resource, pathValues=[]
    ):
        psParams = PolicyStatement(
            -1,
            ps.theGoalName,
            ps.theEnvironmentName,
            ps.theSubject,
            ps.theAccessType,
            ps.theResource,
            ps.thePermission,
        )
        if (env_name == ps.theEnvironmentName) and (
            ps.theSubject != subject
            or ps.theAccessType != access_type
            or ps.theResource != resource
        ):
            if self.db_proxy.nameExists(ps.name(), "policy_statement"):
                raise ARMException(
                    "Policy statement for " + ps.name() + " already exists."
                )
        try:
            psId = self.db_proxy.getDimensionId(
                goal_name
    ...
    def from_json(self, request, to_props=False):
        ...
        json_dict["__python_obj__"] = (
            PolicyStatement.__module__ + "." + PolicyStatement.__name__
        )
