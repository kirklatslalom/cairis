...
class ThreatAPITests(CairisDaemonTestCase):
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/threats/name/%s?session_id=test" % quote(
            self.prepare_new_threat().theName
        )
        ...
        self.app.post(
            "/api/threats", content_type="application/json", data=new_threat_body
        )
    ...
    def test_post(self):
        ...
        self.app.delete(
            "/api/threats/name/%s?session_id=test"
            % quote(self.prepare_new_threat().theName)
        )
    ...
    def test_put(self):
        ...
        self.assertGreater(
            message.find("updated"), -1, "The threat was not successfully updated"
        )

        rv = self.app.get(
            "/api/threats/name/%s?session_id=test" % quote(threat_to_update.theName)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def prepare_new_threat(self):
        new_security_attrs = [
            SecurityAttribute(
                name="Confidentiality", value="High", rationale="As a test"
            ),
            SecurityAttribute(
                name="Availability", value="Medium", rationale="Another test"
            ),
        ]

        more_security_attrs = [
            SecurityAttribute(name="Confidentiality", value="High", rationale="Test")
        ]
