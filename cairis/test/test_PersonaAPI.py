...
class PersonaAPITests(CairisDaemonTestCase):
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            personas, dict, "The result is not a dictionary as expected"
        )
    ...
    def test_get_all_summary(self):
        ...
        self.logger.info(
            "[%s] First persona summary: %s [%s]\n", method, ps[0]["theName"]
        )

    def test_get_by_name(self):
        method = "test_get_by_name"
        url = "/api/personas/name/%s?session_id=test" % quote(
            self.existing_persona_name
        )
        rv = self.app.get(url)
    ...
    def test_delete(self):
        ...
        self.app.post(
            "/api/personas", content_type="application/json", data=new_persona_body
        )
    ...
    def test_post(self):
        ...
        self.app.delete(
            "/api/personas/name/%s?session_id=test"
            % quote(self.prepare_new_persona().name())
        )
    ...
    def test_put(self):
        ...
        rv = self.app.delete(
            "/api/personas/name/%s?session_id=test"
            % quote(self.prepare_new_persona().theName)
        )
        rv = self.app.post(url, content_type="application/json", data=new_persona_body)
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        ...
        self.assertGreater(
            message.find("updated"), -1, "The persona was not successfully updated"
        )

        rv = self.app.get(
            "/api/personas/name/%s?session_id=test" % quote(persona_to_update.name())
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        rv = self.app.delete(
            "/api/personas/name/%s?session_id=test" % quote(persona_to_update.theName)
        )
