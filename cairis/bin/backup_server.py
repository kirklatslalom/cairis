...
def main():
    parser = argparse.ArgumentParser(
        description="Computer Aided Integration of Requirements and Information Security - Backup server"
    )
    ...
    for email in aNames:
        ...
        try:
            ...
            logger.info(
                "Exported " + email + " default database by ignoring validity checks"
            )
        dbProxy.close()
    ...
    with tarfile.open(args.tarArchive, "w") as archive:
        for p in os.listdir(b.tmpDir):
            ...
            archive.add(
                modelName,
                arcname=p,
                filter=lambda x: x if x.name.endswith(".cairis") else None,
            )
