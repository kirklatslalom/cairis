...
class TaskAPITests(CairisDaemonTestCase):
    ...
    def test_load_by_name(self):
        ...
        url = (
            "/api/tasks/name/"
            + quote(self.existing_task_name)
            + "/environment/Psychosis/load?session_id=test"
        )
        rv = self.app.get(url)
        self.assertIsNotNone(rv.data, "No response")
        if sys.version_info > (3,):
    ...
    def test_hindrance_by_name(self):
        ...
        url = (
            "/api/tasks/name/"
            + quote(self.existing_task_name)
            + "/environment/Psychosis/hindrance?session_id=test"
        )
        rv = self.app.get(url)
        self.assertIsNotNone(rv.data, "No response")
        if sys.version_info > (3,):
    ...
    def test_score_by_name(self):
        ...
        url = (
            "/api/tasks/name/"
            + quote(self.existing_task_name)
            + "/environment/Psychosis/score?session_id=test"
        )
        rv = self.app.get(url)
        self.assertIsNotNone(rv.data, "No response")
        if sys.version_info > (3,):
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/tasks/name/%s?session_id=test" % quote(
            self.prepare_new_task().name()
        )
    ...
    def test_post(self):
        ...
        self.app.delete(
            "/api/tasks/name/%s?session_id=test" % quote(self.prepare_new_task().name())
        )
    ...
    def test_put(self):
        ...
        rv = self.app.put(
            "/api/tasks/name/%s?session_id=test"
            % quote(self.prepare_new_task().name()),
            data=upd_env_body,
        ...
        self.assertGreater(
            message.find("updated"), -1, "The task was not successfully updated"
        )

        rv = self.app.get(
            "/api/tasks/name/%s?session_id=test" % quote(task_to_update.name())
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        rv = self.app.delete(
            "/api/tasks/name/%s?session_id=test" % quote(task_to_update.theName)
        )
