...
class AssetAssociationDAO(CairisDAO):
    ...
    def get_asset_association(
        self, environment_name, head_name, tail_name, pathValues=[]
    ):
    ...
    def update_asset_association(
        ...
        try:
            if (
                (assoc.theEnvironmentName != oldEnvName)
                or (assoc.theHeadAsset != oldHeadAsset)
                or (assoc.theTailAsset != oldTailAsset)
            ):
                self.db_proxy.checkAssetAssociation(
                    assoc.theEnvironmentName, assoc.theHeadAsset, assoc.theTailAsset
                )
            caId = self.db_proxy.getDimensionId(
                oldEnvName + "/" + oldHeadAsset + "/" + oldTailAsset, "classassociation"
            )
    ...
    def delete_asset_association(
        self, environment_name, head_name, tail_name, pathValues=[]
    ):
        try:
            caId = self.db_proxy.getDimensionId(
                environment_name + "/" + head_name + "/" + tail_name, "classassociation"
            )
    ...
    def from_json(self, request, to_props=False):
        ...
        json_dict["__python_obj__"] = (
            ClassAssociation.__module__ + "." + ClassAssociation.__name__
        )
