...
from cairis.tools.ModelDefinitions import (
    AttackerModel,
    AttackerEnvironmentPropertiesModel,
)
...
class AttackerDAO(CairisDAO):
    ...
    def get_attacker_capabilities(self, pathValues):
        try:
            ...
            attacker_capabilities = self.db_proxy.getValueTypes(
                "capability", environment_name
            )
    ...
    def add_attacker_capability(self, attacker_capability, pathValues):
        ...
        environment_name = pathValues[0]
        type_exists = self.check_existing_attacker_capability(
            attacker_capability.theName, environment_name=environment_name
        )
            ...
            if len(real_props) > 0:
                for real_prop in real_props:
                    ...
                    for capability in real_prop.theCapabilities:
                        if len(capability) == 2:
                            capabilities.append(
                                {"name": capability[0], "value": capability[1]}
                            )
