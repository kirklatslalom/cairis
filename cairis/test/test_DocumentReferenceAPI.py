...
class DocumentReferenceAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.new_dr_dict = {"session_id": "test", "object": self.new_dr}
    ...
    def test_get_by_name(self):
        ...
        url = "/api/document_references/name/%s?session_id=test" % quote(
            self.existing_dr_name
        )
        rv = self.app.get(url)
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            ...
            data=jsonpickle.encode(self.new_dr_dict),
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
