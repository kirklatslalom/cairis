...
class KaosTextShape(xdot.TextShape):
    ...
    def __init__(self, pen, x, y, j, w, t, modelType, dim, toDim, cSet=False):
        ...
        if (
            (dim == "persona")
            or (dim == "attacker")
            or ((dim == "requirement") and cSet == True)
        ):
            self.y = y + 22
    ...
    def draw(self, cr, highlight=False, zoom_ratio=-1):
        if (
            (
                (self.fromDim == "task" and self.toDim == "asset")
                or (self.fromDim == "usecase" and self.toDim == "asset")
                or (self.fromDim == "misusecase" and self.toDim == "asset")
                or (self.theModelType == "task" and self.fromDim == "asset")
                or (self.fromDim == "comment")
                or (self.fromDim == "goalconcern")
                or (self.fromDim == "obstacleconcern")
                or (self.fromDim == "taskconcern")
            )
            and (zoom_ratio < MEDKAOS_ZOOM_RATIO)
        ) or (
            self.fromDim == "task"
            and self.toDim == "misusecase"
            and zoom_ratio < LOWKAOS_ZOOM_RATIO
        ):
            pass
        else:
...
class KaosPolygonShape(xdot.PolygonShape):
    ...
    def draw(self, cr, highlight=False, zoom_ratio=-1):
        if (
            (
                (
                    (self.theModelType == "task" and self.dim == "asset")
                    or (self.theModelType == "class" and self.dim == "comment")
                    or (self.theModelType == "class" and self.dim == "goalconcern")
                    or (self.theModelType == "class" and self.dim == "obstacleconcern")
                    or (self.theModelType == "class" and self.dim == "taskconcern")
                )
                and (zoom_ratio < MEDKAOS_ZOOM_RATIO)
            )
            or (
                self.assocType
                in (
                    "taskmisusecasethreat_association",
                    "taskmisusecasevulnerability_association",
            ...
            or (
                self.assocType
                in (
                    "misusecasethreatasset_association",
                    "misusecasevulnerabilityasset_association",
                    "misusecasethreatmitigation_association",
                    "misusecasevulnerabilitymitigation_association",
...
class KaosXDotAttrParser(xdot.XDotAttrParser):
    ...
    def handle_ellipse(self, x0, y0, w, h, filled=False):
        if self.dim == "attacker":
            ...
            if filled:
                self.shapes.append(
                    RequirementShape(
                        self.pen, x0, y0, w, h, self.objt, self.dbProxy, filled=True
                    )
                )
            self.shapes.append(
                RequirementShape(self.pen, x0, y0, w, h, self.objt, self.dbProxy)
            )
        else:
            if filled:
                self.shapes.append(
                    KaosEllipseShape(self.pen, x0, y0, w, h, filled=True)
                )
    ...
    def handle_polygon(self, points, filled=False):
        ...
        self.shapes.append(
            KaosPolygonShape(
                self.theModelType, self.dim, self.assocType, self.pen, points
            )
        )
    ...
    def handle_node(self, id, attrs):
        ...
        for attr in ("_draw_", "_ldraw_"):
            if attr in attrs:
                ...
                parser = KaosXDotAttrParser(
                    self, attrs[attr], self.theModelType, dimObjt
                )
                if self.theModelType == "conceptmap":
                    parser.cfSet = self.cfSet
...
class KaosDotWindow(gtk.Window):
    ...
    def loadFilters(self, environments, goals=[], cs=[]):
        ...
        self.environmentHandlerId = self.environmentCombo.connect(
            "changed", self.onEnvironmentChange
        )
    ...
    def refreshModel(self):
        ...
        try:
            ...
            if self.theModelType == "class":
                ...
                associationDictionary = proxy.classModel(
                    environmentName, goalName, hideConcerns
                )
                self.canonicalModel = AssetModel(
                ...
                associationDictionary = proxy.goalModel(
                    environmentName, goalName, topLevelGoals, caseFilter
                )
                self.canonicalModel = KaosModel(
                    associationDictionary.values(),
                    environmentName,
                    self.theModelType,
                    goalName,
                ...
                topLevelGoals = self.hideCheck.get_active()
                associationDictionary = proxy.obstacleModel(
                    environmentName, goalName, topLevelGoals
                )
                self.canonicalModel = KaosModel(
                    associationDictionary.values(),
                    environmentName,
                ...
                associationDictionary = proxy.responsibilityModel(
                    environmentName, goalName
                )
                self.canonicalModel = KaosModel(
                    associationDictionary.values(),
                    environmentName,
                ...
                self.canonicalModel = ConceptMapModel(
                    associationDictionary.values(),
                    environmentName,
                    self.theModelType,
                    cfSet,
                ...
                riskOverlay = proxy.locationsRiskModel(
                    self.theLocationsName, environmentName
                )
                self.canonicalModel = LocationModel(
                    self.theLocationsName, environmentName, riskOverlay
                )
            else:
                associationDictionary = proxy.taskModel(
                    environmentName, goalName, caseFilter
                )
                self.canonicalModel = KaosModel(
                    associationDictionary.values(),
                    environmentName,
        ...
        if chooser.run() == gtk.RESPONSE_OK:
            ...
            if (fileType != "svg") and (fileType != "pdf"):
                dlg = gtk.MessageDialog(
                    ...
                    message_format="Unknown file type",
