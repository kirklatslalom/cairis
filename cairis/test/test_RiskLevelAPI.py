...
class RiskLevelAPITests(CairisDaemonTestCase):
    ...
    def test_get_risk_level(self):
        ...
        url = "/api/risk_level/asset/%s?session_id=test" % quote(
    ...
    def test_get_risk_level_by_environment(self):
        ...
        url = (
            "/api/risk_level/asset/ICT%20Application/environment/Night?session_id=test"
        )
        self.logger.info("[%s] URL: %s", method, url)
        rv = self.app.get(url)
        if sys.version_info > (3,):
    ...
    def test_get_risk_threat_level(self):
        method = "test_get_risk_level"
        url = (
            "/api/risk_level/asset/threat_type/"
            + quote(self.existing_asset_name)
            + "/"
