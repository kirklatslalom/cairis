...
class EnvironmentDAO(CairisDAO):
    ...
    def get_environment_names_by_vulnerability_threat(
        self, vulnerability_name, threat_name, pathValues=[]
    ):
        return self.get_environment_names_by_threat_vulnerability(
            threat_name, vulnerability_name, pathValues
        )

    def get_environment_names_by_threat_vulnerability(
        self, threat_name, vulnerability_name, pathValues=[]
    ):
        try:
            environments = self.db_proxy.riskEnvironments(
                threat_name, vulnerability_name
            )
            return environments
        except DatabaseProxyException as ex:
            self.close()
        ...
        try:
            if not self.check_existing_environment(environment.theName):
                ...
                raise DatabaseProxyException(
                    "Environment name already exists within the database."
                )
        except DatabaseProxyException as ex:
            self.close()
            raise ARMHTTPError(ex)
