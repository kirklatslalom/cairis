...
class UserGoalDAO(CairisDAO):
        ...
        try:
            ...
            if len(ugPCs) == 0:
                ...
                excTxt = (
                    ...
                    + "' with "
                    + ug.theActor
                    + " because it is associated with persona characteristic"
                )
                if len(ugPCs) > 1:
                    excTxt += "s " + ", ".join(ugPCs)
                else:
    ...
    def from_json(self, request, to_props=False):
        ...
        json_dict["__python_obj__"] = (
            ReferenceSynopsis.__module__ + "." + ReferenceSynopsis.__name__
        )
    ...
    def get_user_goal_model(
        self, environment_name, persona_name, filter_element, pathValues=[]
    ):
        ...
        if filter_element == "all":
            filter_element = ""
        try:
            gcs = self.db_proxy.getGoalContributions(
                environment_name, persona_name, filter_element
            )
            ugm = UserGoalModel(
                gcs,
                environment_name,
