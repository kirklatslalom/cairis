...
def arrayToAssetSecurityPropertiesTable(spArray, objtName):
    ...
    buf += (
        '<TR><TD colspan="3" bgcolor="white" border="0" align="center">'
        + objtName
        + "</TD></TR>"
    )
    ci = 0
...
def arrayToThreatSecurityPropertiesTable(spArray, objtName):
    ...
    for x in spArray:
        if x == 0:
            ...
            if x == 1:
                buf += (
                    '<TR><TD border="0" colspan="2"></TD><TD bgcolor="'
                    + colorScheme[ci]
                    + '"></TD></TR>'
                )
            elif x == 2:
                buf += (
                    '<TR><TD border="0" colspan="1"></TD><TD bgcolor="'
                    + colorScheme[ci]
                    + '" colspan="2"></TD></TR>'
                )
            else:
                buf += (
                    '<TR><TD bgcolor="' + colorScheme[ci] + '" colspan="3"></TD></TR>'
                )
        ci += 1
    buf += "</TABLE>>"
    return buf
            ...
            self.theGraph.add_node(
                pydot.Node(
                    ...
                    label=arrayToAssetSecurityPropertiesTable(
                        assetObjt.securityProperties(self.theEnvironmentName), objtName
                    ),
                )
            )
        elif dimName == "threat":
            ...
            self.theGraph.add_node(
                pydot.Node(
                    ...
                    label=arrayToThreatSecurityPropertiesTable(
                        thrObjt.securityProperties(self.theEnvironmentName), objtName
                    ),
                )
            )
