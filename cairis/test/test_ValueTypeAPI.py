...
class ValueTypeAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.new_vt_dict = {"session_id": "test", "object": self.new_vt}
        self.existing_vt_dict = {"session_id": "test", "object": {}}
    ...
    def test_get_all(self):
        ...
        vt = vts[0]
        self.logger.info(
            "[%s] First threat type: %s [%]\n", method, vt["theName"], vt["theType"]
        )

    def test_get_by_name(self):
        method = "test_get_by_name"
        url = (
            "/api/value_types/type/threat_type/environment/all/name/"
            + self.existing_vt_name
            + "?session_id=test"
        )
        rv = self.app.get(url)
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            ...
            data=jsonpickle.encode(self.new_vt_dict),
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_put(self):
        ...
        rv = self.app.put(
            url,
            content_type="application/json",
            data=jsonpickle.encode(self.existing_vt_dict),
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
