...
class MySQLDatabaseProxy:
    def __init__(self, host=None, port=None, user=None, passwd=None, db=None):
        ...
        try:
            ...
            exceptionText = (
                "MySQL error connecting to the CAIRIS database "
                + db
                + " on host "
                + host
                + " at port "
    ...
    def reconnect(self, closeConn=True, session_id=None):
        ...
        try:
            ...
            exceptionText = (
                "MySQL error re-connecting to the CAIRIS database: " + format(e)
            )
            raise DatabaseProxyException(exceptionText)
    ...
    def buildDimensionLookup(self):
        dimRows = self.responseList(
            "call traceDimensions()",
            {},
            "MySQL error building trace dimension lookup tables",
        )
        idLookup = {}
        nameLookup = {}
    ...
    def getRequirements(self, constraintId="", isAsset=1):
        reqRows = self.responseList(
            "call getRequirements(:id,:isAs)",
            {"id": constraintId, "isAs": isAsset},
            "MySQL error getting requirements",
        )
        reqDict = {}
        for (
            reqLabel,
    ...
    def getRequirement(self, reqId):
        reqRows = self.responseList(
            "call getRequirement(:id)", {"id": reqId}, "MySQL error getting requirement"
        )
        reqDict = {}
        for (
            reqLabel,
            ...
            return RequirementFactory.build(
                reqId,
                reqLabel,
                reqName,
                reqDesc,
    ...
    def newId(self):
        return self.responseList(
            "call newId()", {}, "MySQL error getting new identifier"
        )[0]
    ...
    def addRequirement(self, req, assetName, isAsset=True):
        req.validate()
        self.updateDatabase(
            "call addRequirement(:lbl,:id,:vers,:name,:desc,:rationale,:origin,:fCrit,:priority,:type,:asName,:isAs)",
            {
                "lbl": req.label(),
                "id": req.id(),
                "vers": req.version(),
                ...
                "origin": req.originator(),
                "fCrit": req.fitCriterion(),
                "priority": req.priority(),
                "type": req.type(),
                "asName": assetName,
                "isAs": isAsset,
            },
            "MySQL error adding new requirement " + str(req.id()),
        )

    def updateRequirement(self, req):
        req.validate()
        self.updateDatabase(
            "call updateRequirement(:lbl,:id,:vers,:name,:desc,:rationale,:origin,:fCrit,:priority,:type)",
            {
                "lbl": req.label(),
                "id": req.id(),
                "vers": req.version(),
            ...
            "MySQL error updating requirement " + str(req.id()),
        )
    ...
    def addValueTension(self, envId, spId, prId, tId, tRationale):
        self.updateDatabase(
            "call addValueTension(:env,:sp,:pr,:tId,:rationale)",
            {"env": envId, "sp": spId, "pr": prId, "tId": tId, "rationale": tRationale},
            "MySQL error adding value tension for environment id " + str(envId),
        )
    ...
    def addEnvironment(self, parameters):
        ...
        session = self.updateDatabase(
            "call addEnvironment(:id,:name,:sc,:desc)",
            {
                "id": environmentId,
                "name": environmentName,
                "sc": environmentShortCode,
                "desc": environmentDescription,
            },
            "MySQL error adding environment",
            None,
            False,
        )
        if len(parameters.environments()) > 0:
            for c in parameters.environments():
                self.updateDatabase(
                    "call addCompositeEnvironment(:id,:c)",
                    {"id": environmentId, "c": c},
                    "MySQL error adding composite environment",
                    session,
                    False,
                )
            self.addCompositeEnvironmentProperties(
                environmentId,
                parameters.duplicateProperty(),
                parameters.overridingEnvironment(),
                session,
            )
    ...
    def addCompositeEnvironmentProperties(
        self, environmentId, duplicateProperty, overridingEnvironment, session=None
    ):
        self.updateDatabase(
            "call addCompositeEnvironmentProperties(:id,:dp,:oe)",
            {"id": environmentId, "dp": duplicateProperty, "oe": overridingEnvironment},
            "MySQL error adding duplicate properties for environment id "
            + str(environmentId),
            session,
            False,
        )
    ...
    def riskEnvironments(self, threatName, vulName):
        return self.responseList(
            "call riskEnvironments(:thr,:vul)",
            {"thr": threatName, "vul": vulName},
            "MySQL error getting environments associated with threat "
            + threatName
            + " and vulnerability "
            + vulName,
        )
    ...
    def riskEnvironmentsByRisk(self, riskName):
        return self.responseList(
            "call riskEnvironmentsByRisk(:risk)",
            {"risk": riskName},
            "MySQL error getting environments associated with risk " + riskName,
        )
    ...
    def updateEnvironment(self, parameters):
        ...
        session = self.updateDatabase(
            "call deleteEnvironmentComponents(:id)",
            {"id": parameters.id()},
            "MySQL error deleting environment components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateEnvironment(:id,:name,:shortCode,:desc)",
            {
                "id": environmentId,
                "name": environmentName,
                "shortCode": environmentShortCode,
                "desc": environmentDescription,
            },
            "MySQL error updating environment",
            session,
            False,
        )
        if len(parameters.environments()) > 0:
    ...
    def responseList(self, callTxt, argDict, errorTxt, session=None):
        try:
            ...
            exceptionText = (
                "MySQL error calling " + callTxt + " (message:" + format(e) + ")"
            )
            raise DatabaseProxyException(exceptionText)
    ...
    def getEnvironments(self, constraintId=-1):
        envRows = self.responseList(
            "call getEnvironments(:id)",
            {"id": constraintId},
            "MySQL error getting environments",
        )
        environments = {}
        for (
            environmentId,
            ...
            if len(cc) > 0:
                duplicateProperty, overridingEnvironment = self.duplicateProperties(
                    environmentId
                )
    ...
    def compositeEnvironments(self, environmentId):
        return self.responseList(
            "call compositeEnvironments(:id)",
            {"id": environmentId},
            "MySQL error getting environments associated with composite environment id "
            + str(environmentId),
        )

    def duplicateProperties(self, environmentId):
        return self.responseList(
            "call duplicateProperties(:id)",
            {"id": environmentId},
            "MySQL error getting duplicate properties associated with composite environment id "
            + str(environmentId),
        )[0]

    def getAttackers(self, constraintId=-1):
        return self.populateAttackers(
            self.responseList(
                "call getAttackers(:id)",
                {"id": constraintId},
                "MySQL error getting attackers" + str(constraintId),
            )
        )
    ...
    def getPersonalAttackers(self):
        return self.populateAttackers(
            self.responseList(
                "call getPersonalAttackers()",
                {},
                "MySQL error getting personal attackers",
            )
        )
    ...
    def populateAttackers(self, attackerRows):
        ...
        for attackerId, attackerName, attackerDesc, attackerImage in attackerRows:
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                attackerId, "attacker"
            ):
                ...
                properties = AttackerEnvironmentProperties(
                    environmentName, roles, motives, capabilities
                )
                environmentProperties.append(properties)
            p = AttackerParameters(
                attackerName, attackerDesc, attackerImage, tags, environmentProperties
            )
    ...
    def dimensionEnvironments(self, dimId, dimTable):
        return self.responseList(
            "call " + dimTable + "_environments(:id)",
            {"id": dimId},
            "MySQL error getting environments for " + dimTable + " id " + str(dimId),
        )

    def attackerMotives(self, attackerId, environmentId):
        return self.responseList(
            "call attacker_motivation(:aId,:eId)",
            {"aId": attackerId, "eId": environmentId},
            "MySQL error getting motives for attacker id "
            + str(attackerId)
            + " in environment id "
            + str(environmentId),
        )
    ...
    def threatLikelihood(self, threatId, environmentId):
        return self.responseList(
            "select threat_likelihood(:tId,:eId)",
            {"tId": threatId, "eId": environmentId},
            "MySQL error getting likelihood for threat id "
            + str(threatId)
            + " in environment id "
            + str(environmentId),
        )[0]
    ...
    def vulnerabilitySeverity(self, vulId, environmentId):
        return self.responseList(
            "select vulnerability_severity(:vId,:eId)",
            {"vId": vulId, "eId": environmentId},
            "MySQL error getting severity for vulnerability id "
            + str(vulId)
            + " in environment id "
            + str(environmentId),
        )[0]
    ...
    def attackerCapabilities(self, attackerId, environmentId):
        return self.responseList(
            "call attacker_capability(:aId,:eId)",
            {"aId": attackerId, "eId": environmentId},
            "MySQL error getting capabilities for attacker id "
            + str(attackerId)
            + " in environment id "
            + str(environmentId),
        )
    ...
    def addAttacker(self, parameters):
        ...
        self.updateDatabase(
            "call addAttacker(:id,:name,:desc,:image)",
            {
                "id": attackerId,
                "name": attackerName,
                "desc": attackerDesc,
            ...
            "MySQL error adding attacker " + str(attackerId),
        )
        ...
        for environmentProperties in parameters.environmentProperties():
            ...
            self.addAttackerMotives(
                attackerId, environmentName, environmentProperties.motives()
            )
            self.addAttackerCapabilities(
                attackerId, environmentName, environmentProperties.capabilities()
            )
            self.addDimensionRoles(
                attackerId, "attacker", environmentName, environmentProperties.roles()
            )
        return attackerId
    ...
    def addDimensionEnvironment(self, dimId, table, environmentName, session=None):
        if session != None:
            self.updateDatabase(
                "call add_" + table + "_environment(:id,:name)",
                {"id": dimId, "name": environmentName},
                "MySQL error associating "
                + table
                + " id "
                + str(dimId)
                + " with environment "
    ...
    def addAttackerMotives(self, attackerId, environmentName, motives):
        for motive in motives:
            self.updateDatabase(
                "call addAttackerMotive(:aId,:envName,:motive)",
                {"aId": attackerId, "envName": environmentName, "motive": motive},
                "MySQL error updating motivates for attacker id " + str(attackerId),
            )

    def addAttackerCapabilities(self, attackerId, environmentName, capabilities):
        for name, value in capabilities:
            self.updateDatabase(
                "call addAttackerCapability(:aId,:envName,:name,:value)",
                {
                    "aId": attackerId,
                ...
                "MySQL error updating attacker capabilities for attacker id "
                + str(attackerId),
            )
    ...
    def updateAttacker(self, parameters):
        parameters.validate()
        session = self.updateDatabase(
            "call deleteAttackerComponents(:id)",
            {"id": parameters.id()},
            "MySQL error deleting attacker components",
            None,
            False,
        )
        ...
        tags = parameters.tags()
        self.updateDatabase(
            "call updateAttacker(:id,:name,:desc,:image)",
            {
                "id": attackerId,
                "name": attackerName,
                "desc": attackerDesc,
    ...
    def deleteObject(self, objtId, tableName):
        ...
        self.updateDatabase(
            sqlTxt, {"obj": objtId}, "MySQL error deleting " + tableName + "s"
        )
    ...
    def addAsset(self, parameters):
        ...
        self.updateDatabase(
            "call addAsset(:id,:name,:sc,:desc,:sig,:type,:c,:cr)",
            {
                "id": assetId,
                "name": assetName,
                "sc": shortCode,
                ...
                "type": assetType,
                "c": assetCriticality,
                "cr": assetCriticalRationale,
            },
            "MySQL error adding asset",
        )
        ...
        for cProperties in parameters.environmentProperties():
            ...
            self.addAssetAssociations(
                assetId, assetName, environmentName, cProperties.associations()
            )
            self.addSecurityProperties(
                "asset",
                assetId,
                environmentName,
                cProperties.properties(),
                cProperties.rationale(),
            )
        return assetId
    ...
    def updateAsset(self, parameters):
        ...
        ifs = parameters.interfaces()
        session = self.updateDatabase(
            "call deleteAssetComponents(:id)",
            {"id": assetId},
            "MySQL error deleting asset components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateAsset(:id,:name,:shortCode,:desc,:sig,:type,:crit,:rationale)",
            {
                "id": assetId,
                "name": assetName,
                "shortCode": shortCode,
                ...
                "rationale": assetCriticalRationale,
            },
            "MySQL error updating asset",
            session,
        )
        self.addTags(assetName, "asset", tags)
        self.addInterfaces(assetName, "asset", ifs)
        ...
        self.updateDatabase(
            "call deleteWidowedConcerns(:name)",
            {"name": assetName},
            "MySQL error delete widowed concerns",
        )

    def addTemplateAssetProperties(
        self,
        taId,
        cProp,
        iProp,
        ...
        self.updateDatabase(
            ...
            "MySQL error adding security properties to template asset",
        )

    def updateTemplateAssetProperties(
        self,
        taId,
        cProp,
        iProp,
        ...
        session = self.updateDatabase(
            callTxt,
            argDict,
            "MySQL error updating template asset property",
            None,
            False,
        )
    ...
    def addSecurityProperties(
        self, dimTable, objtId, environmentName, securityProperties, pRationale
    ):
        sqlTxt = (
            "call add_"
            + dimTable
            + "_properties(:obj,:env,:cPr,:iPr,:avPr,:acPr,:anPr,:panPr,:unlPr,:unoPr,:cRa,:iRa,:avRa,:acRa,:anRa,:panRa,:unlRa,:unoRa)"
        )
        self.updateDatabase(
            sqlTxt,
            {
                "obj": objtId,
                "env": environmentName,
                "cPr": securityProperties[C_PROPERTY],
                "iPr": securityProperties[I_PROPERTY],
                "avPr": securityProperties[AV_PROPERTY],
                "acPr": securityProperties[AC_PROPERTY],
                "anPr": securityProperties[AN_PROPERTY],
                "panPr": securityProperties[PAN_PROPERTY],
                "unlPr": securityProperties[UNL_PROPERTY],
                "unoPr": securityProperties[UNO_PROPERTY],
                "cRa": pRationale[C_PROPERTY],
                "iRa": pRationale[I_PROPERTY],
                "avRa": pRationale[AV_PROPERTY],
                "acRa": pRationale[AC_PROPERTY],
                "anRa": pRationale[AN_PROPERTY],
                "panRa": pRationale[PAN_PROPERTY],
                "unlRa": pRationale[UNL_PROPERTY],
                "unoRa": pRationale[UNO_PROPERTY],
            },
    ...
    def dimensionObject(self, constraintName, dimensionTable):
        ...
        objts = {}
        if (
            dimensionTable == "provided_interface"
            or dimensionTable == "required_interface"
        ):
        ...
        if dimensionTable == "obstacle":
        ...
        elif dimensionTable == "userstory":
            objts = self.getUserStories(constraintId)
        elif dimensionTable == "misusecase":
            objts = self.getMisuseCases(constraintId)
        elif dimensionTable == "requirement":
            return self.getRequirement(constraintName)
            ...
            objts = self.getPolicyStatements(constraintId)
        return (list(objts.values()))[0]

    def getAssets(self, constraintId=-1):
        return self.populateAssets(
            self.responseList(
                "call getAssets(:id)",
                {"id": constraintId},
                "MySQL error getting assets",
            )
        )
    ...
    def getPersonalInformation(self):
        return self.populateAssets(
            self.responseList(
                "call getPersonalInformation()",
                {},
                "MySQL error getting personal information",
            )
        )
    ...
    def populateAssets(self, assetRows):
        ...
        for (
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                syProperties, pRationale = self.relatedProperties(
                    "asset", assetId, environmentId
                )
                assetAssociations = self.assetAssociations(assetId, environmentId)
                properties = AssetEnvironmentProperties(
                    environmentName, syProperties, pRationale, assetAssociations
                )
                environmentProperties.append(properties)
    ...
    def getAssetsSummary(self):
        objtRows = self.responseList(
            "call getAssetsSummary()", {}, "MySQL error getting asset summary"
        )
        rows = []
    ...
    def getGoalsSummary(self):
        objtRows = self.responseList(
            "call getGoalsSummary()", {}, "MySQL error getting goal summary"
        )
        rows = []
    ...
    def getObstaclesSummary(self):
        objtRows = self.responseList(
            "call getObstaclesSummary()", {}, "MySQL error getting obstacle summary"
        )
        rows = []
    ...
    def getUseCasesSummary(self):
        objtRows = self.responseList(
            "call getUseCasesSummary()", {}, "MySQL error getting use case summary"
        )
        rows = []
    ...
    def getAttackersSummary(self):
        objtRows = self.responseList(
            "call getAttackersSummary()", {}, "MySQL error getting attacker summary"
        )
        rows = []
        for objtName, objtDesc in objtRows:
            rows.append(ObjectSummary(objtName, description=objtDesc))
    ...
    def getPersonasSummary(self):
        objtRows = self.responseList(
            "call getPersonasSummary()", {}, "MySQL error getting persona summary"
        )
        rows = []
        for objtName, objtType in objtRows:
            rows.append(ObjectSummary(objtName, type=objtType))
    ...
    def getThreatsSummary(self):
        objtRows = self.responseList(
            "call getThreatsSummary()", {}, "MySQL error getting threat summary"
        )
        rows = []
        for objtName, objtType in objtRows:
            rows.append(ObjectSummary(objtName, type=objtType))
    ...
    def getPersonaCharacteristicsSummary(self):
        objtRows = self.responseList(
            "call getPersonaCharacteristicsSummary()",
            {},
            "MySQL error getting persona characteristics summary",
        )
        rows = []
        for objtName, objtVar, objtChar in objtRows:
            rows.append(
                ObjectSummary(objtChar, variable=objtVar, characteristic=objtName)
            )
        return rows

    def getThreats(self, constraintId=-1):
        return self.populateThreats(
            self.responseList(
                "call getThreats(:id)",
                {"id": constraintId},
                "MySQL error getting threats",
            )
        )
    ...
    def getPersonalThreats(self):
        return self.populateThreats(
            self.responseList(
                "call getPersonalThreats()", {}, "MySQL error getting personal threats"
            )
        )
    ...
    def populateThreats(self, threatRows):
        ...
        for threatId, threatName, threatType, thrMethod in threatRows:
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                syProperties, pRationale = self.relatedProperties(
                    "threat", threatId, environmentId
                )
                properties = ThreatEnvironmentProperties(
                    environmentName,
                    likelihood,
                    assets,
                    attackers,
            ...
            parameters = ThreatParameters(
                threatName, threatType, thrMethod, tags, environmentProperties
            )
    ...
    def getDimensionId(self, dimensionName, dimensionTable):
        ...
        try:
            session = self.conn()
            sqlText = ""
            if (dimensionTable == "classassociation") or (
                dimensionTable == "goalassociation"
            ):
                associationComponents = dimensionName.split("/")
                if dimensionTable == "goalassociation":
                    rs = session.execute(
                        text("select goalAssociationId(:ac0,:ac1,:ac2,:ac3,ac:4)"),
                        {
                            "ac0": associationComponents[0],
                            "ac1": associationComponents[1],
                            "ac2": associationComponents[2],
                            "ac3": associationComponents[3],
                            "ac4": associationComponents[4],
                        },
                    )
                elif dimensionTable == "classassociation":
                    rs = session.execute(
                        text("select classAssociationId(:ac0,:ac1,:ac2)"),
                        {
                            "ac0": associationComponents[0],
                            "ac1": associationComponents[1],
                            "ac2": associationComponents[2],
            ...
            elif (dimensionTable == "provided_interface") or (
                dimensionTable == "required_interface"
            ):
                cName, ifName = dimensionName.split("_")
                rs = session.execute(
                    text("select interfaceId(:name)"), {"name": ifName}
                )
            elif dimensionTable == "policy_statement":
                psComponents = dimensionName.split("/")
                rs = session.execute(
                    text("select policyStatementId(:goal,:env,:subj,:at,:res)"),
                    {
                        "goal": psComponents[0],
                        "env": psComponents[1],
                        "subj": psComponents[2],
            ...
            if dimId == None and dimensionTable == "requirement":
                rs = session.execute(
                    "select requirementNameId(:name)", {"name": dimensionName}
                )
                row = rs.fetchone()
                dimId = row[0]
            rs.close()
            ...
            exceptionText = "MySQL error getting "
            exceptionText += (
                dimensionTable + " (id:" + str(id) + ",message:" + msg + ")"
            )
            raise DatabaseProxyException(exceptionText)

    def getDimensions(self, dimensionTable, idConstraint=-1):
        dimRows = self.responseList(
            "call getDimensions(:dimensionTable, :idConstraint)",
            {"dimensionTable": dimensionTable, "idConstraint": idConstraint},
            "MySQL error getting " + dimensionTable,
        )
        dimensions = {}
    ...
    def getDimensionNames(self, dimensionTable, currentEnvironment=""):
        if (
            dimensionTable != "template_asset"
            and dimensionTable != "template_requirement"
            and dimensionTable != "template_goal"
            and dimensionTable != "locations"
            and dimensionTable != "persona_characteristic_synopsis"
            and dimensionTable != "document_reference_synopsis"
        ):
            callTxt = "call " + dimensionTable + "Names(:env)"
            argDict = {"env": currentEnvironment}
            return self.responseList(
                callTxt, argDict, "MySQL error getting " + dimensionTable + "s"
            )
        elif dimensionTable == "template_asset":
            return self.responseList(
                "call template_assetNames()",
                {},
                "MySQL error getting " + dimensionTable + "s",
            )
        elif dimensionTable == "template_requirement":
            return self.responseList(
                "call template_requirementNames()",
                {},
                "MySQL error getting " + dimensionTable + "s",
            )
        elif dimensionTable == "template_goal":
            return self.responseList(
                "call template_goalNames()",
                {},
                "MySQL error getting " + dimensionTable + "s",
            )
            ...
            return self.responseList(
                "call persona_characteristic_synopsisNames()",
                {},
                "MySQL error getting " + dimensionTable + "s",
            )
        elif dimensionTable == "document_reference_synopsis":
            return self.responseList(
                "call document_reference_synopsisNames()",
                {},
                "MySQL error getting " + dimensionTable + "s",
            )
    ...
    def getEnvironmentNames(self):
        return self.responseList(
            "call nonCompositeEnvironmentNames()",
            {},
            "MySQL error getting environments",
        )
    ...
    def addThreat(self, parameters, update=False):
        ...
        tags = parameters.tags()
        session = self.updateDatabase(
            "call addThreat(:id,:name,:type,:method)",
            {
                "id": threatId,
                "name": threatName,
                "type": threatType,
        ...
        for cProperties in parameters.environmentProperties():
            ...
            self.updateDatabase(
                "call addThreatLikelihood(:tId,:env,:likely)",
                {
                    "tId": threatId,
                    "env": environmentName,
                    "likely": cProperties.likelihood(),
                },
                "MySQL error adding threat likelihood",
                session,
                False,
            )
            self.addSecurityProperties(
            ...
            for assetName in cProperties.assets():
                self.updateDatabase(
                    "call addAssetThreat(:tId,:env,:assName)",
                    {"tId": threatId, "env": environmentName, "assName": assetName},
                    "MySQL error adding asset threat",
                    session,
                    False,
                )
            for attacker in cProperties.attackers():
                self.updateDatabase(
                    "call addThreatAttacker(:tId,:env,:attacker)",
                    {"tId": threatId, "env": environmentName, "attacker": attacker},
                    "MySQL error adding threat attacker",
                    session,
                    False,
                )
        self.commitDatabase(session)
    ...
    def updateThreat(self, parameters):
        ...
        session = self.updateDatabase(
            "call deleteThreatComponents(:id)",
            {"id": threatId},
            "MySQL error deleting threat components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateThreat(:id,:name,:type,:method)",
            {
                "id": threatId,
                "name": threatName,
                "type": threatType,
    ...
    def getVulnerabilities(self, constraintId=-1):
        return self.populateVulnerabilities(
            self.responseList(
                "call getVulnerabilities(:id)",
                {"id": constraintId},
                "MySQL error getting vulnerabilities",
            )
        )
    ...
    def getPersonalVulnerabilities(self):
        return self.populateVulnerabilities(
            self.responseList(
                "call getPersonalVulnerabilities()",
                {},
                "MySQL error getting personal vulnerabilities",
            )
        )
    ...
    def populateVulnerabilities(self, vulRows):
        ...
        for (
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                properties = VulnerabilityEnvironmentProperties(
                    environmentName, severity, assets
                )
                environmentProperties.append(properties)
            parameters = VulnerabilityParameters(
                vulnerabilityName,
                vulnerabilityDescription,
                vulnerabilityType,
                tags,
    ...
    def addVulnerability(self, parameters):
        ...
        vulId = self.newId()
        session = self.updateDatabase(
            "call addVulnerability(:id,:name,:desc,:type)",
            {"id": vulId, "name": vulName, "desc": vulDesc, "type": vulType},
            "MySQL error adding vulnerabilitity",
            None,
            False,
        )
        ...
        for cProperties in parameters.environmentProperties():
            ...
            self.updateDatabase(
                "call addVulnerabilitySeverity(:vId,:env,:severity)",
                {
                    "vId": vulId,
                    "env": environmentName,
                    "severity": cProperties.severity(),
                },
                "MySQL error adding vulnerability severity",
                session,
                False,
            )
            for assetName in cProperties.assets():
                self.updateDatabase(
                    "call addAssetVulnerability(:vId,:env,:assName)",
                    {"vId": vulId, "env": environmentName, "assName": assetName},
                    "MySQL error adding asset vulnerability",
                    session,
                    False,
                )
        self.commitDatabase(session)
    ...
    def updateVulnerability(self, parameters):
        ...
        session = self.updateDatabase(
            "call deleteVulnerabilityComponents(:id)",
            {"id": vulId},
            "MySQL error deleting vulnerability components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateVulnerability(:id,:name,:desc,:type)",
            {"id": vulId, "name": vulName, "desc": vulDesc, "type": vulType},
            "MySQL error updating vulnerability",
            session,
            False,
        )
        self.addTags(vulName, "vulnerability", tags)
    ...
    def relatedProperties(self, dimTable, objtId, environmentId):
        ...
        row = self.responseList(
            callTxt, argDict, "MySQL error getting related properties"
        )[0]
        properties = []
        properties = array(
    ...
    def templateAssetProperties(self, taId):
        row = self.responseList(
            "call template_assetProperties(:id)",
            {"id": taId},
            "MySQL error getting template asset properties",
        )[0]
        properties = []
        rationale = []
    ...
    def getPersonas(self, constraintId=-1):
        personaRows = self.responseList(
            "call getPersonas(:id)",
            {"id": constraintId},
            "MySQL error getting personas",
        )
        personas = {}
        for (
            personaId,
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                properties = PersonaEnvironmentProperties(
                    environmentName, directFlag, personaDesc, roles, envCodes
                )
                environmentProperties.append(properties)
    ...
    def dimensionRoles(self, dimId, environmentId, table):
        return self.responseList(
            "call " + table + "_roles(:dimId,:envId)",
            {"dimId": dimId, "envId": environmentId},
            "MySQL error getting roles for "
            + table
            + " associated with environment id "
            + str(environmentId),
        )

    def threatAttackers(self, threatId, environmentId):
        return self.responseList(
            "call threat_attacker(:tId,:eId)",
            {"tId": threatId, "eId": environmentId},
            "MySQL error getting attackers for threat id "
            + str(threatId)
            + " in environment "
            + str(environmentId),
        )
    ...
    def addPersona(self, parameters):
        ...
        tags = parameters.tags()
        self.updateDatabase(
            "call addPersona(:id,:name,:act,:att,:apt,:mot,:skills,:intr,:cont,:img,:ass,:type)",
            {
                "id": personaId,
                "name": personaName,
                "act": activities,
        ...
        for environmentProperties in parameters.environmentProperties():
            ...
            self.addPersonaNarrative(
                personaId, environmentName, environmentProperties.narrative()
            )
            self.addPersonaDirect(
                personaId, environmentName, environmentProperties.directFlag()
            )
            self.addDimensionRoles(
                personaId, "persona", environmentName, environmentProperties.roles()
            )
            self.addPersonaEnvironmentCodes(
                personaName, environmentName, environmentProperties.codes()
            )
        return personaId
    ...
    def addDimensionRoles(self, personaId, table, environmentName, roles):
        ...
        for role in roles:
            self.updateDatabase(
                "call add_" + table + "_role (:id,:envName,:role)",
                {"id": personaId, "envName": environmentName, "role": role},
                "MySQL error adding dimension role",
                session,
                False,
            )
        self.commitDatabase(session)
    ...
    def updatePersona(self, parameters):
        ...
        session = self.updateDatabase(
            "call deletePersonaComponents(:id)",
            {"id": personaId},
            "MySQL error deleting persona components",
            None,
            False,
        )
        self.updateDatabase(
            "call updatePersona(:id,:name,:act,:att,:apt,:mot,:skills,:intr,:cont,:img,:ass,:type)",
            {
                "id": personaId,
                "name": personaName,
                "act": activities,
    ...
    def getTasks(self, constraintId=-1):
        taskRows = self.responseList(
            "call getTasks(:id)", {"id": constraintId}, "MySQL error getting tasks"
        )
        tasks = {}
        for (
            taskId,
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                concernAssociations = self.taskConcernAssociations(
                ...
                properties = TaskEnvironmentProperties(
                    environmentName,
                    dependencies,
                    personas,
                    assets,
    ...
    def getMisuseCases(self, constraintId=-1):
        mcRows = self.responseList(
            "call getMisuseCases(:id)",
            {"id": constraintId},
            "MySQL error getting misuse cases",
        )
        mcs = {}
        for mcId, mcName in mcRows:
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                mcEnv = self.misuseCaseEnvironment(
                    environmentName, mcId, riskId, environmentId
                )
    ...
    def misuseCaseEnvironment(
        self, envName, mcId, riskId, envId, thrName="", vulName=""
    ):
        ...
        if thrName != "" and vulName != "":
            mcEnv.theRiskRating = RiskRating(
                ...
                vulName,
                envName,
                self.riskRating(riskId, thrName, vulName, envName),
            )
    ...
    def riskMisuseCase(self, riskId, thrName="", vulName=""):
        rows = []
        if thrName != "" and vulName != "":
            rows = self.responseList(
                "call riskMisuseCaseByTV(:threat,:vulnerability)",
                {"threat": thrName, "vulnerability": vulName},
                "MySQL error getting risk misuse case",
            )
        else:
            rows = self.responseList(
                "call riskMisuseCase(:id)",
                {"id": riskId},
                "MySQL error getting risk misuse case",
            )
        if len(rows) == 0:
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                mcEnv = self.misuseCaseEnvironment(
                    environmentName, mcId, riskId, environmentId, thrName, vulName
                )
                environmentProperties.append(mcEnv)
            parameters = MisuseCaseParameters(mcName, environmentProperties, risk)
            mc = ObjectFactory.build(mcId, parameters)
    ...
    def misuseCaseRisk(self, mcId):
        return self.responseList(
            "select misuseCaseRisk(:id)",
            {"id": mcId},
            "MySQL error getting risk for misuse case id " + str(mcId),
        )[0]

    def taskPersonas(self, taskId, environmentId):
        return self.responseList(
            "call taskPersonas(:tId,:eId)",
            {"tId": taskId, "eId": environmentId},
            "MySQL error getting task personas for environment id "
            + str(environmentId),
        )

    def taskAssets(self, taskId, environmentId):
        return self.responseList(
            "call taskAssets(:tId,:eId)",
            {"tId": taskId, "eId": environmentId},
            "MySQL error getting task assets for environment id " + str(environmentId),
        )
    ...
    def addTask(self, parameters):
        ...
        taskId = self.newId()
        self.updateDatabase(
            "call addTask(:id,:name,:shortCode,:obj,:ass,:auth)",
            {
                "id": taskId,
                "name": taskName,
            ...
            "MySQL error adding task",
        )
        self.addTags(taskName, "task", tags)
        for cProperties in parameters.environmentProperties():
            environmentName = cProperties.name()
            self.addDimensionEnvironment(taskId, "task", environmentName)
            self.addTaskDependencies(
                taskId, cProperties.dependencies(), environmentName
            )
            ...
            self.addTaskConcernAssociations(
                taskId, environmentName, cProperties.concernAssociations()
            )
            self.addTaskNarrative(
                taskId,
                cProperties.narrative(),
                cProperties.consequences(),
                cProperties.benefits(),
                environmentName,
            )
    ...
    def addMisuseCase(self, parameters, session):
        ...
        mcId = self.newId()
        self.updateDatabase(
            "call addMisuseCase(:id,:name)",
            {"id": mcId, "name": mcName},
            "MySQL error adding misuse case " + mcName,
            session,
        )
        ...
        for cProperties in parameters.environmentProperties():
            ...
            self.addMisuseCaseNarrative(
                mcId, cProperties.narrative(), environmentName, session
            )
        return mcId
    ...
    def updateTask(self, parameters):
        ...
        tags = parameters.tags()
        session = self.updateDatabase(
            "call deleteTaskComponents(:id)",
            {"id": taskId},
            "MySQL error deleting task components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateTask(:id,:name,:shortCode,:obj,:ass,:auth)",
            {
                "id": taskId,
                "name": taskName,
                "shortCode": taskShortCode,
    ...
    def updateMisuseCase(self, parameters, session):
        ...
        mcName = parameters.name()
        self.updateDatabase(
            "call deleteMisuseCaseComponents(:id)",
            {"id": mcId},
            "MySQL error dleting misuse case components",
            session,
            False,
        )
        self.updateDatabase(
            "call updateMisuseCase(:id,:name)",
            {"id": mcId, "name": mcName},
            "MySQL error updating misuse case",
            session,
            False,
        )
        self.addMisuseCaseRisk(mcId, parameters.risk(), session)
    ...
    def addTaskPersonas(self, taskId, personas, environmentName):
        ...
        for persona, duration, frequency, demands, goalsupport in personas:
            self.updateDatabase(
                "call addTaskPersona(:tId,:pers,:dur,:freq,:dem,:goal,:env)",
                {
                    "tId": taskId,
                    "pers": persona,
                    "dur": duration,
                ...
                "MySQL error adding task persona",
                session,
                False,
            )
        self.commitDatabase(session)
    ...
    def addTaskAssets(self, taskId, assets, environmentName):
        ...
        for asset in assets:
            self.updateDatabase(
                "call addTaskAsset(:tId,:ass,:env)",
                {"tId": taskId, "ass": asset, "env": environmentName},
                "MySQL error adding task asset",
                session,
                False,
            )
        self.commitDatabase(session)
    ...
    def addMisuseCaseRisk(self, mcId, riskName, session):
        self.updateDatabase(
            "call addMisuseCaseRisk(:id,:risk)",
            {"id": mcId, "risk": riskName},
            "MySQL error associating risk with misuse case",
            session,
            False,
        )
    ...
    def getTraceDimensions(self, dimName, isFrom):
        return self.traceDimensionList(
            self.getDimensionId(dimName, "trace_dimension"), isFrom
        )

    def traceDimensionList(self, dimId, isFrom):
        return self.responseList(
            "call traceDimensionList(:id,:from)",
            {"id": dimId, "from": isFrom},
            "MySQL error getting trace dimensions",
        )

    def getRisks(self, constraintId=-1):
        return self.populateRisks(
            self.responseList(
                "call getRisks(:id)", {"id": constraintId}, "MySQL error getting risks"
            )
        )
    ...
    def getPersonalRisks(self):
        return self.populateRisks(
            self.responseList(
                "call getPersonalRisks()", {}, "MySQL error getting personal risks"
            )
        )
    ...
    def populateRisks(self, parameterList):
        ...
        for parameters in parameterList:
            ...
            parameters = RiskParameters(
                parameters[1], parameters[2], parameters[3], mc, tags
            )
    ...
    def getRisksSummary(self):
        objtRows = self.responseList(
            "call getRisksSummary()", {}, "MySQL error getting risk summary"
        )
        rows = []
    ...
    def addRisk(self, parameters):
        ...
        session = self.updateDatabase(
            "call addRisk(:threat,:vuln,:rId,:risk,:txt)",
            {
                "threat": threatName,
                "vuln": vulName,
                "rId": riskId,
            ...
            "MySQL error adding risk",
            None,
            False,
        )
        mc = parameters.misuseCase()
        mcParameters = MisuseCaseParameters(
            mc.name(), mc.environmentProperties(), mc.risk()
        )
    ...
    def updateRisk(self, parameters):
        ...
        inTxt = parameters.intent()
        session = self.updateDatabase(
            "call updateRisk(:threat,:vuln,:rId,:risk,:txt)",
            {
                "threat": threatName,
                "vuln": vulName,
                "rId": riskId,
        ...
        mcId, oldMcName = self.responseList(
            "call riskMisuseCase(:id)",
            {"id": riskId},
            "MySQL error getting risk misuse case",
        )[0]
        mcParameters = MisuseCaseParameters(
            mcName, mc.environmentProperties(), riskName
        )
    ...
    def getResponses(self, constraintId=-1):
        return self.populateResponses(
            self.responseList(
                "call getResponses(:id)",
                {"id": constraintId},
                "MySQL error getting responses",
            )
        )
    ...
    def getPersonalResponses(self):
        return self.populateResponses(
            self.responseList(
                "call getPersonalResponses()",
                {},
                "MySQL error getting personal responses",
            )
        )
        ...
        for respId, respName, respType, respRisk in responseRows:
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                if respType == "Accept":
                    ...
                    properties = AcceptEnvironmentProperties(
                        environmentName, respCost, respDescription
                    )
                    environmentProperties.append(properties)
                elif respType == "Transfer":
                    respDescription = self.responseDescription(respId, environmentId)
                    respRoles = self.responseRoles(respId, environmentId)
                    properties = TransferEnvironmentProperties(
                        environmentName, respDescription, respRoles
                    )
                    environmentProperties.append(properties)
                else:
                    ...
                    properties = MitigateEnvironmentProperties(
                        environmentName, mitType, detPoint, detMechs
                    )
                    environmentProperties.append(properties)

            parameters = ResponseParameters(
                respName, respRisk, tags, environmentProperties, respType
            )
    ...
    def responseCost(self, responseId, environmentId):
        return self.responseList(
            "select responseCost(:rId,:eId)",
            {"rId": responseId, "eId": environmentId},
            "MySQL error getting cost associated with response id "
            + str(responseId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def responseDescription(self, responseId, environmentId):
        return self.responseList(
            "select responseDescription(:rId,:eId)",
            {"rId": responseId, "eId": environmentId},
            "MySQL error getting description associated with response id "
            + str(responseId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def responseRoles(self, responseId, environmentId):
        return self.responseList(
            "call responseRoles(:rId,:eId)",
            {"rId": responseId, "eId": environmentId},
            "MySQL error getting roles associated with response id "
            + str(responseId)
            + " in environment "
            + str(environmentId),
        )
    ...
    def mitigationType(self, responseId, environmentId):
        return self.responseList(
            "select mitigationType(:rId,:eId)",
            {"rId": responseId, "eId": environmentId},
            "MySQL error getting mitigation type associated with response id "
            + str(responseId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def riskComponents(self, riskName):
        row = self.responseList(
            "call riskComponents(:rId)",
            {"rId": riskName},
            "MySQL error getting risk components",
        )[0]
        threatName = row[0]
        vulName = row[1]
    ...
    def addResponse(self, parameters):
        ...
        respId = self.newId()
        self.updateDatabase(
            "call addResponse(:id,:name,:type,:risk)",
            {"id": respId, "name": respName, "type": respType, "risk": respRisk},
            "MySQL error adding response",
        )
        ...
        for cProperties in parameters.environmentProperties():
            ...
            if respType == "Accept":
                ...
                self.addResponseDescription(
                    respId, cProperties.description(), environmentName
                )
            elif respType == "Transfer":
                self.addResponseDescription(
                    respId, cProperties.description(), environmentName
                )
                self.addResponseRoles(
                    respId,
                    cProperties.roles(),
                    environmentName,
                    cProperties.description(),
                )
            else:
                mitType = cProperties.type()
                self.addMitigationType(respId, mitType, environmentName)
                if mitType == "Detect":
                    self.addDetectionPoint(
                        respId, cProperties.detectionPoint(), environmentName
                    )
                elif mitType == "React":
                    for detMech in cProperties.detectionMechanisms():
                        self.addReactionDetectionMechanism(
                            respId, detMech, environmentName
                        )
        return respId

    def addMitigationType(self, responseId, mitType, environmentName):
        self.updateDatabase(
            "call add_response_mitigate(:rId,:env,:type)",
            {"rId": responseId, "env": environmentName, "type": mitType},
            "MySQL error adding mitigation type",
        )

    def addResponseCost(self, responseId, costName, environmentName):
        self.updateDatabase(
            "call addResponseCost(:rId,:name,:env)",
            {"rId": responseId, "name": costName, "env": environmentName},
            "MySQL error adding response cost",
        )

    def addResponseDescription(self, responseId, descriptionText, environmentName):
        self.updateDatabase(
            "call addResponseDescription(:id,:desc,:env)",
            {"id": responseId, "desc": descriptionText, "env": environmentName},
            "MySQL error adding response description",
        )
    ...
    def addResponseRoles(self, responseId, roles, environmentName, respDesc):
        ...
        for role, cost in roles:
            self.updateDatabase(
                "call addResponseRole(:id,:role,:cost,:env,:desc)",
                {
                    "id": responseId,
                    "role": role,
                    "cost": cost,
                ...
                "MySQL error adding response role",
                session,
                False,
            )
        self.commitDatabase(session)
    ...
    def updateResponse(self, parameters):
        ...
        session = self.updateDatabase(
            "call deleteResponseComponents(:id)",
            {"id": respId},
            "MySQL error deleting response components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateResponse(:id,:name,:type,:risk)",
            {"id": respId, "name": respName, "type": respType, "risk": respRisk},
            "MySQL error updating response",
            session,
        )
        self.addTags(respName, "response", tags)
        for cProperties in parameters.environmentProperties():
    ...
    def detectionPoint(self, responseId, environmentId):
        return self.responseList(
            "select mitigatePoint(:rId,:eId)",
            {"rId": responseId, "eId": environmentId},
            "MySQL error getting detection point for detection response id "
            + str(responseId),
        )[0]

    def addDetectionPoint(self, mitId, detPoint, environmentName):
        self.updateDatabase(
            "call add_mitigate_point(:id,:env,:point)",
            {"id": mitId, "env": environmentName, "point": detPoint},
            "MySQL error adding detection point",
        )

    def addReactionDetectionMechanism(self, mitId, detMech, environmentName):
        self.updateDatabase(
            "call add_reaction_detection_mechanism(:id,:methc,:env)",
            {"id": mitId, "metch": detMech, "env": environmentName},
            "MySQL error adding reaction detection mechanism",
        )

    def detectionMechanisms(self, responseId, environmentId):
        return self.responseList(
            "call detectionMechanisms(:rId,:eId)",
            {"rId": responseId, "eId": environmentId},
            "MySQL error getting detection mechanisms for detection response id "
            + str(responseId),
        )

    def riskAnalysisModel(self, environmentName, dimensionName="", objectName=""):
        if (dimensionName == "risk" and objectName != "") or (
            objectName != "" and self.isRisk(objectName)
        ):
            return self.riskModel(environmentName, objectName)

        traceRows = self.responseList(
            "call riskAnalysisModel(:env)",
            {"env": environmentName},
            "MySQL error getting risk analysis model",
        )
        traces = []
    ...
    def removableTraces(self, environmentName):
        traceRows = self.responseList(
            "call viewRemovableTraces(:env)",
            {"env": environmentName},
            "MySQL error getting removable trace relations",
        )
        traces = []
    ...
    def allowableTraceDimension(self, fromId, toId):
        return self.responseList(
            "call allowableTraceDimension(:frm,:to)",
            {"frm": fromId, "to": toId},
            "MySQL error getting allowable trace dimensions for from_id "
            + str(fromId)
            + " and to_id "
            + str(toId),
        )[0]
    ...
    def reportDependencies(self, dimName, objtId):
        return self.responseList(
            "call reportDependents(:id,:name)",
            {"id": objtId, "name": dimName},
            "MySQL error getting dependencies for " + dimName + " id " + str(objtId),
        )
    ...
    def threatenedAssets(self, threatId, environmentId):
        return self.responseList(
            "call threat_asset(:tId,:eId)",
            {"tId": threatId, "eId": environmentId},
            "MySQL error getting assets associated with threat id "
            + str(threatId)
            + " in environment id "
            + str(environmentId),
        )
    ...
    def vulnerableAssets(self, vulId, environmentId):
        return self.responseList(
            "call vulnerability_asset(:vId,:eId)",
            {"vId": vulId, "eId": environmentId},
            "MySQL error getting assets associated with vulnerability id "
            + str(vulId)
            + " in environment id "
            + str(environmentId),
        )
    ...
    def addTrace(self, traceTable, fromId, toId, label="supports"):
        if (
            traceTable != "requirement_task"
            and traceTable != "requirement_usecase"
            and traceTable != "requirement_requirement"
        ):
            self.updateDatabase(
                "insert into " + traceTable + " values(:fromId,:toId)",
                {"fromId": fromId, "toId": toId},
                "MySQL error adding trace",
            )
        elif traceTable == "requirement_requirement":
            self.updateDatabase(
                "insert into " + traceTable + ' values(:fromId,:toId,":label")',
                {"fromId": fromId, "toId": toId, "label": label},
                "MySQL error adding trace",
            )
        else:
            refTypeId = self.getDimensionId(label, "reference_type")
            self.updateDatabase(
                "insert into " + traceTable + " values(:fromId,:toId,:refTypeId)",
                {"fromId": fromId, "toId": toId, "refTypeId": refTypeId},
                "MySQL error adding trace",
            )
    ...
    def deleteEnvironment(self, environmentId):
        try:
            ...
            exceptionText = (
                "Cannot remove environment due to dependent data (" + str(e) + ")."
            )
    ...
    def riskRating(self, riskId, thrName, vulName, environmentName):
        return self.responseList(
            "call riskRating(:riskId,:thr,:vuln,:env)",
            {"riskId": riskId, "thr": thrName, "vuln": vulName, "env": environmentName},
            "MySQL error rating risk associated with threat/vulnerability/environment "
            + thrName
            + "/"
            + vulName
            + "/"
    ...
    def riskScore(self, threatName, vulName, environmentName, riskName=""):
        return self.responseList(
            "call riskScore(:threat,:vuln,:env,:risk)",
            {
                "threat": threatName,
                "vuln": vulName,
                "env": environmentName,
            ...
            "MySQL error rating risk associated with risk " + riskName,
        )
    ...
    def reqTargetNames(self, reqLabel, envName):
        rows = self.responseList(
            "call targetNames(:req,:env)",
            {"req": reqLabel, "env": envName},
            "MySQL error getting target names",
        )
        targets = {}
    ...
    def getRoles(self, constraintId=-1):
        roleRows = self.responseList(
            "call getRoles(:id)", {"id": constraintId}, "MySQL error getting roles"
        )
        roles = {}
        for roleId, roleName, roleType, shortCode, roleDescription in roleRows:
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                properties = RoleEnvironmentProperties(
                    environmentName,
                    roleResponses,
                    roleCountermeasures,
                    roleGoals,
            ...
            parameters = RoleParameters(
                roleName, roleType, shortCode, roleDescription, environmentProperties
            )
    ...
    def addRole(self, parameters):
        ...
        self.updateDatabase(
            "call addRole(:id,:name,:type,:shortCode,:desc)",
            {
                "id": roleId,
                "name": roleName,
                "type": roleType,
    ...
    def updateRole(self, parameters):
        ...
        roleDesc = parameters.description()
        self.updateDatabase(
            "call updateRole(:id,:name,:type,:shortCode,:desc)",
            {
                "id": roleId,
                "name": roleName,
                "type": roleType,
    ...
    def roleResponsibilities(self, roleId, environmentId):
        return self.responseList(
            "call roleResponses(:rId,:eId)",
            {"rId": roleId, "eId": environmentId},
            "MySQL error getting responses for role id "
            + str(roleId)
            + " in environment "
            + str(environmentId),
        )
    ...
    def roleCountermeasures(self, roleId, environmentId):
        return self.responseList(
            "call roleCountermeasures(:rId,:eId)",
            {"rId": roleId, "eId": environmentId},
            "MySQL error getting countermeasures for role id "
            + str(roleId)
            + " in environment "
            + str(environmentId),
        )
    ...
    def roleGoals(self, roleId, environmentId):
        return self.responseList(
            "call roleGoalResponsibilities(:rId,:eId)",
            {"rId": roleId, "eId": environmentId},
            "MySQL error getting goals for role id "
            + str(roleId)
            + " in environment "
            + str(environmentId),
        )
    ...
    def roleRequirements(self, roleId, environmentId):
        return self.responseList(
            "call roleRequirementResponsibilities(:rId,:eId)",
            {"rId": roleId, "eId": environmentId},
            "MySQL error getting requirements for role id "
            + str(roleId)
            + " in environment "
            + str(environmentId),
        )
    ...
    def getCountermeasures(self, constraintId=-1):
        cmRows = self.responseList(
            "call getCountermeasures(:id)",
            {"id": constraintId},
            "MySQL error getting countermeasures",
        )
        countermeasures = {}
        for cmId, cmName, cmDesc, cmType in cmRows:
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                properties, pRationale = self.relatedProperties(
                    "countermeasure", cmId, environmentId
                )
                ...
                environmentProperties.append(properties)
            parameters = CountermeasureParameters(
                cmName, cmDesc, cmType, tags, environmentProperties
            )
    ...
    def countermeasureCost(self, cmId, environmentId):
        return self.responseList(
            "select countermeasureCost(:cmId,:envId)",
            {"cmId": cmId, "envId": environmentId},
            "MySQL error getting cost associated with countermeasure id " + str(cmId),
        )[0]

    def countermeasureTargets(self, cmId, environmentId):
        reqs = self.responseList(
            "call countermeasureRequirements(:cmId,:envId)",
            {"cmId": cmId, "envId": environmentId},
            "MySQL error getting requirements with countermeasure id " + str(cmId),
        )
        targets = self.responseList(
            "call countermeasureTargets(:cmId,:envId)",
            {"cmId": cmId, "envId": environmentId},
            "MySQL error getting targets with countermeasure id " + str(cmId),
        )
        return (reqs, targets)

    def countermeasureRoles(self, cmId, environmentId):
        return self.responseList(
            "call countermeasureRoles(:cmId,:envId)",
            {"cmId": cmId, "envId": environmentId},
            "MySQL error getting roles associated with countermeasure id " + str(cmId),
        )

    def countermeasurePersonas(self, cmId, environmentId):
        return self.responseList(
            "call countermeasurePersonas(:cmId,:envId)",
            {"cmId": cmId, "envId": environmentId},
            "MySQL error getting personas associated with countermeasure id "
            + str(cmId),
        )
    ...
    def addCountermeasure(self, parameters):
        ...
        cmId = self.newId()
        self.updateDatabase(
            "call addCountermeasure(:id,:name,:desc,:type)",
            {"id": cmId, "name": cmName, "desc": cmDesc, "type": cmType},
            "MySQL error adding countermeasure",
        )
        ...
        for cProperties in parameters.environmentProperties():
            ...
            self.addCountermeasureTargets(
                cmId, cProperties.requirements(), cProperties.targets(), environmentName
            )
            self.addSecurityProperties(
                "countermeasure",
                cmId,
            ...
            self.addCountermeasurePersonas(
                cmId, cProperties.personas(), environmentName
            )
            self.addRequirementRoles(
                cmName, cProperties.roles(), cProperties.requirements(), environmentName
            )
        return cmId
    ...
    def updateCountermeasure(self, parameters):
        ...
        session = self.updateDatabase(
            "call deleteCountermeasureComponents(:id)",
            {"id": cmId},
            "MySQL error deleting countermeasure components",
        )
        self.updateDatabase(
            "call updateCountermeasure(:id,:name,:desc,:type)",
            {"id": cmId, "name": cmName, "desc": cmDesc, "type": cmType},
            "MySQL error updating countermeasures",
            session,
        )
        self.addTags(cmName, "countermeasure", tags)
        for cProperties in environmentProperties:
            ...
            environmentName = cProperties.name()
            self.addDimensionEnvironment(cmId, "countermeasure", environmentName)
            self.updateCountermeasureTargets(
                cmId, cProperties.requirements(), cProperties.targets(), environmentName
            )
            self.addSecurityProperties(
                "countermeasure",
    ...
    def addCountermeasureTargets(self, cmId, reqs, targets, environmentName):
        ...
        for reqLabel in reqs:
            self.updateDatabase(
                "call addCountermeasureRequirement(:cmId,:lbl,:env)",
                {"cmId": cmId, "lbl": reqLabel, "env": environmentName},
                "MySQL error adding countermeasure requirement",
                session,
                False,
            )
        for target in targets:
            self.updateDatabase(
                "call addCountermeasureTarget(:cmId,:name,:effectiveness,:rationale,:env)",
                {
                    "cmId": cmId,
                    "name": target.name(),
                ...
                "MySQL error adding countermeasure target",
                session,
                False,
            )
        self.commitDatabase(session)
    ...
    def updateCountermeasureTargets(self, cmId, reqs, targets, environmentName):
        ...
        for reqLabel in reqs:
            self.updateDatabase(
                "call updateCountermeasureRequirement(:cmId,:lbl,:env)",
                {"cmId": cmId, "lbl": reqLabel, "env": environmentName},
                "MySQL error updating countermeasure requirement",
                session,
                False,
            )
        for target in targets:
    ...
    def addRequirementRole(self, cmName, roleName, reqName, envName):
        ...
        self.updateDatabase(
            "call addRequirementRole(:aId,:cm,:role,:req,:env)",
            {
                "aId": associationId,
                "cm": cmName,
                "role": roleName,
            ...
            "MySQL error adding requirement role",
        )

    def updateRequirementRole(self, cmName, roleName, reqName, envName):
        associationId = self.newId()
        self.updateDatabase(
            "call updateRequirementRole(:aId,:cm,:role,:req,:env)",
            {
                "aId": associationId,
                "cm": cmName,
                "role": roleName,
            ...
            "MySQL error updating requirement role",
        )

    def deleteRequirementRole(self, roleName, reqName, envName):
        self.updateDatabase(
            "call deleteRequirementRole(:role,:req,:env)",
            {"role": roleName, "req": reqName, "env": envName},
            "MySQL error deleting requirement role",
        )

    def addCountermeasureCost(self, cmId, costName, environmentName):
        self.updateDatabase(
            "call addCountermeasureCost(:cmId,:cost,:env)",
            {"cmId": cmId, "cost": costName, "env": environmentName},
            "MySQL error adding countermeasure cost",
        )
    ...
    def addCountermeasureRoles(self, cmId, roles, environmentName):
        ...
        for role in roles:
            self.updateDatabase(
                "call addCountermeasureRole(:cmId,:role,:env)",
                {"cmId": cmId, "role": role, "env": environmentName},
                "MySQL error adding countermeasure role",
                session,
                False,
            )
        self.commitDatabase(session)
    ...
    def addCountermeasurePersonas(self, cmId, personas, environmentName):
        ...
        for task, persona, duration, frequency, demands, goalSupport in personas:
            self.updateDatabase(
                "call addCountermeasurePersona(:id,:persona,:task,:dur,:freq,:dem,:goal,:env)",
                {
                    "id": cmId,
                    "persona": persona,
                    "task": task,
                ...
                "MySQL error adding countermeasure persona",
                session,
                False,
            )
        self.commitDatabase(session)
    ...
    def personaNarrative(self, scId, environmentId):
        return self.responseList(
            "select personaNarrative(:scId,:env)",
            {"scId": scId, "env": environmentId},
            "MySQL error getting narrative for persona id "
            + str(scId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def personaDirect(self, scId, environmentId):
        directFlag = self.responseList(
            "select personaDirect(:scId,:env)",
            {"scId": scId, "env": environmentId},
            "MySQL error getting directFlag for persona id "
            + str(scId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def addPersonaNarrative(self, stId, environmentName, descriptionText):
        self.updateDatabase(
            "call addPersonaNarrative(:stId,:desc,:env)",
            {"stId": stId, "desc": descriptionText, "env": environmentName},
            "MySQL error adding persona narrative",
        )

    def addPersonaDirect(self, stId, environmentName, directText):
        self.updateDatabase(
            "call addPersonaDirect(:stId,:txt,:env)",
            {"stId": stId, "txt": directText, "env": environmentName},
            "MySQL error adding persona direct flag",
        )

    def taskNarrative(self, scId, environmentId):
        return self.responseList(
            "select taskNarrative(:scId,:env)",
            {"scId": scId, "env": environmentId},
            "MySQL error getting narrative for task id "
            + str(scId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def taskConsequences(self, scId, environmentId):
        return self.responseList(
            "select taskConsequences(:scId,:env)",
            {"scId": scId, "env": environmentId},
            "MySQL error getting consequences for task id "
            + str(scId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def taskBenefits(self, scId, environmentId):
        return self.responseList(
            "select taskBenefits(:scId,:env)",
            {"scId": scId, "env": environmentId},
            "MySQL error getting benefits for task id "
            + str(scId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def addTaskNarrative(self, scId, narrativeText, cText, bText, environmentName):
        self.updateDatabase(
            "call addTaskNarrative(:scId,:nTxt,:cTxt,:bTxt,:env)",
            {
                "scId": scId,
                "nTxt": narrativeText,
                "cTxt": cText,
            ...
            "MySQL error adding task narrative",
        )

    def misuseCaseNarrative(self, mcId, environmentId):
        return self.responseList(
            "select misuseCaseNarrative(:mcId,:env)",
            {"mcId": mcId, "env": environmentId},
            "MySQL error getting narrative for misusecase id "
            + str(mcId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def addMisuseCaseNarrative(self, mcId, narrativeText, environmentName, session):
        self.updateDatabase(
            "call addMisuseCaseNarrative(:mcId,:nTxt,:env)",
            {"mcId": mcId, "nTxt": narrativeText, "env": environmentName},
            "MySQL error adding misuse case narrative",
            session,
        )

    def taskDependencies(self, tId, environmentId):
        return self.responseList(
            "select taskDependencies(:tId,:eId)",
            {"tId": tId, "eId": environmentId},
            "MySQL error getting dependencies for task id "
            + str(tId)
            + " in environment "
            + str(environmentId),
        )[0]
    ...
    def addTaskDependencies(self, tId, depsText, environmentName):
        self.updateDatabase(
            "call addTaskDependencies(:tId,:txt,:env)",
            {"tId": tId, "txt": depsText, "env": environmentName},
            "MySQL error adding task dependencies",
        )

    def mitigatedRisks(self, cmId):
        return self.responseList(
            "call mitigatedRisks(:id)",
            {"id": cmId},
            "MySQL error getting risks mitigated by countermeasure id " + str(cmId),
        )

    def deleteTrace(self, fromObjt, fromName, toObjt, toName):
        self.updateDatabase(
            "call delete_trace(:fObj,:fName,:tObj,:tName)",
            {"fObj": fromObjt, "fName": fromName, "tObj": toObjt, "tName": toName},
            "MySQL error deleting trace",
        )
    ...
    def addGoal(self, parameters):
        ...
        tags = parameters.tags()
        self.updateDatabase(
            "call addGoal(:id,:name,:orig)",
            {"id": goalId, "name": goalName, "orig": goalOrig},
            "MySQL error adding goal",
        )
        ...
        for environmentProperties in parameters.environmentProperties():
            ...
            self.addGoalDefinition(
                goalId, environmentName, environmentProperties.definition()
            )
            self.addGoalCategory(
                goalId, environmentName, environmentProperties.category()
            )
            self.addGoalPriority(
                goalId, environmentName, environmentProperties.priority()
            )
            self.addGoalFitCriterion(
                goalId, environmentName, environmentProperties.fitCriterion()
            )
            self.addGoalIssue(goalId, environmentName, environmentProperties.issue())
            self.addGoalRefinements(
                goalId,
                goalName,
                environmentName,
                environmentProperties.goalRefinements(),
                environmentProperties.subGoalRefinements(),
            )
            self.addGoalConcerns(
                goalId, environmentName, environmentProperties.concerns()
            )
            self.addGoalConcernAssociations(
                goalId, environmentName, environmentProperties.concernAssociations()
            )
    ...
    def updateGoal(self, parameters):
        ...
        tags = parameters.tags()
        session = self.updateDatabase(
            "call deleteGoalComponents(:id)",
            {"id": goalId},
            "MySQL error deleting goal components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateGoal(:id,:name,:orig)",
            {"id": goalId, "name": goalName, "orig": goalOrig},
            "MySQL error updating goal",
            session,
        )
        self.addTags(goalName, "goal", tags)
        for environmentProperties in parameters.environmentProperties():
    ...
    def getGoals(self, constraintId=-1):
        goalRows = self.responseList(
            "call getGoals(:id)", {"id": constraintId}, "MySQL error getting goals"
        )
        goals = {}
        for goalId, goalName, goalOrig in goalRows:
            ...
            parameters = GoalParameters(
                goalName, goalOrig, tags, self.goalEnvironmentProperties(goalId)
            )
    ...
    def getColouredGoals(self, constraintId=-1):
        goalRows = self.responseList(
            "call getColouredGoals(:id)",
            {"id": constraintId},
            "MySQL error getting coloured goals",
        )
        goals = {}
    ...
    def roleTasks(self, environmentName, roles):
        ...
        for role in roles:
            rows = self.responseList(
                "call countermeasureTaskPersonas(:role,:env)",
                {"role": role, "env": environmentName},
                "MySQL error getting role tasks",
            )
    ...
    def taskUsabilityScore(self, taskName, environmentName):
        return int(
            self.responseList(
                "select task_usability(:task,:env)",
                {"task": taskName, "env": environmentName},
                "MySQL error getting task usability",
            )[0]
        )

    def taskLoad(self, taskId, environmentId):
        return self.responseList(
            "select usability_score(:tId,:eId)",
            {"tId": taskId, "eId": environmentId},
            "MySQL error getting task load",
        )[0]

    def countermeasureLoad(self, taskId, environmentId):
        return self.responseList(
            "select hindrance_score(:tId,:eId)",
            {"tId": taskId, "eId": environmentId},
            "MySQL error getting task countermeasure load",
        )[0]

    def environmentDimensions(self, dimension, envName):
        return self.responseList(
            "call " + dimension + "Names(:env)",
            {"env": envName},
            "MySQL error getting "
            + dimension
            + "s associated with environment "
            + envName,
        )
    ...
    def goalModelElements(self, envName):
        return self.responseList(
            "call goalModelElements(:env)",
            {"env": envName},
            "MySQL error getting goal model elements",
        )

    def obstacleModelElements(self, envName):
        return self.responseList(
            "call obstacleModelElements(:env)",
            {"env": envName},
            "MySQL error getting obstacle model elements",
        )

    def responsibilityModelElements(self, envName):
        return self.responseList(
            "call responsibilityModelElements(:env)",
            {"env": envName},
            "MySQL error getting responsibility model elements",
        )

    def taskModelElements(self, envName):
        return self.responseList(
            "call taskModelElements(:env)",
            {"env": envName},
            "MySQL error getting task model elements",
        )
    ...
    def classModelElements(self, envName, hideConcerns=False):
        ...
        return self.responseList(
            callTxt, argDict, "MySQL error getting class model elements"
        )

    def classModel(self, envName, asName="", hideConcerns=False):
        if hideConcerns == True:
            if asName == "":
                return self.classAssociations(
                    "call concernlessClassModel(:id)", envName
                )
            else:
                return self.classTreeAssociations(
                    "call concernlessClassTree(:id1,:id2)", asName, envName
                )
        else:
            if asName == "":
                ...
                return self.classTreeAssociations(
                    "call classTree(:id1,:id2)", asName, envName
                )
    ...
    def classAssociations(self, procName, constraintId=""):
        rows = self.responseList(
            procName, {"id": constraintId}, "MySQL error getting class associations"
        )
        associations = {}
        for (
            associationId,
    ...
    def classTreeAssociations(self, procName, assetName, envName):
        rows = self.responseList(
            procName,
            {"id1": assetName, "id2": envName},
            "MySQL error getting class associations",
        )
        associations = {}
        for (
            associationId,
    ...
    def addClassAssociation(self, parameters):
        ...
        self.updateDatabase(
            "call addClassAssociation(:ass,:env,:hAss,:hType,:hNav,:hMult,:hRole,:tRole,:tMult,:tNav,:tType,:tAss,:rationale)",
            {
                "ass": associationId,
                "env": envName,
                "hAss": headAsset,
                ...
                "tType": tailType,
                "tAss": tailAsset,
                "rationale": assocRationale,
            },
            "MySQL error adding class association",
        )
    ...
    def updateClassAssociation(self, parameters):
        ...
        assocRationale = parameters.rationale()
        self.updateDatabase(
            "call updateClassAssociation(:ass,:env,:hAss,:hType,:hNav,:hMult,:hRole,:tRole,:tMult,:tNav,:tType,:tAss,:rationale)",
            {
                "ass": associationId,
                "env": envName,
                "hAss": headAsset,
            ...
            "MySQL error updating class association",
        )
    ...
    def goalModel(self, envName, goalName="", topLevelGoals=0, caseFilter=0):
        if goalName == "":
            ...
            return self.goalTreeAssociations(
                "call goalTree(:id1,:id2,:id3,:id4)",
                goalName,
                envName,
                topLevelGoals,
                caseFilter,
    ...
    def responsibilityModel(self, envName, roleName=""):
        if roleName == "":
            ...
            return self.goalTreeAssociations(
                "call subResponsibilityModel(:id1,:id2)", envName, roleName
            )
    ...
    def obstacleModel(self, envName, goalName="", topLevelGoals=0):
        if goalName == "":
            ...
            return self.goalTreeAssociations(
                "call obstacleTree(:id1,:id2,:id3,:id4)",
                goalName,
                envName,
                topLevelGoals,
            )
    ...
    def taskModel(self, envName, taskName="", mcFilter=False):
        if taskName == "":
            ...
            if mcFilter == True:
                return self.goalTreeAssociations(
                    "call subMisuseCaseModel(:id1,:id2)", taskName, envName
                )
            else:
                return self.goalTreeAssociations(
                    "call subTaskModel(:id1,:id2)", taskName, envName
                )
    ...
    def getGoalAssociation(self, envName, goalName, subGoalName):
        rows = self.responseList(
            "call getGoalAssociation(:environment,:goal,:subgoal)",
            {"environment": envName, "goal": goalName, "subgoal": subGoalName},
            "MySQL error getting goal association",
        )
        if len(rows) == 0:
            raise DatabaseProxyException(
                "Goal association "
                + envName
                + "/"
                + goalName
    ...
    def goalAssociations(self, procName, constraintId=""):
        rows = self.responseList(
            procName, {"id": constraintId}, "MySQL error getting goal associations"
        )
        associations = {}
        for (
            associationId,
    ...
    def riskObstacleModel(self, riskName, envName):
        rows = self.responseList(
            "call riskObstacleTree(:risk,:env,0)",
            {"risk": riskName, "env": envName},
            "MySQL error getting risk obstacle model",
        )
        associations = {}
        for (
            associationId,
    ...
    def goalTreeAssociations(
        self, procName, goalName, envName, topLevelGoals=0, caseFilter=0
    ):
        rows = []
        if (procName == "call goalTree(:id1,:id2,:id3,:id4)") or (
            procName == "call obstacleTree(:id1,:id2,:id3,:id4)"
        ):
            rows = self.responseList(
                procName,
                {
                ...
                "MySQL error getting goal tree associations",
            )
        else:
            rows = self.responseList(
                procName,
                {"id1": goalName, "id2": envName},
                "MySQL error getting goal tree associations",
            )
        associations = {}
        for (
    ...
    def addGoalAssociation(self, parameters):
        ...
        self.updateDatabase(
            "call addGoalAssociation(:assId,:env,:goal,:dim,:type,:sGName,:sGDName,:altId,:rationale)",
            {
                "assId": associationId,
                "env": envName,
                "goal": goalName,
                ...
                "type": aType,
                "sGName": subGoalName,
                "sGDName": subGoalDimName,
                "altId": alternativeId,
                "rationale": rationale,
            },
            "MySQL error adding goal association",
        )
        return associationId
    ...
    def updateGoalAssociation(self, parameters):
        ...
        rationale = parameters.rationale()
        self.updateDatabase(
            "call updateGoalAssociation(:assId,:env,:goal,:dim,:type,:sGName,:sGDName,:altId,:rationale)",
            {
                "assId": associationId,
                "env": envName,
                "goal": goalName,
            ...
            "MySQL error updating goal association",
        )

    def deleteGoalAssociation(self, associationId, goalDimName, subGoalDimName):
        self.updateDatabase(
            "call delete_goalassociation(:ass,:gDName,:sGDName)",
            {"ass": associationId, "gDName": goalDimName, "sGDName": subGoalDimName},
            "MySQL error deleting goal association",
        )

    def addGoalDefinition(self, goalId, environmentName, goalDef):
        self.updateDatabase(
            "call addGoalDefinition(:gId,:env,:gDef)",
            {"gId": goalId, "env": environmentName, "gDef": goalDef},
            "MySQL error adding goal definition",
        )

    def addGoalCategory(self, goalId, environmentName, goalCat):
        self.updateDatabase(
            "call addGoalCategory(:gId,:env,:gCat)",
            {"gId": goalId, "env": environmentName, "gCat": goalCat},
            "MySQL error adding goal category",
        )

    def addGoalPriority(self, goalId, environmentName, goalPri):
        self.updateDatabase(
            "call addGoalPriority(:gId,:env,:gPri)",
            {"gId": goalId, "env": environmentName, "gPri": goalPri},
            "MySQL error adding goal priority",
        )

    def addGoalFitCriterion(self, goalId, environmentName, goalFC):
        self.updateDatabase(
            "call addGoalFitCriterion(:gId,:env,:gFC)",
            {"gId": goalId, "env": environmentName, "gFC": goalFC},
            "MySQL error adding goal fit criterion",
        )

    def addGoalIssue(self, goalId, environmentName, goalIssue):
        self.updateDatabase(
            "call addGoalIssue(:gID,:env,:gIssue)",
            {"gID": goalId, "env": environmentName, "gIssue": goalIssue},
            "MySQL error adding goal issue",
        )

    def addGoalRefinements(
        self, goalId, goalName, environmentName, goalAssociations, subGoalAssociations
    ):
    ...
    def addGoalConcernAssociations(self, goalId, environmentName, associations):
        for (
            ...
            self.addGoalConcernAssociation(
                goalId,
                environmentName,
                source,
                sourceMultiplicity,
    ...
    def addGoalConcernAssociation(
        ...
        self.updateDatabase(
            "call addGoalConcernAssociation(:gId,:env,:src,:sMulti,:link,:trgt,:tMulti)",
            {
                "gId": goalId,
                "env": environmentName,
                "src": source,
                "sMulti": sourceMultiplicity,
                "link": link,
                "trgt": target,
                "tMulti": targetMultiplicity,
            },
            "MySQL error adding goal concern association",
        )
    ...
    def addTaskConcernAssociations(self, taskId, environmentName, associations):
        for (
            ...
            self.addTaskConcernAssociation(
                taskId,
                environmentName,
                source,
                sourceMultiplicity,
    ...
    def addTaskConcernAssociation(
        ...
        self.updateDatabase(
            "call addTaskConcernAssociation(:tId,:env,:src,:sMulti,:link,:trgt,:tMulti)",
            {
                "tId": taskId,
                "env": environmentName,
                "src": source,
            ...
            "MySQL error adding task concern association",
        )

    def addObstacleRefinements(
        self, goalId, goalName, environmentName, goalAssociations, subGoalAssociations
    ):
        for goal, goalDim, refinement, altName, rationale in subGoalAssociations:
            alternativeId = 0
        ...
        for concern in concerns:
            ...
            if assetId == -1:
                ...
                if assetId != -1:
                    ...
                    exceptionText = (
                        "Cannot add obstacle concern: asset or template asset "
                        + concern
                        + " does not exist."
                    )
                    raise DatabaseProxyException(exceptionText)
    ...
    def addObstacleConcern(self, obsId, environmentName, concern):
        self.updateDatabase(
            "call add_obstacle_concern(:oId,:env,:conc)",
            {"oId": obsId, "env": environmentName, "conc": concern},
            "MySQL error adding obstacle concern",
        )
        ...
        for concern in concerns:
            ...
            if assetId == -1:
                ...
                if assetId != -1:
                    ...
                    exceptionText = (
                        "Cannot add goal concern: asset or template asset "
                        + concern
                        + " does not exist."
                    )
                    raise DatabaseProxyException(exceptionText)
    ...
    def addGoalConcern(self, goalId, environmentName, concern):
        self.updateDatabase(
            "call add_goal_concern(:goal,:env,:conc)",
            {"goal": goalId, "env": environmentName, "conc": concern},
            "MySQL error adding goal concern",
        )

    def addAssetAssociations(
        self, assetId, assetName, environmentName, assetAssociations
    ):
        for (
            headNav,
            headAdornment,
    ...
    def goalLabel(self, goalId, environmentId):
        return self.responseList(
            "select goal_label(:gId,:eId)",
            {"gId": goalId, "eId": environmentId},
            "MySQL error getting goal label",
        )[0]

    def goalDefinition(self, goalId, environmentId):
        return self.responseList(
            "select goal_definition(:gId,:eId)",
            {"gId": goalId, "eId": environmentId},
            "MySQL error getting goal definition",
        )[0]

    def goalCategory(self, goalId, environmentId):
        return self.responseList(
            "select goal_category(:gId,:eId)",
            {"gId": goalId, "eId": environmentId},
            "MySQL error getting goal category",
        )[0]

    def goalPriority(self, goalId, environmentId):
        return self.responseList(
            "select goal_priority(:gId,:eId)",
            {"gId": goalId, "eId": environmentId},
            "MySQL error getting goal priority",
        )[0]

    def goalFitCriterion(self, goalId, environmentId):
        return self.responseList(
            "select goal_fitcriterion(:gId,:eId)",
            {"gId": goalId, "eId": environmentId},
            "MySQL error getting goal fit criterion",
        )[0]

    def goalIssue(self, goalId, environmentId):
        return self.responseList(
            "select goal_issue(:gId,:eId)",
            {"gId": goalId, "eId": environmentId},
            "MySQL error getting goal issue",
        )[0]

    def goalRefinements(self, goalId, environmentId):
        rows = self.responseList(
            "call goalRefinements(:gId,:eId)",
            {"gId": goalId, "eId": environmentId},
            "MySQL error getting goal refinements",
        )
        goalRefinements = []
        for (
            associationId,
        ...
        rows = self.responseList(
            "call subGoalRefinements(:gId,:eId)",
            {"gId": goalId, "eId": environmentId},
            "MySQL error getting goal refinements",
        )
        subGoalRefinements = []
        for (
            ...
            if alternativeId == 1:
                altName = "Yes"
            subGoalRefinements.append(
                (subGoalName, subGoalDimName, aType, altName, rationale)
            )
        return goalRefinements, subGoalRefinements

    def assetAssociations(self, assetId, environmentId):
        rows = self.responseList(
            "call assetAssociations(:aId,:eId)",
            {"aId": assetId, "eId": environmentId},
            "MySQL error getting asset associations",
        )
        associations = []
        for (
            associationId,
    ...
    def getDomainProperties(self, constraintId=-1):
        dpRows = self.responseList(
            "call getDomainProperties(:id)",
            {"id": constraintId},
            "MySQL error getting domain properties",
        )
        dps = {}
    ...
    def addDomainProperty(self, parameters):
        ...
        tags = parameters.tags()
        self.updateDatabase(
            "call addDomainProperty(:id,:name,:desc,:type,:orig)",
            {
                "id": dpId,
                "name": dpName,
                "desc": dpDesc,
            ...
            "MySQL error adding domain property",
        )
    ...
    def updateDomainProperty(self, parameters):
        ...
        tags = parameters.tags()
        self.updateDatabase(
            "call updateDomainProperty(:id,:name,:desc,:type,:orig)",
            {
                "id": dpId,
                "name": dpName,
                "desc": dpDesc,
            ...
            "MySQL error updating domain property",
        )
        self.addTags(dpName, "domainproperty", tags)
        session = self.conn()
        session.commit()
    ...
    def getObstacles(self, constraintId=-1):
        obstacleRows = self.responseList(
            "call getObstacles(:id)",
            {"id": constraintId},
            "MySQL error getting obstacles",
        )
        obstacles = {}
        for obsId, obsName, obsOrig in obstacleRows:
            ...
            parameters = ObstacleParameters(
                obsName, obsOrig, tags, environmentProperties
            )
    ...
    def obstacleEnvironmentProperties(self, obsId):
        ...
        for environmentId, environmentName in self.dimensionEnvironments(
            ...
            properties = ObstacleEnvironmentProperties(
                environmentName,
                obsLabel,
                obsDef,
                obsType,
    ...
    def obstacleDefinition(self, obsId, environmentId):
        obsDef = self.responseList(
            "select obstacle_definition(:oId,:eId)",
            {"oId": obsId, "eId": environmentId},
            "MySQL error getting obstacle definition",
        )[0]
    ...
    def obstacleProbability(self, obsId, environmentId):
        row = self.responseList(
            "call obstacle_probability(:oId,:eId)",
            {"oId": obsId, "eId": environmentId},
            "MySQL error getting obstacle probability",
        )[0]
        obsAttr = row[0]
        obsRationale = row[1]
    ...
    def obstacleCategory(self, obsId, environmentId):
        return self.responseList(
            "select obstacle_category(:oId,:eId)",
            {"oId": obsId, "eId": environmentId},
            "MySQL error getting obstacle category",
        )[0]
    ...
    def addObstacle(self, parameters):
        ...
        tags = parameters.tags()
        self.updateDatabase(
            "call addObstacle(:id,:name,:orig)",
            {"id": obsId, "name": obsName, "orig": obsOrig},
            "MySQL error adding obstacle",
        )
        ...
        for environmentProperties in parameters.environmentProperties():
            ...
            self.addObstacleDefinition(
                obsId,
                environmentName,
                environmentProperties.definition(),
                environmentProperties.probability(),
                environmentProperties.probabilityRationale(),
            )
            self.addObstacleCategory(
                obsId, environmentName, environmentProperties.category()
            )
            self.addObstacleRefinements(
                obsId,
                obsName,
                environmentName,
                environmentProperties.goalRefinements(),
            ...
            self.addObstacleConcerns(
                obsId, environmentName, environmentProperties.concerns()
            )
        return obsId
    ...
    def updateObstacle(self, parameters):
        ...
        tags = parameters.tags()
        session = self.updateDatabase(
            "call deleteObstacleComponents(:id)",
            {"id": obsId},
            "MySQL error deleting obstacle components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateObstacle(:id,:name,:orig)",
            {"id": obsId, "name": obsName, "orig": obsOrig},
            "MySQL error updating obstacle",
            session,
        )
        self.addTags(obsName, "obstacle", tags)
        for environmentProperties in parameters.environmentProperties():
    ...
    def addObstacleDefinition(
        self, obsId, environmentName, obsDef, obsProb, obsProbRat
    ):
        self.updateDatabase(
            "call addObstacleDefinition(:id,:env,:def,:prob,:probRat)",
            {
                "id": obsId,
                "env": environmentName,
                "def": obsDef,
            ...
            "MySQL error adding obstacle definition",
        )

    def addObstacleCategory(self, obsId, environmentName, obsCat):
        self.updateDatabase(
            "call addObstacleCategory(:obs,:env,:cat)",
            {"obs": obsId, "env": environmentName, "cat": obsCat},
            "MySQL error adding obstacle category",
        )
    ...
    def updateSettings(
        ...
        session = self.updateDatabase(
            "call updateProjectSettings(:proj,:bg,:goals,:scope,:picture,:fontSize,:font)",
            {
                "proj": projName,
                "bg": background,
                "goals": goals,
            ...
            "MySQL error updating project settings",
            None,
            False,
        )
        self.updateDatabase(
            ...
            {},
            "MySQL error deleting directory",
            session,
            False,
        )
        for entry in definitions:
            self.updateDatabase(
                "call addDictionaryEntry(:e0,:e1)",
                {"e0": entry["name"], "e1": entry["value"]},
                "MySQL error adding dictionary entry",
                session,
                False,
            )
        self.updateDatabase(
            "call deleteContributors()",
            {},
            "MySQL error deleting contributions",
            session,
            False,
        )
        for entry in contributors:
            self.updateDatabase(
                "call addContributorEntry(:e0,:e1,:e2,:e3)",
                {"e0": entry[0], "e1": entry[1], "e2": entry[2], "e3": entry[3]},
                "MySQL error adding contribution entry",
                session,
                False,
            )
        self.updateDatabase(
            "call deleteRevisions()",
            {},
            "MySQL error deleting revisions",
            session,
            False,
        )
        for entry in revisions:
            self.updateDatabase(
                "call addRevision(:e0,:e1,:e2)",
                {"e0": entry[0], "e1": entry[1], "e2": entry[2]},
                "MySQL error adding revision",
                session,
                False,
            )
        self.commitDatabase(session)
    ...
    def getProjectSettings(self):
        rows = self.responseList(
            "call getProjectSettings()", {}, "MySQL error getting project settings"
        )
        pSettings = {}
    ...
    def getDictionary(self):
        rows = self.responseList(
            "call getDictionary()", {}, "MySQL error getting dictionary"
        )
        pDict = []
        for key, value in rows:
    ...
    def getContributors(self):
        return self.responseList(
            "call getContributors()", {}, "MySQL error getting contributors"
        )

    def getRevisions(self):
        return self.responseList(
            "call getRevisions()", {}, "MySQL error getting revisions"
        )

    def getRequirementVersions(self, reqId):
        return self.responseList(
            "call getRequirementVersions(:id)",
            {"id": reqId},
            "MySQL error getting requirement versions",
        )

    def existingResponseGoal(self, responseId):
        return int(
            self.responseList(
                "select existingResponseGoal(:id)",
                {"id": responseId},
                "MySQL error getting existing response goal",
            )[0]
        )

    def getValueTypes(self, dimName, envName=""):
        customisableValues = set(
            [
                "asset_value",
                "threat_value",
                "risk_class",
        ...
        values = []
        rows = self.responseList(
            "call getCustomisableValues(:dim,:env)",
            {"dim": dimName, "env": envName},
            "MySQL error getting customisable values",
        )
        for typeId, typeName, typeDesc, typeValue, typeRat in rows:
            parameters = ValueTypeParameters(
                typeName, typeDesc, dimName, envName, typeValue, typeRat
            )
    ...
    def addValueType(self, parameters):
        ...
        if (
            (vtType == "asset_value")
            or (vtType == "threat_value")
            or (vtType == "risk_class")
            or (vtType == "countermeasure_value")
        ):
            exceptionText = "Cannot add " + vtType + "s"
            raise DatabaseProxyException(exceptionText)
        self.updateDatabase(
            "call addValueType(:id,:name,:desc,:type,:score,:rat)",
            {
                "id": valueTypeId,
                "name": vtName,
                "desc": vtDesc,
    ...
    def updateValueType(self, parameters):
        ...
        if vtType == "access_right" and vtScore == 0:
            raise DatabaseProxyException("Access right value cannot be set to 0")
        self.updateDatabase(
            "call updateValueType(:id,:name,:desc,:type,:env,:score,:rat)",
            {
                "id": valueTypeId,
                "name": vtName,
                "desc": vtDesc,
            ...
            "MySQL error updating value type",
        )

    def getVulnerabilityDirectory(self, vulName=""):
        return self.responseList(
            "call getVulnerabilityDirectory(:vuln)",
            {"vuln": vulName},
            "MySQL error getting vulnerability directory",
        )

    def getThreatDirectory(self, thrName=""):
        return self.responseList(
            "call getThreatDirectory(:threat)",
            {"threat": thrName},
            "MySQL error getting threat directory",
        )

    def obstacleConcerns(self, obsId, envId):
        return self.responseList(
            "call obstacleConcerns(:obs,:env)",
            {"obs": obsId, "env": envId},
            "MySQL error getting obstacle concerns",
        )

    def goalConcerns(self, goalId, envId):
        return self.responseList(
            "call goalConcerns(:goal,:env)",
            {"goal": goalId, "env": envId},
            "MySQL error getting goal concerns",
        )

    def goalConcernAssociations(self, goalId, envId):
        return self.responseList(
            "call goalConcernAssociations(:goal,:env)",
            {"goal": goalId, "env": envId},
            "MySQL error getting goal concern associations",
        )

    def taskConcernAssociations(self, taskId, envId):
        return self.responseList(
            "call taskConcernAssociations(:task,:env)",
            {"task": taskId, "env": envId},
            "MySQL error getting task concern associations",
        )

    def getDependencies(self, constraintId=""):
        depRows = self.responseList(
            "call getDependencies(:id)",
            {"id": constraintId},
            "MySQL error getting dependencies",
        )
        dependencies = {}
        for (
            depId,
            ...
            parameters = DependencyParameters(
                envName, depender, dependee, dType, dependencyName, rationale
            )
    ...
    def addDependency(self, parameters):
        ...
        rationale = parameters.rationale()
        self.updateDatabase(
            "call addDependency(:id,:env,:depender,:dependee,:type,:name,:rationale)",
            {
                "id": depId,
                "env": envName,
                "depender": depender,
    ...
    def updateDependency(self, parameters):
        ...
        rationale = parameters.rationale()
        self.updateDatabase(
            "call updateDependency(:id,:env,:depender,:dependee,:type,:name,:rationale)",
            {
                "id": depId,
                "env": envName,
                "depender": depender,
            ...
            "MySQL error updating dependency",
        )

    def deleteDependency(self, depId, depType):
        self.updateDatabase(
            "call delete_dependency(:id,:type)",
            {"id": depId, "type": depType},
            "MySQL error deleting dependency",
        )

    def getDependencyTable(self, envName):
        return self.responseList(
            "call dependencyTable(:env)",
            {"env": envName},
            "MySQL error getting dependency table",
        )
    ...
    def reportAssociationDependencies(self, fromAsset, toAsset, envName):
        return self.responseList(
            "call associationDependencyCheck(:from,:to,:env)",
            {"from": fromAsset, "to": toAsset, "env": envName},
            "MySQL error reporting association dependencies",
        )

    def reportAssociationTargetDependencies(self, assetProperties, toAsset, envName):
        return self.responseList(
            "call associationTargetDependencyCheck(:a0,:a1,:a2,:a3,:a4,:a5,:a6,:a7,:to,:env)",
            {
                "a0": assetProperties[0],
                "a1": assetProperties[1],
                "a2": assetProperties[2],
                "a3": assetProperties[3],
                "a4": assetProperties[4],
                "a5": assetProperties[5],
                "a6": assetProperties[6],
                "a7": assetProperties[7],
                "to": toAsset,
                "env": envName,
            },
            "MySQL error reporting association target dependencies",
        )
    ...
    def addTemplateAsset(self, parameters):
        ...
        ifs = parameters.interfaces()
        self.updateDatabase(
            "call addTemplateAsset(:id,:name,:shortCode,:desc,:sig,:type,:surfType,:rights)",
            {
                "id": assetId,
                "name": assetName,
                "shortCode": shortCode,
    ...
    def updateTemplateAsset(self, parameters):
        ...
        self.updateDatabase(
            "call updateTemplateAsset(:id,:name,:shortCode,:desc,:sig,:type,:surfType,:rights)",
            {
                "id": assetId,
                "name": assetName,
                "shortCode": shortCode,
            ...
            "MySQL error updating template asset",
        )
        self.addTags(assetName, "template_asset", tags)
        self.addInterfaces(assetName, "template_asset", ifs)
        self.updateTemplateAssetProperties(
            assetId,
            cProp,
            iProp,
            avProp,
    ...
    def getTemplateAssets(self, constraintId=-1):
        vals = self.responseList(
            "call getTemplateAssets(:id)",
            {"id": constraintId},
            "MySQL error getting template assets",
        )
        templateAssets = {}
        for (
            assetId,
    ...
    def getSecurityPatterns(self, constraintId=-1):
        patternRows = self.responseList(
            "call getSecurityPatterns(:id)",
            {"id": constraintId},
            "MySQL error getting security patterns",
        )
        patterns = {}
        for (
            patternId,
            ...
            parameters = SecurityPatternParameters(
                patternName,
                patternContext,
                patternProblem,
                patternSolution,
    ...
    def patternStructure(self, patternId):
        return self.responseList(
            "call getSecurityPatternStructure(:pat)",
            {"pat": patternId},
            "MySQL error getting pattern structure",
        )

    def patternRequirements(self, patternId):
        return self.responseList(
            "call getSecurityPatternRequirements(:pat)",
            {"pat": patternId},
            "MySQL error getting pattern requirements",
        )
    ...
    def addSecurityPattern(self, parameters):
        ...
        self.updateDatabase(
            "call addSecurityPattern(:id,:name,:cont,:prob,:sol)",
            {
                "id": patternId,
                "name": patternName,
                "cont": patternContext,
            ...
            "MySQL error adding security pattern",
        )
    ...
    def updateSecurityPattern(self, parameters):
        ...
        patternRequirements = parameters.requirements()
        session = self.updateDatabase(
            "call deleteSecurityPatternComponents(:pat)",
            {"pat": patternId},
            "MySQL error deleting security pattern components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateSecurityPattern(:id,:name,:cont,:prob,:sol)",
            {
                "id": patternId,
                "name": patternName,
                "cont": patternContext,
            ...
            "MySQL error updating security pattern",
            session,
        )
        self.addPatternStructure(patternId, patternStructure)
        self.addPatternRequirements(patternId, patternRequirements)
    ...
    def addPatternStructure(self, patternId, patternStructure):
        for (
            ...
            self.addPatternAssetAssociation(
                patternId,
                headAsset,
                headAdornment,
                headNry,
    ...
    def addPatternRequirement(self, reqLabel, patternId, reqName):
        self.updateDatabase(
            "call addSecurityPatternRequirement(:reqLbl,:pat,:req)",
            {"reqLbl": reqLabel, "pat": patternId, "req": reqName},
            "MySQL error adding pattern requirement",
        )

    def addPatternAssetAssociation(
        self,
        patternId,
        headAsset,
        headAdornment,
        ...
        assocId = self.newId()
        self.updateDatabase(
            "call addSecurityPatternStructure(:ass,:pat,:hAss,:hAd,:hNry,:hRole,:tRole,:tNry,:tAd,:tAss)",
            {
                "ass": assocId,
                "pat": patternId,
                "hAss": headAsset,
            ...
            "MySQL adding pattern asset association",
        )

    def patternAssets(self, patternId):
        return self.responseList(
            "call securityPatternAssets(:pat)",
            {"pat": patternId},
            "MySQL error getting pattern assets",
        )
    ...
    def situatePatternAsset(self, patternId, assetId):
        self.updateDatabase(
            "call situatePatternAsset(:ass,:pat)",
            {"ass": assetId, "pat": patternId},
            "MySQL error situating pattern asset",
        )

    def isCountermeasureAssetGenerated(self, cmId):
        return self.responseList(
            "select isCountermeasureAssetGenerated(:cm)",
            {"cm": cmId},
            "MySQL error checking assets associated with countermeasure",
        )[0]

    def isCountermeasurePatternGenerated(self, cmId):
        return self.responseList(
            "select isCountermeasurePatternGenerated(:cm)",
            {"cm": cmId},
            "MySQL error checking patterns associated with countermeasure",
        )[0]

    def updateCountermeasuresEffectiveness(self, objtId, dimName, expCMs):
        for envName, cmName, assetName, cmEffectiveness in expCMs:
            self.updateCountermeasureEffectiveness(
                objtId, dimName, cmName, assetName, envName, cmEffectiveness
            )

    def updateCountermeasureEffectiveness(
        self, objtId, dimName, cmName, assetName, envName, cmEffectiveness
    ):
        self.updateDatabase(
            "call updateCountermeasureEffectiveness(:obj,:dim,:cm,:ass,:env,:cmEff)",
            {
                "obj": objtId,
                "dim": dimName,
                "cm": cmName,
                "ass": assetName,
                "env": envName,
                "cmEff": cmEffectiveness,
            },
            "MySQL error updating effectiveness of countermeasure " + cmName,
        )

    def countermeasurePatterns(self, cmId):
        return self.responseList(
            "call countermeasurePatterns(:cm)",
            {"cm": cmId},
            "MySQL error getting patterns associated with countermeasure",
        )

    def deleteSituatedPattern(self, cmId, patternName):
        self.updateDatabase(
            "call deleteSituatedPattern(:cm,:pat)",
            {"cm": cmId, "pat": patternName},
            "MySQL deleting situated pattern",
        )

    def candidateCountermeasurePatterns(self, cmId):
        return self.responseList(
            "call candidateCountermeasurePatterns(:cm)",
            {"cm": cmId},
            "MySQL error getting candidate countermeasure patterns",
        )

    def associateCountermeasureToPattern(self, cmId, patternName):
        self.updateDatabase(
            "call associateCountermeasureToPattern(:cm,:pat)",
            {"cm": cmId, "pat": patternName},
            "MySQL error associating countermeasure to pattern",
        )
    ...
    def nameCheck(self, objtName, dimName):
        ...
        if dimName == "policy_statement":
            ...
            objtCount = self.responseList(
                "call policyStatementExists(:env,:subj,:at,:res)",
                {"env": envName, "subj": subjName, "at": atName, "res": resName},
                "MySQL error checking existence of " + dimName + " " + objtName,
            )[0]
        else:
            objtCount = self.responseList(
                "call nameExists(:obj,:dim)",
                {"obj": objtName, "dim": dimName},
                "MySQL error checking existence of " + dimName + " " + objtName,
            )[0]
        if objtCount > 0:
            raise ARMException("Object with name " + objtName + " already exists.")

    def nameCheckEnvironment(self, objtName, envName, dimName):
        objtCount = self.responseList(
            "call nameEnvironmentExists(:obj,:env,:dim)",
            {"obj": objtName, "env": envName, "dim": dimName},
            "MySQL error naming checking in environment",
        )[0]
        if objtCount > 0:
            raise ARMException(
                dimName
    ...
    def nameExists(self, objtName, dimName):
        ...
        if dimName == "policy_statement":
            ...
            objtCount = self.responseList(
                "call nameExists(:obj,:dim)",
                {"obj": objtName, "dim": dimName},
                "MySQL error checking name exists",
            )[0]
        if objtCount > 0:
            return True
        else:
    ...
    def getExternalDocuments(self, constraintId=-1):
        edRows = self.responseList(
            "call getExternalDocuments(:id)",
            {"id": constraintId},
            "MySQL error getting external documents",
        )
        eDocs = {}

        for docId, docName, docVersion, docPubDate, docAuthors, docDesc in edRows:
            parameters = ExternalDocumentParameters(
                docName, docVersion, docPubDate, docAuthors, docDesc
            )
    ...
    def getDocumentReferences(self, constraintId=-1):
        drRows = self.responseList(
            "call getDocumentReferences(:id)",
            {"id": constraintId},
            "MySQL error getting document references",
        )
        dRefs = {}
    ...
    def getPersonaDocumentReferences(self, personaName):
        return self.responseList(
            "call getPersonaDocumentReferences(:pers)",
            {"pers": personaName},
            "MySQL error getting document references for persona " + personaName,
        )

    def getPersonaConceptReferences(self, personaName):
        return self.responseList(
            "call getPersonaConceptReferences(:pers)",
            {"pers": personaName},
            "MySQL error getting concept references for persona " + personaName,
        )

    def getPersonaExternalDocuments(self, personaName):
        return self.responseList(
            "call getPersonaExternalDocuments(:pers)",
            {"pers": personaName},
            "MySQL error getting external documents for persona " + personaName,
        )

    def getPersonaCharacteristics(self, constraintId=-1):
        pcSumm = self.responseList(
            "call getPersonaCharacteristics(:id)",
            {"id": constraintId},
            "MySQL error getting persona characteristics",
        )
        pChars = {}
        for pcId, pName, bvName, qualName, pcDesc in pcSumm:
            grounds, warrant, backing, rebuttal = self.characteristicReferences(
                pcId, "characteristicReferences"
            )
            parameters = PersonaCharacteristicParameters(
                pName, qualName, bvName, pcDesc, grounds, warrant, backing, rebuttal
            )
    ...
    def characteristicReferences(self, pcId, spName):
        rows = self.responseList(
            "call " + spName + "(:pc)",
            {"pc": pcId},
            "MySQL error getting characteristic references",
        )
        refDict = {}
        refDict["grounds"] = []
        refDict["warrant"] = []
    ...
    def addExternalDocument(self, parameters):
        ...
        if sys.version_info > (3,):
            ...
            docName = self.conn.connection().connection.escape_string(
                docName.replace("\\u2018", "'")
                .replace("\\u2019", "'")
                .replace("\\u2013", "-")
                .replace("\\u2022", "*")
        ...
        docAuthors = self.conn.connection().connection.escape_string(
            parameters.authors()
        )
        docDesc = self.conn.connection().connection.escape_string(
            parameters.description()
        )
        if sys.version_info > (3,):
            self.updateDatabase(
                "call addExternalDocument(:id,:name,:vers,:date,:auth,:desc)",
                {
                    "id": docId,
                    "name": docName,
                    "vers": docVersion,
                ...
                "MySQL error adding external document",
            )
        else:
            self.updateDatabase(
                "call addExternalDocument(:id,:name,:vers,:date,:auth,:desc)",
    ...
    def updateExternalDocument(self, parameters):
        ...
        if sys.version_info > (3,):
            self.updateDatabase(
                "call updateExternalDocument(:id,:name,:vers,:date,:auth,:desc)",
                {
                    "id": docId,
                    "name": docName,
                    "vers": docVersion,
                ...
                "MySQL error updating external document",
            )
        else:
            self.updateDatabase(
                "call updateExternalDocument(:id,:name,:vers,:date,:auth,:desc)",
    ...
    def addDocumentReference(self, parameters):
        ...
        if sys.version_info < (3,):
            ...
            refName = self.conn.connection().connection.escape_string(
                refName.replace("\\u2018", "'")
                .replace("\\u2019", "'")
                .replace("\\u2013", "-")
                .replace("\\u2022", "*")
            ...
            self.updateDatabase(
                "call addDocumentReference(:rId,:rName,:dName,:cName,:rExec)",
                {
                    "rId": refId,
                    "rName": refName,
                    "dName": docName,
                ...
                "MySQL error adding document reference",
            )
        else:
            self.updateDatabase(
                "call addDocumentReference(:rId,:rName,:dName,:cName,:rExec)",
    ...
    def updateDocumentReference(self, parameters):
        ...
        if sys.version_info > (3,):
            self.updateDatabase(
                "call updateDocumentReference(:rId,:rName,:dName,:cName,:rExec)",
                {
                    "rId": refId,
                    "rName": refName,
                    "dName": docName,
                ...
                "MySQL error updating document reference",
            )
        else:
            self.updateDatabase(
                "call updateDocumentReference(:rId,:rName,:dName,:cName,:rExec)",
    ...
    def addPersonaCharacteristic(self, parameters):
        ...
        if sys.version_info > (3,):
            self.updateDatabase(
                "call addPersonaCharacteristic(:pc,:pers,:qual,:bVar,:cDesc)",
                {
                    "pc": pcId,
                    "pers": personaName,
                    "qual": qualName,
                ...
                "MySQL error adding persona characteristic",
            )
        else:
            self.updateDatabase(
                "call addPersonaCharacteristic(:pc,:pers,:qual,:bVar,:cDesc)",
    ...
    def updatePersonaCharacteristic(self, parameters):
        ...
        rebuttal = parameters.rebuttal()
        session = self.updateDatabase(
            "call deletePersonaCharacteristicComponents(:pers)",
            {"pers": pcId},
            "MySQL error deleting persona characteristic components",
            None,
            False,
        )
        if sys.version_info > (3,):
            self.updateDatabase(
                "call updatePersonaCharacteristic(:pc,:pers,:qual,:bVar,:cDesc)",
                {
                    "pc": pcId,
                    "pers": personaName,
                    "qual": qualName,
                ...
                "MySQL error updating persona characteristic",
                session,
            )
        else:
            self.updateDatabase(
    ...
    def getConceptReferences(self, constraintId=-1):
        crRows = self.responseList(
            "call getConceptReferences(:id)",
            {"id": constraintId},
            "MySQL error getting concept references",
        )
        cRefs = {}
    ...
    def addConceptReference(self, parameters):
        ...
        self.updateDatabase(
            "call addConceptReference(:rId,:rName,:dName,:obj,:cDesc)",
            {
                "rId": refId,
                "rName": refName,
                "dName": dimName,
            ...
            "MySQL error adding concept reference",
        )
        return refId
    ...
    def updateConceptReference(self, parameters):
        ...
        cDesc = parameters.description()
        self.updateDatabase(
            "call updateConceptReference(:rId,:rName,:dName,:obj,:cDesc)",
            {
                "rId": refId,
                "rName": refName,
                "dName": dimName,
            ...
            "MySQL error updating concept reference",
        )
        return refId

    def deleteConceptReference(self, refId, dimName):
        self.updateDatabase(
            "call delete_concept_reference(:ref,:dim)",
            {"ref": refId, "dim": dimName},
            "MySQL error deleting concept reference",
        )
    ...
    def addPersonaCharacteristicReference(
        self, pcId, refName, crTypeName, refDesc, dimName
    ):
        self.updateDatabase(
            "call addPersonaCharacteristicReference(:pc,:ref,:cr,:refD,:dim)",
            {
                "pc": pcId,
                "ref": refName,
                "cr": crTypeName,
            ...
            "MySQL error adding persona characteristic reference",
        )

    def referenceDescription(self, dimName, refName):
        return self.responseList(
            "call referenceDescription(:dim,:ref)",
            {"dim": dimName, "ref": refName},
            "MySQL error getting reference description",
        )[0]

    def documentReferenceNames(self, docName):
        return self.responseList(
            "call documentReferenceNames(:doc)",
            {"doc": docName},
            "MySQL error getting document reference names",
        )

    def referenceUse(self, refName, dimName):
        return self.responseList(
            "call referenceUse(:ref,:dim)",
            {"ref": refName, "dim": dimName},
            "MySQL error getting reference use",
        )
    ...
    def characteristicBacking(self, pcId, spName):
        ...
        return self.responseList(
            callTxt, argDict, "MySQL error getting characteristic backing"
        )

    def assumptionPersonaModel(self, personaName="", bvName="", pcName=""):
        return self.responseList(
            "call assumptionPersonaModel(:pers,:bv,:pc)",
            {"pers": personaName, "bv": bvName, "pc": pcName},
            "MySQL error getting assumption persona model",
        )
    ...
    def getArgReference(self, atName, constraintName):
        row = self.responseList(
            "call get" + atName + "(:const)",
            {"const": constraintName},
            "MySQL error getting " + atName + ":" + constraintName,
        )[0]
        groundsName = row[0]
        dimName = row[1]
        objtName = row[2]
    ...
    def addDirectoryEntry(self, dLabel, dName, dDesc, dTypeId, dRef, dimName):
        ...
        self.updateDatabase(
            "call add" + dimName + "DirectoryEntry(:lbl,:name,:desc,:type,:ref)",
            {"lbl": dLabel, "name": dName, "desc": dDesc, "type": dTypeId, "ref": dRef},
            "MySQL error adding directory entry",
        )

    def lastRequirementLabel(self, assetName):
        return self.responseList(
            "select lastRequirementLabel(:ass)",
            {"ass": assetName},
            "MySQL error getting last requirement label",
        )[0]

    def getUseCases(self, constraintId=-1):
        ucRows = self.responseList(
            "call getUseCases(:id)",
            {"id": constraintId},
            "MySQL error getting use cases",
        )
        ucs = {}
        for ucId, ucName, ucAuth, ucCode, ucDesc in ucRows:
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                properties = UseCaseEnvironmentProperties(
                    environmentName, preConds, ucSteps, postConds
                )
                environmentProperties.append(properties)
                parameters = UseCaseParameters(
                    ucName, ucAuth, ucCode, ucRoles, ucDesc, tags, environmentProperties
                )
    ...
    def useCaseRoles(self, ucId):
        return self.responseList(
            "call useCaseRoles(:id)",
            {"id": ucId},
            "MySQL error getting actors associated with use case id " + str(ucId),
        )

    def useCaseConditions(self, ucId, envId):
        return self.responseList(
            "call useCaseConditions(:uc,:env)",
            {"uc": ucId, "env": envId},
            "MySQL error getting conditions associated with use case id " + str(ucId),
        )[0]

    def useCaseSteps(self, ucId, envId):
        stepRows = self.responseList(
            "call useCaseSteps(:uc,:env)",
            {"uc": ucId, "env": envId},
            "MySQL error getting steps associated with use case id " + str(ucId),
        )
        steps = Steps()
    ...
    def useCaseStepExceptions(self, ucId, envId, stepNo):
        return self.responseList(
            "call useCaseStepExceptions(:uc,:env,:step)",
            {"uc": ucId, "env": envId, "step": stepNo},
            "MySQL error getting exceptions associated with use case id " + str(ucId),
        )

    def useCaseStepTags(self, ucId, envId, stepNo):
        return self.responseList(
            "call useCaseStepTags(:uc,:env,:step)",
            {"uc": ucId, "env": envId, "step": stepNo},
            "MySQL error getting tags associated with use case id " + str(ucId),
        )
    ...
    def addUseCase(self, parameters):
        ...
        ucId = self.newId()
        self.updateDatabase(
            "call addUseCase(:id,:name,:auth,:code,:desc)",
            {
                "id": ucId,
                "name": ucName,
                "auth": ucAuth,
        ...
        for cProperties in parameters.environmentProperties():
            ...
            self.addUseCaseConditions(
                ucId,
                environmentName,
                cProperties.preconditions(),
                cProperties.postconditions(),
            )
    ...
    def addUseCaseRole(self, ucId, actor):
        self.updateDatabase(
            "call addUseCaseRole(:id,:act)",
            {"id": ucId, "act": actor},
            "MySQL error adding use case role",
        )

    def addUseCaseConditions(self, ucId, envName, preCond, postCond):
        self.updateDatabase(
            "call addUseCaseConditions(:id,:env,:pre,:post)",
            {"id": ucId, "env": envName, "pre": preCond, "post": postCond},
            "MySQL error adding use case conditions",
        )
    ...
    def addUseCaseStep(self, ucId, envName, stepNo, step):
        step.validate()
        self.updateDatabase(
            "call addUseCaseStep(:id,:env,:step,:text,:synopsis,:actor,:type)",
            {
                "id": ucId,
                "env": envName,
                "step": stepNo,
            ...
            "MySQL error adding use case step",
        )
        for tag in set(step.tags()):
            self.addUseCaseStepTag(ucId, envName, stepNo, tag)
        for idx, exc in list((step.theExceptions).items()):
            self.addUseCaseStepException(
                ucId, envName, stepNo, exc[0], exc[1], exc[2], exc[3], exc[4]
            )

    def addUseCaseStepTag(self, ucId, envName, stepNo, tag):
        self.updateDatabase(
            "call addUseCaseStepTag(:id,:env,:step,:tag)",
            {"id": ucId, "env": envName, "step": stepNo, "tag": tag},
            "MySQL error adding use case step tag",
        )

    def addUseCaseStepException(
        self, ucId, envName, stepNo, exName, dimType, dimName, catName, exDesc
    ):
        self.updateDatabase(
            "call addUseCaseStepException(:uc,:env,:step,:ex,:dType,:dName,:cName,:desc)",
            {
                "uc": ucId,
                "env": envName,
                "step": stepNo,
            ...
            "MySQL error adding use case step exception",
        )
    ...
    def updateUseCase(self, parameters):
        ...
        tags = parameters.tags()
        session = self.updateDatabase(
            "call deleteUseCaseComponents(:uc)",
            {"uc": ucId},
            "MySQL error deleting use case components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateUseCase(:id,:name,:auth,:code,:desc)",
            {
                "id": ucId,
                "name": ucName,
                "auth": ucAuth,
    ...
    def riskModel(self, environmentName, riskName):
        rows = self.responseList(
            "call riskModel(:risk,:env)",
            {"risk": riskName, "env": environmentName},
            "MySQL error getting risk model",
        )
        traces = []
    ...
    def isRisk(self, candidateRiskName):
        return self.responseList(
            "select is_risk(:cand)",
            {"cand": candidateRiskName},
            "MySQL error checking candidate risk",
        )[0]

    def textualArgumentationModel(self, personaName, bvType):
        return self.responseList(
            "call assumptionPersonaModel_textual(:pers,:type)",
            {"pers": personaName, "type": bvType},
            "MySQL error getting textual argumentation model",
        )

    def riskAnalysisToXml(self, includeHeader=True):
        return self.responseList(
            "call riskAnalysisToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting risk analysis artifacts to XML",
        )[0]

    def riskAnalysisToJSON(self):
        return self.responseList(
            "call riskAnalysisToJSON()",
            {},
            "MySQL error exporting risk analysis artifacts to JSON",
        )[0]

    def goalsToXml(self, includeHeader=True):
        return self.responseList(
            "call goalsToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting goals to XML",
        )[0]

    def goalsToJSON(self):
        return self.responseList(
            "call goalsToJSON()", {}, "MySQL error exporting goals to JSON"
        )[0]

    def usabilityToXml(self, includeHeader=True):
        return self.responseList(
            "call usabilityToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting usability data to XML",
        )[0]

    def usabilityToJSON(self):
        return self.responseList(
            "call usabilityToJSON()", {}, "MySQL error exporting usability data to JSON"
        )[0]

    def misusabilityToXml(self, includeHeader=True):
        return self.responseList(
            "call misusabilityToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting misusability data to XML",
        )[0]

    def misusabilityToJSON(self):
        return self.responseList(
            "call misusabilityToJSON()",
            {},
            "MySQL error exporting misusability data to JSON",
        )[0]

    def synopsesToXml(self, includeHeader=True):
        return self.responseList(
            "call synopsesToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting synopses data to XML",
        )[0]

    def associationsToXml(self, includeHeader=True):
        return self.responseList(
            "call associationsToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting association data to XML",
        )[0]

    def dataflowsToXml(self, includeHeader=True):
        return self.responseList(
            "call dataflowsToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting dataflow data to XML",
        )[0]

    def dataflowsToJSON(self):
        return self.responseList(
            "call dataflowsToJSON()", {}, "MySQL error exporting dataflow data to JSON"
        )[0]

    def projectToXml(self, includeHeader=True):
        return self.responseList(
            "call projectToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting project data to XML",
        )[0]

    def projectToJSON(self):
        return self.responseList(
            "call projectToJSON()", {}, "MySQL error exporting project data to JSON"
        )[0]

    def architecturalPatternToXml(self, apName):
        return self.responseList(
            "call architecturalPatternToXml(:name)",
            {"name": apName},
            "MySQL error exporting architectural pattern to XML",
        )[0]

    def getTaskCharacteristics(self, constraintId=-1):
        tcSumm = self.responseList(
            "call getTaskCharacteristics(:id)",
            {"id": constraintId},
            "MySQL error getting task characteristics",
        )
        tChars = {}
        for tcId, tName, qualName, tcDesc in tcSumm:
            grounds, warrant, backing, rebuttal = self.characteristicReferences(
                tcId, "taskCharacteristicReferences"
            )
            parameters = TaskCharacteristicParameters(
                tName, qualName, tcDesc, grounds, warrant, backing, rebuttal
            )
    ...
    def addTaskCharacteristic(self, parameters):
        ...
        qualName = self.conn.connection().connection.escape_string(
            parameters.qualifier()
        )
        cDesc = self.conn.connection().connection.escape_string(
        ...
        rebuttal = parameters.rebuttal()
        self.updateDatabase(
            "call addTaskCharacteristic(:id,:task,:qual,:desc)",
            {"id": tcId, "task": taskName, "qual": qualName, "desc": cDesc},
            "MySQL error adding task characteristic",
        )
    ...
    def addTaskCharacteristicReferences(self, tcId, grounds, warrant, rebuttal):
        ...
        for w, desc, dim in warrant:
            self.addTaskCharacteristicReference(tcId, w, "warrant", desc, dim)

        for r, desc, dim in rebuttal:
            self.addTaskCharacteristicReference(tcId, r, "rebuttal", desc, dim)

    def addTaskCharacteristicReference(
        self, tcId, refName, crTypeName, refDesc, dimName
    ):
        self.updateDatabase(
            "call addTaskCharacteristicReference(:id,:ref,:type,:desc,:dim)",
            {
                "id": tcId,
                "ref": refName,
                "type": crTypeName,
            ...
            "MySQL error adding task characteristic reference",
        )
    ...
    def updateTaskCharacteristic(self, parameters):
        ...
        rebuttal = parameters.rebuttal()
        session = self.updateDatabase(
            "call deleteTaskCharacteristicComponents(:task)",
            {"task": tcId},
            "MySQL error deleting task characteristic components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateTaskCharacteristic(:id,:task,:qual,:desc)",
            {"id": tcId, "task": taskName, "qual": qualName, "desc": cDesc},
            "MySQL error updating task characteristic",
            session,
        )
        self.addTaskCharacteristicReferences(tcId, grounds, warrant, rebuttal)
    ...
    def assumptionTaskModel(self, taskName="", tcName=""):
        return self.responseList(
            "call assumptionTaskModel(:task,:tc)",
            {"task": taskName, "tc": tcName},
            "MySQL error getting assumption task model",
        )
    ...
    def searchModel(self, inTxt, opts):
        ...
        return self.responseList(
            "call grepModel(:in,:ps,:env,:role,:pc,:tc,:ref,:p,:task,:uc,:dp,:goal,:obs,:req,:asset,:vul,:attacker,:thr,:risk,:resp,:cm,:dir,:code,:memo,:id,:tag)",
            argDict,
            "MySQL error searching model",
        )

    def getExternalDocumentReferencesByExternalDocument(self, edName):
        return self.responseList(
            "call getExternalDocumentReferences(:name)",
            {"name": edName},
            "MySQL error external document references",
        )

    def dimensionNameByShortCode(self, scName):
        return self.responseList(
            "call dimensionNameByShortCode(:shortCode)",
            {"shortCode": scName},
            "MySQL error calling dimension name by short code",
        )

    def misuseCaseRiskComponents(self, mcName):
        return self.responseList(
            "call misuseCaseRiskComponents(:misuse)",
            {"misuse": mcName},
            "MySQL error getting risk components associated with misuse case " + mcName,
        )[0]

    def personaToXml(self, pName):
        return self.responseList(
            "call personaToXml(:persona)",
            {"persona": pName},
            "MySQL error exporting persona to XML",
        )[0]
    ...
    def defaultEnvironment(self):
        return self.responseList(
            ...
            {},
            "MySQL error obtaining default environment",
        )[0]

    def environmentTensions(self, envName):
        rows = self.responseList(
            "call environmentTensions(:env)",
            {"env": envName},
            "MySQL error getting environment tensions",
        )
        vts = {}
        rowIdx = 0
    ...
    def getReferenceSynopsis(self, refName):
        row = self.responseList(
            "call getReferenceSynopsis(:ref)",
            {"ref": refName},
            "MySQL error getting response synopsis",
        )[0]
        rsId = row[0]
        synName = row[1]
        dimName = row[2]
        ...
        rs = ReferenceSynopsis(
            rsId, refName, synName, dimName, aType, aName, synDim, gsName, goals
        )
        return rs

    def getReferenceContribution(self, charName, refName):
        row = self.responseList(
            "call getReferenceContribution(:ref,:char)",
            {"ref": refName, "char": charName},
            "MySQL error getting reference contribution",
        )[0]
        rsName = row[0]
        csName = row[1]
        me = row[2]
    ...
    def addReferenceContribution(self, rc):
        ...
        self.updateDatabase(
            "call addReferenceContribution(:rs,:cs,:me,:cont)",
            {"rs": rsName, "cs": csName, "me": meName, "cont": contName},
            "MySQL error adding reference contribution",
        )
    ...
    def updateReferenceContribution(self, rc):
        ...
        contName = rc.contribution()
        self.updateDatabase(
            "call updateReferenceContribution(:rs,:cs,:me,:cont)",
            {"rs": rsName, "cs": csName, "me": meName, "cont": contName},
            "MySQL error updating reference contribution",
        )
    ...
    def addReferenceSynopsis(self, rs):
        ...
        self.updateDatabase(
            "call addReferenceSynopsis(:rsId,:ref,:rs,:dim,:atName,:actName,:gSat)",
            {
                "rsId": rsId,
                "ref": refName,
                "rs": rsName,
            ...
            "MySQL error adding reference synopsis",
        )
    ...
    def updateReferenceSynopsis(self, rs):
        ...
        gSat = rs.satisfaction()
        session = self.updateDatabase(
            "call deleteUserGoalComponents(:id)",
            {"id": rsId},
            "MySQL error deleting user goal components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateReferenceSynopsis(:rsId,:ref,:rs,:dim,:atName,:actName,:gSat)",
            {
                "rsId": rsId,
                "ref": refName,
                "rs": rsName,
            ...
            "MySQL error updating reference synopsis",
            session,
        )
        for sysGoal in rs.goals():
            self.addUserSystemGoalLink(rsName, sysGoal)
    ...
    def addCharacteristicSynopsis(self, cs):
        ...
        self.updateDatabase(
            "call addCharacteristicSynopsis(:cName,:csName,:csDim,:atName,:actName,:gSat)",
            {
                "cName": cName,
                "csName": csName,
                "csDim": csDim,
            ...
            "MySQL error adding characteristic synopsis",
        )
    ...
    def updateCharacteristicSynopsis(self, cs):
        ...
        self.updateDatabase(
            "call updateCharacteristicSynopsis(:cName,:csName,:csDim,:atName,:actName,:gSat)",
            {
                "cName": cName,
                "csName": csName,
                "csDim": csDim,
            ...
            "MySQL error updating characteristic synopsis",
            session,
        )
        for sysGoal in cs.goals():
            self.addUserSystemGoalLink(csName, sysGoal)
    ...
    def referenceCharacteristic(self, refName):
        return self.responseList(
            "call referenceCharacteristic(:ref)",
            {"ref": refName},
            "MySQL error getting characteristics associated with reference " + refName,
        )

    def getCharacteristicSynopsis(self, cName):
        row = self.responseList(
            "call getCharacteristicSynopsis(:characteristic)",
            {"characteristic": cName},
            "MySQL error getting characteristic synopsis",
        )[0]
        synName = row[0]
        dimName = row[1]
        aType = row[2]
    ...
    def hasCharacteristicSynopsis(self, charName):
        return self.responseList(
            "select hasCharacteristicSynopsis(:characteristic)",
            {"characteristic": charName},
            "MySQL error finding characteristic synopsis",
        )[0]

    def hasReferenceSynopsis(self, refName):
        return self.responseList(
            "select hasReferenceSynopsis(:ref)",
            {"ref": refName},
            "MySQL error finding reference synopsis",
        )[0]

    def addUseCaseSynopsis(self, cs):
        self.updateDatabase(
            "call addUseCaseSynopsis(:cName,:csName,:csDim,:atName,:actName)",
            {
                "cName": cs.reference(),
                "csName": cs.synopsis(),
                "csDim": cs.dimension(),
            ...
            "MySQL error adding use case synopsis",
        )

    def getUseCaseContributions(self, ucName):
        rows = self.responseList(
            "call getUseCaseContributions(:useCase)",
            {"useCase": ucName},
            "MySQL error getting use case contributions",
        )
        ucs = {}
    ...
    def addUseCaseContribution(self, rc):
        ...
        contName = rc.contribution()
        self.updateDatabase(
            "call addUseCaseContribution(:useCase,:csName,:meName,:cont)",
            {"useCase": ucName, "csName": csName, "meName": meName, "cont": contName},
            "MySQL error adding use case contribution",
        )
    ...
    def updateUseCaseContribution(self, rc):
        ...
        contName = rc.contribution()
        self.updateDatabase(
            "call updateUseCaseContribution(:useCase,:csName,:meName,:cont)",
            {"useCase": ucName, "csName": csName, "meName": meName, "cont": contName},
            "MySQL error updating use case contribution",
        )
    ...
    def addTaskContribution(self, tc):
        ...
        self.updateDatabase(
            "call addTaskContribution(:src,:dest,:env,:cont)",
            {"src": src, "dest": dest, "env": env, "cont": cont},
            "MySQL error adding task contribution",
        )
    ...
    def updateTaskContribution(self, tc):
        ...
        cont = tc.contribution()
        self.updateDatabase(
            "call updateTaskContribution(:src,:dest,:env,:cont)",
            {"src": src, "dest": dest, "env": env, "cont": cont},
            "MySQL error updating task contribution",
        )

    def pcToGrl(self, pNames, tNames, envName):
        return self.responseList(
            'call pcToGrl(":pNames", ":tNames", :env)',
            {"pNames": pNames, "tNames": tNames, "env": envName},
            "MySQL error exporting to GRL",
        )[0]

    def dependentLabels(self, goalName, envName):
        return self.responseList(
            "call dependentLabels(:goal,:env)",
            {"goal": goalName, "env": envName},
            "MySQL error getting dependent labels",
        )

    def relabelGoals(self, envName):
        self.updateDatabase(
            "call relabelGoals(:env)", {"env": envName}, "MySQL error relabelling goals"
        )

    def relabelObstacles(self, envName):
        self.updateDatabase(
            "call relabelObstacles(:env)",
            {"env": envName},
            "MySQL error relabelling obstacles",
        )

    def obstacleLabel(self, goalId, environmentId):
        return self.responseList(
            "select obstacle_label(:goal,:env)",
            {"goal": goalId, "env": environmentId},
            "MySQL error getting obstacle label",
        )[0]

    def redmineGoals(self, envName):
        goalRows = self.responseList(
            "call redmineGoals(:env)",
            {"env": envName},
            "MySQL error getting redmine goals",
        )
        goals = {}
        for (
            goalId,
    ...
    def redmineUseCases(self):
        return self.responseList(
            "call usecasesToRedmine()", {}, "MySQL exporting usecases to Redmine"
        )

    def redmineScenarios(self):
        return self.responseList(
            "call redmineScenarios()", {}, "MySQL exporting scenarios to Redmine"
        )

    def redmineArchitecture(self):
        return self.responseList(
            "call redmineArchitecture()", {}, "MySQL exporting architecture to Redmine"
        )

    def redmineAttackPatterns(self):
        return self.responseList(
            "call redmineAttackPatterns()",
            {},
            "MySQL exporting attack patterns to Redmine",
        )

    def tvTypesToXml(self, includeHeader=True):
        return self.responseList(
            "call tvTypesToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting threat and vulnerability types to XML",
        )[0]
    ...
    def tvTypesToJSON(self):
        return self.responseList(
            "call tvTypesToJSON()",
            {},
            "MySQL error exporting threat and vulnerability types to JSON",
        )[0]

    def domainValuesToXml(self, includeHeader=True):
        return self.responseList(
            "call domainValuesToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting domain values to XML",
        )[0]

    def domainValuesToJSON(self, includeHeader=True):
        return self.responseList(
            "call domainValuesToJSON()",
            {},
            "MySQL error exporting domain values to JSON",
        )[0]

    def locationsToXml(self):
        return self.responseList(
            "call locationsToXml()", {}, "MySQL error exporting locations to XML"
        )[0]

    def locationsToJSON(self):
        return self.responseList(
            "call locationsToJSON()", {}, "MySQL error exporting locations to JSON"
        )[0]

    def associationsToJSON(self):
        return self.responseList(
            "call associationsToJSON()",
            {},
            "MySQL error exporting associations to JSON",
        )[0]
    ...
    def conceptMapModel(self, envName, reqName="all"):
        ...
        rows = self.responseList(
            callTxt, argDict, "MySQL error getting concept map model"
        )
        associations = {}

        for fromName, toName, lbl, fromEnv, toEnv in rows:
            cmLabel = fromName + "#" + toName + "#" + lbl
            assoc = ConceptMapAssociationParameters(
                fromName, toName, lbl, fromEnv, toEnv
            )
    ...
    def traceabilityScore(self, reqName):
        return self.responseList(
            "select traceabilityScore(:req)",
            {"req": reqName},
            "MySQL error getting traceability score",
        )[0]

    def getRedmineRequirements(self):
        reqRows = self.responseList(
            "select name,originator,priority,comments,description,environment_code,environment from redmine_requirement order by 1",
            {},
            "MySQL error getting redmine requirements",
        )
        reqs = {}
    ...
    def getRequirementScenarios(self, reqName):
        scs = self.responseList(
            "call requirementScenarios(:req)",
            {"req": reqName},
            "MySQL error getting redmine scenarios",
        )
        if len(scs) == 0:
            scs.append("None")
        return scs
    ...
    def getRequirementUseCases(self, reqName):
        ucs = self.responseList(
            "call requirementUseCases(:req)",
            {"req": reqName},
            "MySQL error getting redmine use cases",
        )
        if len(ucs) == 0:
            ucs.append("None")
        return ucs
    ...
    def getRequirementBacklog(self, reqName):
        bis = self.responseList(
            "call requirementBacklog(:req)",
            {"req": reqName},
            "MySQL error getting backlog items",
        )
        if len(bis) == 0:
            bis.append("None")
        return bis
    ...
    def environmentRequirements(self, envName):
        return self.responseList(
            "call requirementNames(:env)",
            {"env": envName},
            "MySQL error getting requirements associated with environment " + envName,
        )

    def deleteTags(self, tagObjt, tagDim):
        self.updateDatabase(
            "call deleteTags(:obj,:dim)",
            {"obj": tagObjt, "dim": tagDim},
            "MySQL error deleting tags",
        )

    def deleteDataFlowTags(self, dfName, fromType, fromName, toType, toName, envName):
        self.updateDatabase(
            "call deleteDataFlowTags(:dfName, :fromType, :fromName, :toType, :toName, :envName)",
            {
                "dfName": dfName,
                "fromType": fromType,
                "fromName": fromName,
            ...
            "MySQL error deleting data flow tags",
        )
    ...
    def addTags(self, dimObjt, dimName, tags):
        ...
        for tag in set(tags):
            try:
                ...
                raise DatabaseProxyException(
                    "MySQL error adding tag (message: " + format(e)
                )
            except DatabaseError as e:
                raise DatabaseProxyException(
                    "MySQL error adding "
    ...
    def addDataFlowTags(
        self, dfName, fromType, fromName, toType, toName, envName, tags
    ):
        ...
        for tag in set(tags):
            try:
                curs.execute(
                    "call addDataFlowTag(%s,%s,%s,%s,%s,%s,%s)",
                    [dfName, fromType, fromName, toType, toName, envName, tag],
                )
            except OperationalError as e:
                raise DatabaseProxyException(
                    "MySQL error adding dataflow tag (message: " + format(e)
                )
            except DatabaseError as e:
                raise DatabaseProxyException(
                    "MySQL error adding dataflow tag " + tag + ": " + format(e)
                )
        curs.close()

    def getTags(self, dimObjt, dimName):
        return self.responseList(
            "call getTags(:obj,:name)",
            {"obj": dimObjt, "name": dimName},
            "MySQL error getting tags",
        )

    def getDataFlowTags(self, dfName, fromType, fromName, toType, toName, envName):
        return self.responseList(
            "call getDataFlowTags(:dfName,:fromType,:fromName,:toType,:toName,:envName)",
            {
                "dfName": dfName,
                "fromType": fromType,
                "fromName": fromName,
            ...
            "MySQL error getting data flow tags",
        )
    ...
    def componentView(self, cvName):
        interfaces = self.responseList(
            "call componentViewInterfaces(:cv)",
            {"cv": cvName},
            "MySQL error getting component view interfaces",
        )
    ...
    def componentViewConnectors(self, cvName):
        return self.responseList(
            "call componentViewConnectors(:cv)",
            {"cv": cvName},
            "MySQL error getting component view connectors",
        )

    def addComponentToView(self, cId, cvId):
        self.updateDatabase(
            "call addComponentToView(:cId,:cvId)",
            {"cId": cId, "cvId": cvId},
            "MySQL error adding component to view",
        )
    ...
    def addComponent(self, parameters, cvId=-1):
        ...
        session = self.updateDatabase(
            "call addComponent(:id,:name,:desc)",
            {"id": componentId, "name": componentName, "desc": componentDesc},
            "MySQL error adding component",
            None,
            False,
        )
        if cvId != -1:
            self.updateDatabase(
                "call addComponentToView(:compId,:cvId)",
                {"compId": componentId, "cvId": cvId},
                "MySQL error adding component to view",
                session,
                False,
            )
    ...
    def updateComponent(self, parameters, cvId=-1):
        ...
        session = self.updateDatabase(
            "call deleteComponentComponents(:comp)",
            {"comp": componentId},
            "MySQL error deleting component components",
            None,
            False,
        )
        if componentId != -1:
            self.updateDatabase(
                "call updateComponent(:id,:name,:desc)",
                {"id": componentId, "name": componentName, "desc": componentDesc},
                "MySQL error updating component",
                session,
            )
        else:
            componentId = self.newId()
    ...
    def addComponentInterface(self, componentId, ifName, ifType, arName, pName):
        self.updateDatabase(
            "call addComponentInterface(:compId,:ifName,:ifType,:arName,:pName)",
            {
                "compId": componentId,
                "ifName": ifName,
                "ifType": ifType,
            ...
            "MySQL error adding component interface",
        )
    ...
    def addConnector(self, parameters):
        ...
        self.updateDatabase(
            "call addConnector(:connId,:cvName,:cName,:fName,:fRole,:fIf,:tName,:tIf,:tRole,:conAsset,:pName,:arName)",
            {
                "connId": connId,
                "cvName": cvName,
                "cName": cName,
            ...
            "MySQL error adding connector",
        )

    def getInterfaces(self, dimObjt, dimName):
        rows = self.responseList(
            "call getInterfaces(:obj,:name)",
            {"obj": dimObjt, "name": dimName},
            "MySQL error getting interfaces",
        )
        ifs = []
    ...
    def addInterfaces(self, dimObjt, dimName, ifs):
        try:
            ...
            exceptionText = (
                "MySQL error adding interfaces to "
                + dimName
                + " "
                + dimObjt
                + " (message:"
    ...
    def deleteInterfaces(self, ifName, ifDim):
        self.updateDatabase(
            "call deleteInterfaces(:name,:dim)",
            {"name": ifName, "dim": ifDim},
            "MySQL error deleting interfaces",
        )

    def addInterface(self, ifObjt, ifName, ifType, arName, pName, ifDim):
        self.updateDatabase(
            "call addInterface(:ifObj,:ifName,:ifType,:arName,:pName,:ifDim)",
            {
                "ifObj": ifObjt,
                "ifName": ifName,
                "ifType": ifType,
    ...
    def addComponentStructure(self, componentId, componentStructure):
        for (
            ...
            tailAdornment,
            tailAsset,
        ) in componentStructure:
            self.addComponentAssetAssociation(
                componentId,
                headAsset,
                headAdornment,
                headNav,
    ...
    def addComponentAssetAssociation(
        self,
        componentId,
        headAsset,
        headAdornment,
        ...
        assocId = self.newId()
        self.updateDatabase(
            "call addComponentStructure(:aId,:cId,:hAss,:hAd,:hNav,:hNry,:hRole,:tRole,:tNry,:tNav,:tAd,:tAss)",
            {
                "aId": assocId,
                "cId": componentId,
                "hAss": headAsset,
            ...
            "MySQL error adding component asset association",
        )

    def componentStructure(self, componentId):
        return self.responseList(
            "call getComponentStructure(:comp)",
            {"comp": componentId},
            "MySQL error getting structure for component",
        )
    ...
    def addComponentRequirement(self, reqLabel, componentId, reqName):
        self.updateDatabase(
            "call addComponentRequirement(:reqLbl,:comp,:req)",
            {"reqLbl": reqLabel, "comp": componentId, "req": reqName},
            "MySQL error adding component requirement",
        )

    def getComponentViews(self, constraintId=-1):
        cvRows = self.responseList(
            "call getComponentView(:cons)",
            {"cons": constraintId},
            "MySQL error getting component view",
        )
        cvs = {}
        for cvId, cvName, cvSyn in cvRows:
            ...
            for componentId, componentName, componentDesc in viewComponents:
                ...
                comParameters = ComponentParameters(
                    componentName,
                    componentDesc,
                    componentInterfaces,
                    componentStructure,
            ...
            parameters = ComponentViewParameters(
                cvName, cvSyn, [], [], [], [], [], components, connectors, asm
            )
    ...
    def componentRequirements(self, componentId):
        return self.responseList(
            "call getComponentRequirements(:comp)",
            {"comp": componentId},
            "MySQL error getting component requirements",
        )

    def componentInterfaces(self, componentId):
        rows = self.responseList(
            "call componentInterfaces(:comp)",
            {"comp": componentId},
            "MySQL error getting component interfaces",
        )
        ifs = []
    ...
    def addComponentView(self, parameters):
        ...
        self.updateDatabase(
            "call addComponentView(:id,:name,:syn)",
            {"id": cvId, "name": cvName, "syn": cvSyn},
            "MySQL error adding component view",
        )
    ...
    def updateComponentView(self, parameters):
        ...
        session = self.updateDatabase(
            "call deleteComponentViewComponents(:id)",
            {"id": cvId},
            "MySQL error deleting component view components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateComponentView(:id,:name,:syn)",
            {"id": cvId, "name": cvName, "syn": cvSyn},
            "MySQL error updating component view",
            session,
        )
        for taParameters in cvAssets:
    ...
    def componentViewComponents(self, cvId):
        return self.responseList(
            "call getComponents(:id)", {"id": cvId}, "MySQL error getting components"
        )

    def componentViewWeaknesses(self, cvName, envName):
        rows = self.responseList(
            "call componentViewWeaknesses(:cv,:env)",
            {"cv": cvName, "env": envName},
            "MySQL error getting component view weaknesses",
        )
        thrDict = {}
        vulDict = {}
    ...
    def componentAssets(self, cvName, reqName=""):
        return self.responseList(
            "call componentAssets(:cv,:req)",
            {"cv": cvName, "req": reqName},
            "MySQL error getting component assets",
        )

    def componentGoalAssets(self, cvName, goalName=""):
        return self.responseList(
            "call componentGoalAssets(:cv,:goal)",
            {"cv": cvName, "goal": goalName},
            "MySQL error getting component goal assets",
        )
    ...
    def existingObject(self, objtName, dimName):
        ...
        return self.responseList(
            callTxt, argDict, "MySQL error checking existence of object"
        )[0]

    def situateComponentView(
        self, cvName, envName, acDict, assetParametersList, targets, obstructParameters
    ):
        for assetParameters in assetParametersList:
    ...
    def situateComponentAsset(self, componentName, assetId):
        self.updateDatabase(
            "call situateComponentAsset(:ass,:comp)",
            {"ass": assetId, "comp": componentName},
            "MySQL error situating component asset",
        )
    ...
    def addComponentViewTargets(self, target, envName):
        ...
        for componentName in target.components():
            self.updateDatabase(
                "call addComponentTarget(:comp,:asset,:name,:effectiveness,:rationale,:env)",
                {
                    "comp": componentName,
                    "asset": target.asset(),
                    "name": target.name(),
                ...
                "MySQL error adding component target",
                session,
                False,
            )
        self.commitDatabase(session)
    ...
    def assetComponents(self, assetName, envName):
        return self.responseList(
            "call assetComponents(:ass,:env)",
            {"ass": assetName, "env": envName},
            "MySQL error getting asset components",
        )
    ...
    def addTemplateRequirement(self, parameters):
        ...
        self.updateDatabase(
            "call addTemplateRequirement(:id,:name,:asset,:type,:desc,:rat,:fc)",
            {
                "id": reqId,
                "name": reqName,
                "asset": reqAsset,
            ...
            "MySQL error adding template requirement",
        )
        return reqId
    ...
    def updateTemplateRequirement(self, parameters):
        ...
        reqFC = parameters.fitCriterion()
        self.updateDatabase(
            "call updateTemplateRequirement(:id,:name,:asset,:type,:desc,:rat,:fc)",
            {
                "id": reqId,
                "name": reqName,
                "asset": reqAsset,
            ...
            "MySQL error updating template requirement",
        )

    def getTemplateRequirements(self, constraintId=-1):
        rows = self.responseList(
            "call getTemplateRequirements(:const)",
            {"const": constraintId},
            "MySQL error getting template requirements",
        )
        templateReqs = {}
        for reqId, reqName, assetName, reqType, reqDesc, reqRat, reqFC in rows:
            parameters = TemplateRequirementParameters(
                reqName, assetName, reqType, reqDesc, reqRat, reqFC
            )
    ...
    def componentViewRequirements(self, cvName):
        return self.responseList(
            "call componentViewRequirements(:cv)",
            {"cv": cvName},
            "MySQL error getting component view requirements",
        )
    ...
    def situateComponentViewRequirements(self, cvName):
        self.updateDatabase(
            "call situateComponentViewRequirements(:cv)",
            {"cv": cvName},
            "MySQL error situating component view requirements",
        )

    def getComponents(self, constraintId=-1):
        componentRows = self.responseList(
            "call getAllComponents(:const)",
            {"const": constraintId},
            "MySQL error getting components",
        )
        components = {}
    ...
    def personasImpact(self, cvName, envName):
        rows = self.responseList(
            "call personasImpact(:cv,:env)",
            {"cv": cvName, "env": envName},
            "MySQL error getting personas impact",
        )
        pImpact = []
        for c1, c2 in rows:
    ...
    def taskUseCases(self, taskName):
        return self.responseList(
            "call taskUseCases(:task)",
            {"task": taskName},
            "MySQL error getting task use cases",
        )

    def usecaseComponents(self, ucName):
        return self.responseList(
            "call usecaseComponents(:useCase)",
            {"useCase": ucName},
            "MySQL error getting use case components",
        )

    def attackSurfaceMetric(self, cvName):
        return self.responseList(
            "call attackSurfaceMetric(:cv)",
            {"cv": cvName},
            "MySQL error getting attack surface metrics",
        )[0]

    def componentAssetModel(self, componentName):
        rows = self.responseList(
            "call componentClassModel(:comp)",
            {"comp": componentName},
            "MySQL error getting component asset model",
        )
        associations = {}
        for (
            headName,
    ...
    def getInternalDocuments(self, constraintId=-1):
        rows = self.responseList(
            "call getInternalDocuments(:id)",
            {"id": constraintId},
            "MySQL error getting internal documents",
        )
        idObjts = {}
        for docId, docName, docDesc, docContent in rows:
            docCodes = self.documentCodes(docName)
            docMemos = self.documentMemos(docName)
            parameters = InternalDocumentParameters(
                docName, docDesc, docContent, docCodes, docMemos
            )
    ...
    def addInternalDocument(self, parameters):
        ...
        self.updateDatabase(
            "call addInternalDocument(:id,:name,:desc,:cont)",
            {"id": docId, "name": docName, "desc": docDesc, "cont": docContent},
            "MySQL error adding internal document",
        )
    ...
    def updateInternalDocument(self, parameters):
        ...
        docMemos = parameters.memos()
        session = self.updateDatabase(
            "call deleteInternalDocumentComponents(:id)",
            {"id": docId},
            "MySQL error deleting internal document components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateInternalDocument(:id,:name,:desc,:cont)",
            {"id": docId, "name": docName, "desc": docDesc, "cont": docContent},
            "MySQL error updating internal document",
            session,
        )
        self.addDocumentCodes(docName, docCodes)
        self.addDocumentMemos(docName, docMemos)
    ...
    def getCodes(self, constraintId=-1):
        rows = self.responseList(
            "call getCodes(:const)", {"const": constraintId}, "MySQL error getting code"
        )
        cObjts = {}
        for codeId, codeName, codeType, codeDesc, incCriteria, codeEg in rows:
            parameters = CodeParameters(
                codeName, codeType, codeDesc, incCriteria, codeEg
            )
    ...
    def addCode(self, parameters):
        ...
        self.updateDatabase(
            "call addCode(:id,:name,:type,:desc,:crit,:eg)",
            {
                "id": codeId,
                "name": codeName,
                "type": codeType,
    ...
    def updateCode(self, parameters):
        ...
        codeEg = parameters.example()
        self.updateDatabase(
            "call updateCode(:id,:name,:type,:desc,:crit,:eg)",
            {
                "id": codeId,
                "name": codeName,
                "type": codeType,
            ...
            "MySQL error updating code",
        )

    def documentCodes(self, docName):
        rows = self.responseList(
            "call documentCodes(:name)",
            {"name": docName},
            "MySQL error getting document code",
        )
        codes = {}
    ...
    def addDocumentCode(
        self, docName, docCode, startIdx, endIdx, codeLabel="", codeSynopsis=""
    ):
        self.updateDatabase(
            "call addDocumentCode(:name,:code,:sIdx,:eIdx,:lbl,:syn)",
            {
                "name": docName,
                "code": docCode,
                "sIdx": startIdx,
            ...
            "MySQL error adding document code",
        )

    def artifactCodes(self, artName, artType, sectName):
        rows = self.responseList(
            "call artifactCodes(:art,:type,:sect)",
            {"art": artName, "type": artType, "sect": sectName},
            "MySQL error getting artifact codes",
        )
        codes = {}
        for codeName, startIdx, endIdx in rows:
            startIdx = int(startIdx)
    ...
    def addPersonaEnvironmentCodes(self, pName, envName, codes):
        if len(codes) > 0:
            for sectName in ["narrative"]:
                self.addArtifactEnvironmentCodes(
                    pName, envName, "persona", sectName, codes[sectName]
                )
    ...
    def addTaskEnvironmentCodes(self, tName, envName, codes):
        if len(codes) > 0:
            for sectName in ["narrative", "benefits", "consequences"]:
                self.addArtifactEnvironmentCodes(
                    tName, envName, "task", sectName, codes[sectName]
                )
    ...
    def addArtifactCode(self, artName, artType, sectName, docCode, startIdx, endIdx):
        self.updateDatabase(
            "call addArtifactCode(:art,:type,:sect,:code,:sIdx,:eIdx)",
            {
                "art": artName,
                "type": artType,
                "sect": sectName,
            ...
            "MySQL error adding artifact code",
        )
    ...
    def personaEnvironmentCodes(self, pName, envName):
        codeBook = {}
        for sectName in ["narrative"]:
            codeBook[sectName] = self.artifactEnvironmentCodes(
                pName, envName, "persona", sectName
            )
        return codeBook
    ...
    def taskEnvironmentCodes(self, tName, envName):
        ...
        for sectName in ["narrative", "benefits", "consequences"]:
            codeBook[sectName] = self.artifactEnvironmentCodes(
                tName, envName, "task", sectName
            )
        return codeBook

    def artifactEnvironmentCodes(self, artName, envName, artType, sectName):
        rows = self.responseList(
            "call artifactEnvironmentCodes(:art,:env,:type,:sect)",
            {"art": artName, "env": envName, "type": artType, "sect": sectName},
            "MySQL error getting artifact environment codes",
        )
        codes = {}
        for codeName, startIdx, endIdx in rows:
            startIdx = int(startIdx)
    ...
    def addArtifactEnvironmentCodes(
        self, artName, envName, artType, sectName, docCodes
    ):
        for startIdx, endIdx in docCodes:
            docCode = docCodes[(startIdx, endIdx)]
            self.addArtifactEnvironmentCode(
                artName, envName, artType, sectName, docCode, startIdx, endIdx
            )

    def addArtifactEnvironmentCode(
        self, artName, envName, artType, sectName, docCode, startIdx, endIdx
    ):
        self.updateDatabase(
            "call addArtifactEnvironmentCode(:art,:env,:type,:sect,:code,:sIdx,:eIdx)",
            {
                "art": artName,
                "env": envName,
                "type": artType,
            ...
            "MySQL error adding artifact environment code",
        )

    def personaCodeNetwork(self, personaName, fromCode="", toCode=""):
        return self.responseList(
            "call artifactCodeNetwork(:pers,:a,:fCode,:tCode)",
            {"pers": personaName, "a": "persona", "fCode": fromCode, "tCode": toCode},
            "MySQL error getting persona code network",
        )

    def addCodeRelationship(self, personaName, fromName, toName, rshipType):
        self.updateDatabase(
            "call addArtifactCodeNetwork(:pers,:a,:fName,:tName,:type)",
            {
                "pers": personaName,
                "a": "persona",
                "fName": fromName,
            ...
            "MySQL error adding code relationship",
        )

    def updateCodeNetwork(self, personaName, rships):
        self.updateDatabase(
            "call deleteArtifactCodeNetwork(:pers,:a)",
            {"pers": personaName, "a": "persona"},
            "MySQL error deleting artifact code network",
        )
    ...
    def impliedProcessNetwork(self, ipName):
        return self.responseList(
            "call impliedProcessNetwork(:name)",
            {"name": ipName},
            "MySQL error getting implied process network",
        )
    ...
    def addImpliedProcess(self, parameters):
        ...
        self.updateDatabase(
            "call addImpliedProcess(:id,:name,:desc,:proc,:spec)",
            {"id": ipId, "name": ipName, "desc": ipDesc, "proc": pName, "spec": ipSpec},
            "MySQL error adding implied process",
        )
    ...
    def addImpliedProcessNetwork(self, ipId, personaName, cNet):
        for fromName, fromType, toName, toType, rType in cNet:
            self.addImpliedProcessNetworkRelationship(
                ipId, personaName, fromName, toName, rType
            )

    def addImpliedProcessNetworkRelationship(
        self, ipId, personaName, fromName, toName, rType
    ):
        self.updateDatabase(
            "call addImpliedProcessNetworkRelationship(:id,:pers,:fName,:tName,:type)",
            {
                "id": ipId,
                "pers": personaName,
                "fName": fromName,
            ...
            "MySQL error adding implied process network relationship",
        )
    ...
    def addStepSynopsis(self, ucName, envName, stepNo, synName, aType, aName):
        self.updateDatabase(
            "call addStepSynopsis(:uc,:env,:step,:syn,:aName,:aType)",
            {
                "uc": ucName,
                "env": envName,
                "step": stepNo,
            ...
            "MySQL error adding step synopsis",
        )

    def directoryEntry(self, objtName, dType):
        return self.responseList(
            "call directoryEntry(:obj,:dir)",
            {"obj": objtName, "dir": dType},
            "MySQL error getting directory entry",
        )[0]

    def getTemplateGoals(self, constraintId=-1):
        tgRows = self.responseList(
            "call getTemplateGoals(:const)",
            {"const": constraintId},
            "MySQL error getting template goals",
        )
        templateGoals = {}
        for tgId, tgName, tgDef, tgRat in tgRows:
            ...
            parameters = TemplateGoalParameters(
                tgName, tgDef, tgRat, tgConcerns, tgResps
            )
    ...
    def situateComponentViewGoals(self, cvName, envName):
        self.updateDatabase(
            "call situateComponentViewGoals(:cv,:env)",
            {"cv": cvName, "env": envName},
            "MySQL error situating component view goals",
        )

    def situateComponentViewGoalAssociations(self, cvName, envName):
        self.updateDatabase(
            "call situateComponentViewGoalAssociations(:cv,:env)",
            {"cv": cvName, "env": envName},
            "MySQL error situating component view goal associations",
        )

    def templateGoalConcerns(self, tgId):
        return self.responseList(
            "call templateGoalConcerns(:tg)",
            {"tg": tgId},
            "MySQL error getting template goal concerns",
        )
    ...
    def addTemplateGoal(self, parameters):
        ...
        self.updateDatabase(
            "call addTemplateGoal(:id,:name,:def,:rat)",
            {"id": goalId, "name": goalName, "def": goalDef, "rat": goalRat},
            "MySQL error adding template goal",
        )
    ...
    def updateTemplateGoal(self, parameters):
        ...
        goalResponsibilities = parameters.responsibilities()
        session = self.updateDatabase(
            "call deleteTemplateGoalComponents(:id)",
            {"id": goalId},
            "MySQL error deleting template goal components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateTemplateGoal(:id,:name,:def,:rat)",
            {"id": goalId, "name": goalName, "def": goalDef, "rat": goalRat},
            "MySQL error updating template goals",
            session,
        )
        self.addTemplateGoalConcerns(goalId, goalConcerns)
        self.addTemplateGoalResponsibilities(goalId, goalResponsibilities)
    ...
    def addTemplateGoalConcern(self, goalId, concern):
        self.updateDatabase(
            "call add_template_goal_concern(:id,:con)",
            {"id": goalId, "con": concern},
            "MySQL error adding template goal concern",
        )

    def componentGoals(self, componentId):
        return self.responseList(
            "call getComponentGoals(:comp)",
            {"comp": componentId},
            "MySQL error getting component goals",
        )
    ...
    def addComponentGoal(self, componentId, goalName):
        self.updateDatabase(
            "call addComponentGoal(:comp,:goal)",
            {"comp": componentId, "goal": goalName},
            "MySQL error adding component goal",
        )

    def addComponentAssociations(self, componentId, assocs):
        for idx, assoc in enumerate(assocs):
            self.addComponentGoalAssociation(
                componentId, assoc[0], assoc[1], assoc[2], assoc[3]
            )

    def addComponentGoalAssociation(
        self, componentId, goalName, subGoalName, refType, rationale
    ):
        self.updateDatabase(
            "call addComponentGoalAssociation(:comp,:goal,:sGoal,:ref,:rationale)",
            {
                "comp": componentId,
                "goal": goalName,
                "sGoal": subGoalName,
            ...
            "MySQL error adding component goal association",
        )

    def componentGoalAssociations(self, componentId):
        return self.responseList(
            "call componentGoalAssociations(:comp)",
            {"comp": componentId},
            "MySQL error getting component goal associations",
        )

    def componentAttackSurface(self, cName):
        return self.responseList(
            "call componentAttackSurfaceMetric(:comp)",
            {"comp": cName},
            "MySQL error getting component attack surface metric",
        )[0]

    def componentGoalModel(self, componentName):
        rows = self.responseList(
            "call componentGoalModel(:comp)",
            {"comp": componentName},
            "MySQL error getting component goal model",
        )
        associations = {}
        for (
            associationId,
    ...
    def addTemplateGoalResponsibility(self, goalId, resp):
        self.updateDatabase(
            "call add_template_goal_responsibility(:goal,:resp)",
            {"goal": goalId, "resp": resp},
            "MySQL error adding template goal responsibility",
        )

    def templateGoalResponsibilities(self, tgId):
        return self.responseList(
            "call templateGoalResponsibilities(:tg)",
            {"tg": tgId},
            "MySQL error getting template goal responsibilities",
        )

    def importTemplateAsset(self, taName, environmentName):
        self.updateDatabase(
            "call importTemplateAssetIntoEnvironment(:ta,:env)",
            {"ta": taName, "env": environmentName},
            "MySQL error importing template asset",
        )

    def candidateGoalObstacles(self, cvName, envName):
        return self.responseList(
            "call candidateGoalObstacles(:cv,:env)",
            {"cv": cvName, "env": envName},
            "MySQL error getting candidate goal obstacles",
        )

    def templateGoalDefinition(self, tgId):
        return self.responseList(
            "select definition from template_goal where id =:tg",
            {"tg": tgId},
            "MySQL error getting template goal definition",
        )[0]

    def redmineArchitectureSummary(self, envName):
        return self.responseList(
            "call redmineArchitectureSummary(:env)",
            {"env": envName},
            "MySQL error getting redmine architecture summary",
        )

    def redmineAttackPatternsSummary(self, envName):
        return self.responseList(
            "call redmineAttackPatternsSummary(:env)",
            {"env": envName},
            "MySQL error getting redmine attack patterns summary",
        )[0]

    def processesToXml(self, includeHeader=True):
        return self.responseList(
            "call processesToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting processes to XML",
        )[0]
    ...
    def addQuotation(self, quotation):
        ...
        if artType == "internal_document":
            if qType == "memo":
                ...
                self.addDocumentCode(
                    artName, cmName, startIdx, endIdx, codeLabel, codeSynopsis
                )
        else:
            if envName == "None":
                self.addArtifactCode(
                    artName, artType, sectName, cmName, startIdx, endIdx
                )
            else:
                self.addArtifactEnvironmentCode(
                    artName, envName, artType, sectName, cmName, startIdx, endIdx
                )
    ...
    def addMemo(self, parameters):
        ...
        self.updateDatabase(
            "call addMemo(:id,:name,:desc)",
            {"id": memoId, "name": memoName, "desc": memoDesc},
            "MySQL error adding memo",
        )
        return memoId
    ...
    def updateMemo(self, parameters):
        ...
        memoDesc = parameters.description()
        self.updateDatabase(
            "call updateMemo(:id,:name,:desc)",
            {"id": memoId, "name": memoName, "desc": memoDesc},
            "MySQL error updating memo",
        )

    def documentMemos(self, docName):
        rows = self.responseList(
            "call documentMemos(:doc)",
            {"doc": docName},
            "MySQL error getting document memo",
        )
        memos = {}
    ...
    def addDocumentMemo(self, docName, memoName, memoTxt, startIdx, endIdx):
        self.updateDatabase(
            "call addDocumentMemo(:doc,:mem,:txt,:sIdx,:eIdx)",
            {
                "doc": docName,
                "mem": memoName,
                "txt": memoTxt,
            ...
            "MySQL error adding document memo",
        )

    def impliedProcess(self, procName):
        return self.responseList(
            "call impliedProcess(:proc)",
            {"proc": procName},
            "MySQL error getting implied process",
        )[0]
    ...
    def addImpliedProcessChannel(self, ipId, channelName, dataType):
        self.updateDatabase(
            "call addImpliedProcessChannel(:id,:chan,:type)",
            {"id": ipId, "chan": channelName, "type": dataType},
            "MySQL error adding implied process channel",
        )

    def getQuotations(self):
        return self.responseList(
            "call getQuotations()", {}, "MySQL error getting quotations"
        )
    ...
    def updateQuotation(
        ...
        if atName == "internal_document":
            self.updateDatabase(
                "call updateDocumentCode(:aName,:code,:oSIdx,:oEIdx,:sIdx,:eIdx,:syn,:lbl)",
                {
                    "aName": aName,
                    "code": codeName,
                    "oSIdx": oldStartIdx,
                ...
                "MySQL error updating quotation",
            )

    def deleteQuotation(self, codeName, atName, aName, startIdx, endIdx):
        if atName == "internal_document":
            self.updateDatabase(
                "call deleteDocumentCode(:aName,:code,:sIdx,:eIdx)",
                {"aName": aName, "code": codeName, "sIdx": startIdx, "eIdx": endIdx},
                "MySQL error deleting quotation",
            )

    def impliedCharacteristic(self, pName, fromCode, toCode, rtName):
        row = self.responseList(
            "call impliedCharacteristic(:pName,:fCode,:tCode,:rt)",
            {"pName": pName, "fCode": fromCode, "tCode": toCode, "rt": rtName},
            "MySQL error getting implied characteristic",
        )[0]
        if len(row) == 0:
    ...
    def addImpliedCharacteristic(self, parameters):
        ...
        self.updateDatabase(
            "call addImpliedCharacteristic(:pName,:fCode,:tCode,:rt,:pChar,:qual,:cType)",
            {
                "pName": pName,
                "fCode": fromCode,
                "tCode": toCode,
            ...
            "MySQL error adding implied characteristic",
        )
    ...
    def updateImpliedCharacteristic(self, parameters):
        ...
        self.updateDatabase(
            "call updateImpliedCharacteristic(:pName,:fCode,:tCode,:rt,:char,:qual,:cType)",
            {
                "pName": pName,
                "fCode": fromCode,
                "tCode": toCode,
            ...
            "MySQL error updating implied characteristic",
        )
        for lblName, rtName in lhsCodes:
    ...
    def updateImpliedCharacteristicIntention(self, charName, intName, intType):
        self.updateDatabase(
            "call updateImpliedCharacteristicIntention(:char,:int,:type)",
            {"char": charName, "int": intName, "type": intType},
            "MySQL error updating implied characteristic intention",
        )

    def addImpliedCharacteristicElement(self, charName, lblName, rtName):
        self.updateDatabase(
            "call addImpliedCharacteristicElement(:char,:lbl,:rt)",
            {"char": charName, "lbl": lblName, "rt": rtName},
            "MySQL error adding implied characteristic element",
        )

    def updateImpliedCharacteristicElement(self, charName, lblName, rtName):
        self.updateDatabase(
            "call updateImpliedCharacteristicElement(:char,:lbl,:rt)",
            {"char": charName, "lbl": lblName, "rt": rtName},
            "MySQL error updating implied characteristic element",
        )

    def codeCount(self, codeName):
        return self.responseList(
            "select codeCount(:code)",
            {"code": codeName},
            "MySQL error getting code count",
        )[0]
    ...
    def addIntention(self, intention):
        ...
        self.updateDatabase(
            "call addIntention(:ref,:rType,:int,:iType)",
            {
                "ref": refName,
                "rType": refType,
                "int": intentionName,
    ...
    def addContribution(self, contribution):
        ...
        self.updateDatabase(
            "call addContribution(:src,:dest,:means,:val)",
            {"src": srcName, "dest": destName, "means": meansEnd, "val": valName},
            "MySQL error adding contribution",
        )

    def impliedCharacteristicIntention(self, synName, pName, fromCode, toCode, rtName):
        return self.responseLis(
            "select impliedCharacteristicIntention(:syn,:pName,:fCode,:tCode,:rt)",
            {
                "syn": synName,
                "pName": pName,
                "fCode": fromCode,
            ...
            "MySQL error getting implied characteristic intention",
        )[0].split("#")

    def impliedCharacteristicElementIntention(self, ciName, elName):
        return self.responseList(
            "select impliedCharacteristicElementIntention(:ci,:el)",
            {"ci": ciName, "el": elName},
            "MySQL error getting implied characteristic element intention",
        )[0].split("#")

    def updateImpliedCharacteristicElementIntention(
        self, ciName, elName, intName, intDim, meName, contName
    ):
        self.updateDatabase(
            "call updateImpliedCharacteristicElementIntention(:ci,:el,:int,:dim,:me,:cont)",
            {
                "ci": ciName,
                "el": elName,
                "int": intName,
            ...
            "MySQL error updating intention for element "
            + elName
            + " for implied characteristic "
            + ciName,
        )

    def deniedGoals(self, codeName):
        return self.responseList(
            "call deniedGoals(:code)",
            {"code": codeName},
            "MySQL error getting denied goals",
        )
    ...
    def addLocations(self, parameters):
        ...
        self.updateDatabase(
            "call addLocations(:id,:name,:diag)",
            {"id": locsId, "name": locsName, "diag": locDiagram},
            "MySQL error adding locations",
        )
    ...
    def addLocation(self, locsId, location):
        ...
        self.updateDatabase(
            "call addLocation(:locsId,:locId,:locName)",
            {"locsId": locsId, "locId": locId, "locName": locName},
            "MySQL error adding location",
        )
    ...
    def addAssetInstance(self, locId, assetInstance):
        ...
        self.updateDatabase(
            "call addAssetInstance(:lId,:iId,:iName,:assName)",
            {
                "lId": locId,
                "iId": instanceId,
                "iName": instanceName,
            ...
            "MySQL error adding asset instance",
        )
    ...
    def addPersonaInstance(self, locId, personaInstance):
        ...
        self.updateDatabase(
            "call addPersonaInstance(:lId,:iId,:iName,:pName)",
            {
                "lId": locId,
                "iId": instanceId,
                "iName": instanceName,
            ...
            "MySQL error adding persona instance",
        )
    ...
    def addLocationLink(self, locsId, link):
        ...
        headLoc = link[1]
        self.updateDatabase(
            "call addLocationLink(:lId,:tLoc,:hLoc)",
            {"lId": locsId, "tLoc": tailLoc, "hLoc": headLoc},
            "MySQL error adding location link",
        )

    def getLocations(self, constraintId=-1):
        locsRows = self.responseList(
            "call getLocations(:const)",
            {"const": constraintId},
            "MySQL error getting locations",
        )
        locationsDict = {}
    ...
    def getLocationNames(self, locsName):
        return self.responseList(
            "call getLocationNames(:locs)",
            {"locs": locsName},
            "MySQL error getting location names",
        )

    def getLocationLinks(self, locsName):
        rows = self.responseList(
            "call getLocationLinks(:locs)",
            {"locs": locsName},
            "MySQL error getting location links",
        )
        linkDict = {}
    ...
    def getAssetInstances(self, locName):
        return self.responseList(
            "call getAssetInstances(:locs)",
            {"locs": locName},
            "MySQL error getting asset instances",
        )

    def getPersonaInstances(self, locName):
        return self.responseList(
            "call getPersonaInstances(:locs)",
            {"locs": locName},
            "MySQL error getting persona instances",
        )
    ...
    def locationsRiskModel(self, locationsName, environmentName):
        traceRows = self.responseList(
            "call locationsRiskModel(:locs,:env)",
            {"locs": locationsName, "env": environmentName},
            "MySQL error getting location risk model",
        )
        traces = []
        for fromObjt, fromName, toObjt, toName in traceRows:
            parameters = DotTraceParameters(fromObjt, fromName, toObjt, toName)
    ...
    def templateAssetMetrics(self, taName):
        return self.responseList(
            "call templateAssetMetrics(:ta)",
            {"ta": taName},
            "MySQL error getting template asset metrics",
        )[0]

    def riskModelElements(self, envName):
        rows = self.responseList(
            "call riskAnalysisModelElements(:env)",
            {"env": envName},
            "MySQL error getting risk analysis model elements",
        )
        elNames = []
        for c0, c1 in rows:
            elNames.append(c1)
    ...
    def assetThreatRiskLevel(self, assetName, threatName, envName):
        return self.responseList(
            "call assetThreatRiskLevel(:ass,:thr,:env)",
            {"ass": assetName, "thr": threatName, "env": envName},
            "MySQL error getting asset threat risk level",
        )[0]

    def assetRiskLevel(self, assetName, envName):
        return self.responseList(
            "call assetRiskLevel(:ass,:env)",
            {"ass": assetName, "env": envName},
            "MySQL error getting asset risk level",
        )[0]

    def dimensionSummary(self, dimName, envName):
        return self.responseList(
            "call " + dimName + "Summary(:name)",
            {"name": envName},
            "MySQL error getting " + dimName + " summary for environment " + envName,
        )
    ...
    def deleteDatabase(self, dbName, session_id):
        ...
        try:
            ...
            stmts = [
                ...
                'delete from cairis_owner.db_owner where db = "'
                + dbName
                + '" and owner = "'
                + dbUser
                + '"',
            ...
            exceptionText = (
                "MySQL error creating CAIRIS database "
                + dbName
                + "(message:"
                + format(e)
                + ")"
    ...
    def getUseCaseRequirements(self, ucName):
        return self.responseList(
            "call useCaseRequirements(:uc)",
            {"uc": ucName},
            "MySQL error getting requirements associated with use case " + ucName,
        )

    def getUseCaseGoals(self, ucName, envName):
        return self.responseList(
            "call useCaseGoals(:uc,:env)",
            {"uc": ucName, "env": envName},
            "MySQL error getting goals associated with use case " + ucName,
        )

    def synopsisId(self, synTxt):
        return self.responseList(
            "select synopsisId(:syn)",
            {"syn": synTxt},
            "MySQL error finding synopsis id for text " + synTxt,
        )[0]
    ...
    def hasContribution(self, contType, rsName, csName):
        ...
        hasRC = self.responseList(
            "select " + sqlTxt + "(:rName,:cName)",
            {"rName": rsName, "cName": csName},
            "MySQL error checking contribution",
        )[0]
        if hasRC == 1:
            return True
        else:
    ...
    def removeUseCaseContributions(self, ucId):
        self.updateDatabase(
            "call removeUseCaseContributions(:id)",
            {"id": ucId},
            "MySQL error removing use case contribution",
        )

    def getDataFlows(
        self, dfName="", fromName="", fromType="", toName="", toType="", envName=""
    ):
        dfRows = self.responseList(
            "call getDataFlows(:df,:fromName,:fromType,:toName,:toType,:env)",
            {
                "df": dfName,
                "fromName": fromName,
                "fromType": fromType,
            ...
            "MySQL error getting data flows",
        )
        dataFlows = []
        for dfName, dfType, envName, fromName, fromType, toName, toType in dfRows:
            tags = self.getDataFlowTags(
                dfName, fromType, fromName, toType, toName, envName
            )
            dfAssets = self.getDataFlowAssets(
                dfName, fromName, fromType, toName, toType, envName
            )
            dfObs = self.getDataFlowObstacles(
                dfName, fromName, fromType, toName, toType, envName
            )
    ...
    def getDataFlowAssets(self, dfName, fromName, fromType, toName, toType, envName):
        return self.responseList(
            "call getDataFlowAssets(:df,:fromName,:fromType,:toName,:toType,:env)",
            {
                "df": dfName,
                "fromName": fromName,
                "fromType": fromType,
            ...
            "MySQL error getting assets for data flow " + dfName,
        )

    def getDataFlowObstacles(self, dfName, fromName, fromType, toName, toType, envName):
        return self.responseList(
            "call getDataFlowObstacles(:df,:fromName,:fromType,:toName,:toType,:env)",
            {
                "df": dfName,
                "fromName": fromName,
                "fromType": fromType,
            ...
            "MySQL error getting obstacles for data flow " + dfName,
        )
    ...
    def addDataFlow(self, parameters):
        ...
        tags = parameters.tags()
        self.updateDatabase(
            "call addDataFlow(:df,:dfType,:env,:fName,:fType,:tName,:tType)",
            {
                "df": dfName,
                "dfType": dfType,
                "env": envName,
            ...
            "MySQL error adding data flow",
        )
        self.addDataFlowTags(dfName, fromType, fromName, toType, toName, envName, tags)
        for dfAsset in dfAssets:
            self.addDataFlowAsset(
                dfName, envName, fromType, fromName, toType, toName, dfAsset
            )
        for dfOb in dfObs:
            self.addDataFlowObstacle(
                dfName, envName, fromType, fromName, toType, toName, dfOb
            )

    def addDataFlowAsset(
        self, dfName, envName, fromType, fromName, toType, toName, dfAsset
    ):
        self.updateDatabase(
            "call addDataFlowAsset(:df,:env,:fromType,:fromName,:toType,:toName,:ass)",
            {
                "df": dfName,
                "env": envName,
                "fromType": fromType,
            ...
            "MySQL error adding data flow asset",
        )

    def addDataFlowObstacle(
        self, dfName, envName, fromType, fromName, toType, toName, dfOb
    ):
        obsName, kwd, dfoContext = dfOb
        self.updateDatabase(
            "call addDataFlowObstacle(:df,:env,:fromType,:fromName,:toType,:toName,:ob,:kwd,:dfoContext)",
            {
                "df": dfName,
                "env": envName,
                "fromType": fromType,
                ...
                "ob": obsName,
                "kwd": kwd,
                "dfoContext": dfoContext,
            },
            "MySQL error adding data flow obstacle",
        )
    ...
    def updateDataFlow(
        ...
        tags = parameters.tags()
        session = self.updateDatabase(
            "call deleteDataFlowAssets(:df,:fromName,:fromType,:toName,:toType,:env)",
            {
                "df": oldDfName,
                "fromName": fromName,
                "fromType": fromType,
            ...
            "MySQL error deleting data flow assets",
            None,
            False,
        )
        self.updateDatabase(
            "call deleteDataFlowObstacles(:df,:fromName,:fromType,:toName,:toType,:env)",
            {
                "df": oldDfName,
                "fromName": fromName,
                "fromType": fromType,
            ...
            "MySQL error deleting data flow obstacles",
            session,
            False,
        )
        self.updateDatabase(
            "call updateDataFlow(:oldDfName,:oldFromName,:oldFromType,:oldToName,:oldToType,:oldEnvName,:dfName,:fromName,:fromType,:toName,:toType,:envName,:dfType)",
            {
                "oldDfName": oldDfName,
                "oldFromName": oldFromName,
                "oldFromType": oldFromType,
                "oldToName": oldToName,
                "oldToType": oldToType,
            ...
            "MySQL error updating data flow",
            session,
        )
        self.addDataFlowTags(dfName, fromType, fromName, toType, toName, envName, tags)
        for dfAsset in dfAssets:
    ...
    def deleteDataFlow(self, dfName, fromName, fromType, toName, toType, envName):
        self.updateDatabase(
            "call deleteDataFlow(:df,:fromName,:fromType,:toName,:toType,:env)",
            {
                "df": dfName,
                "fromName": fromName,
                "fromType": fromType,
            ...
            "MySQL Error deleting data flow",
        )

    def dataFlowDiagram(self, envName, filterType="None", filterElement=""):
        return self.responseList(
            "call dataFlowDiagram(:env,:ft,:fe)",
            {"env": envName, "ft": filterType, "fe": filterElement},
            "MySQL error getting data flow diagram",
        )

    def personalDataFlowDiagram(self, envName, filterElement=""):
        return self.responseList(
            "call personalDataFlowDiagram(:env,:fe)",
            {"env": envName, "fe": filterElement},
            "MySQL error getting personal data flow diagram",
        )

    def relabelRequirements(self, reqReference):
        self.updateDatabase(
            "call relabelRequirements(:reqReference)",
            {"reqReference": reqReference},
            "MySQL error relabelling requirements",
        )

    def getTrustBoundaries(self, constraintId=-1):
        tbRows = self.responseList(
            "call getTrustBoundaries(:id)",
            {"id": constraintId},
            "MySQL error getting trust boundaries",
        )
        tbs = []
        for tbId, tbName, tbType, tbDesc in tbRows:
            ...
            for environmentId, environmentName in self.dimensionEnvironments(
                ...
                components[environmentName] = self.trustBoundaryComponents(
                    tbId, environmentId
                )
                privileges[environmentName] = self.trustBoundaryPrivilege(
                    tbId, environmentId
                )
            tbs.append(
                TrustBoundary(
                    tbId, tbName, tbType, tbDesc, components, privileges, tags
                )
            )
        return tbs
    ...
    def trustBoundaryComponents(self, tbId, envId):
        return self.responseList(
            "call trustBoundaryComponents(:tbId,:envId)",
            {"tbId": tbId, "envId": envId},
            "MySQL error getting trust boundary components for trust boundary id "
            + str(tbId),
        )

    def trustBoundaryPrivilege(self, tbId, envId):
        return self.responseList(
            "select trustBoundaryPrivilege(:tbId,:envId)",
            {"tbId": tbId, "envId": envId},
            "MySQL error getting the trust boundary privilege level for trust boundary id "
            + str(tbId),
        )[0]
    ...
    def addTrustBoundary(self, tb):
        tbId = self.newId()
        self.updateDatabase(
            "call addTrustBoundary(:id,:name,:type,:desc)",
            {
                "id": tbId,
                "name": tb.name(),
                "type": tb.type(),
            ...
            "MySQL error adding trust boundary " + str(tbId),
        )
        ...
        for environmentName in list(tbPrivilegeLevels.keys()):
            self.addTrustBoundaryPrivilege(
                tbId,
                environmentName,
                self.privilegeValue(tbPrivilegeLevels[environmentName]),
            )

    def privilegeValue(self, pName):
        return self.responseList(
            "select privilegeValue(:pName)",
            {"pName": pName},
            "MySQL error getting privilege value",
        )[0]

    def addTrustBoundaryComponent(self, tbId, envName, tbComponent):
        self.updateDatabase(
            "call addTrustBoundaryComponent(:id,:environment,:component)",
            {"id": tbId, "environment": envName, "component": tbComponent},
            "MySQL error adding trust boundary component "
            + tbComponent
            + " to trust boundary id "
            + str(tbId),
        )

    def addTrustBoundaryPrivilege(self, tbId, envName, pValue):
        self.updateDatabase(
            "call addTrustBoundaryPrivilege(:id,:environment,:pValue)",
            {"id": tbId, "environment": envName, "pValue": pValue},
            "MySQL error adding trust boundary privilege "
            + str(pValue)
            + " to trust boundary id "
            + str(tbId),
        )
    ...
    def updateTrustBoundary(self, tb):
        self.updateDatabase(
            "call deleteTrustBoundaryComponents(:id)",
            {"id": tb.id()},
            "MySQL error deleting trust boundary components for " + tb.name(),
        )
        self.updateDatabase(
            "call updateTrustBoundary(:id,:name,:type,:desc)",
            {
                "id": tb.id(),
                "name": tb.name(),
                "type": tb.type(),
            ...
            "MySQL error adding trust boundary " + tb.name(),
        )
        self.addTags(tb.name(), "trust_boundary", tb.tags())
        defaultPrivilegeLevels = {}
        for environmentName in list(tb.components().keys()):
    ...
    def threatenedEntities(self, envName):
        return self.responseList(
            "call threatenedEntities(:envName)",
            {"envName": envName},
            "MySQL error getting threatened entities",
        )

    def threatenedProcesses(self, envName):
        return self.responseList(
            "call threatenedProcesses(:envName)",
            {"envName": envName},
            "MySQL error getting threatened processes",
        )

    def threatenedDatastores(self, envName):
        return self.responseList(
            "call threatenedDatastores(:envName)",
            {"envName": envName},
            "MySQL error getting threatened datastores",
        )

    def threatenedDataflows(self, envName):
        return self.responseList(
            "call threatenedDataflows(:envName)",
            {"envName": envName},
            "MySQL error getting threatened dataflows",
        )

    def defaultValue(self, valueType):
        return self.responseList(
            "call defaultValue(:valueType)",
            {"valueType": valueType},
            "MySQL error getting default value for " + valueType,
        )[0]

    def cairisVersion(self):
        return self.responseList(
            "select cairisVersion()", {}, "MySQL error getting CAIRIS version"
        )[0]

    def modelValidation(self, envName):
        objtRows = self.responseList(
            "call modelValidation(:envName)",
            {"envName": envName},
            "MySQL error validating model",
        )
        objtRows += self.responseList(
            "call taintFlowAnalysis(:envName)",
            {"envName": envName},
            "MySQL error validating model",
        )
        rows = []
    ...
    def processDataMaps(self):
        return self.responseList(
            "call processDataMaps()", {}, "MySQL error getting process data maps"
        )[0]

    def datastoreDataMaps(self):
        return self.responseList(
            "call datastoreDataMaps()", {}, "MySQL error getting datastore data maps"
        )[0]

    def lawfulProcessingTable(self, envName):
        return self.responseList(
            "call lawfulProcessingTable(:envName)",
            {"envName": envName},
            "MySQL error getting lawful processing table",
        )

    def reassociateRequirement(self, reqName, domName):
        self.updateDatabase(
            "call reassociateRequirement(:reqName,:domName)",
            {"reqName": reqName, "domName": domName},
            "MySQL error associating requirement "
            + reqName
            + " with domain "
            + domName,
        )
    ...
    def getDependency(self, envName, dependerName, dependeeName, dependencyName):
        deps = self.responseList(
            "call getDependency(:env,:dep,:dee,:dpy)",
            {
                "env": envName,
                "dep": dependerName,
                "dee": dependeeName,
            ...
            "MySQL error getting dependency",
        )
        if len(deps) == 0:
        ...
        parameters = DependencyParameters(
            deps[0][1], deps[0][2], deps[0][3], deps[0][4], deps[0][5], deps[0][6]
        )
        return ObjectFactory.build(deps[0][0], parameters)

    def assetAttackSurface(self, assetName, envName):
        return self.responseList(
            "select assetAttackSurface(:asset,:environment)",
            {"asset": assetName, "environment": envName},
            "MySQL error getting asset attack surface",
        )[0]

    def setImage(self, imageName, imageContent, mimeType):
        self.updateDatabase(
            "call setImage(:name,:content,:type)",
            {"name": imageName, "content": b64encode(imageContent), "type": mimeType},
            "MySQL error setting image",
        )

    def getImage(self, imageName):
        imgResponse = self.responseList(
            "call getImage(:name)", {"name": imageName}, "MySQL error getting image"
        )
    ...
    def checkDataFlowExists(self, dfName, fromType, fromName, toType, toName, envName):
        objtCount = self.responseList(
            "call checkDataFlowExists(:dfName,:fromType,:fromName,:toType,:toName,:env)",
            {
                "dfName": dfName,
                "fromType": fromType,
                "fromName": fromName,
            ...
            "MySQL error checking dataflow in environment",
        )[0]
        if objtCount > 0:
            raise ARMException(
                dfName
    ...
    def riskModelTags(self, envName):
        rows = self.responseList(
            "call riskModelTags(:env)",
            {"env": envName},
            "MySQL error getting risk model tags",
        )
        tagDict = {}
        for row in rows:
            tag = row[0]
    ...
    def securityPatternAssetModel(self, spName):
        rows = self.responseList(
            "call securityPatternClassModel(:sp)",
            {"sp": spName},
            "MySQL error getting security pattern asset model",
        )
        associations = {}
        for (
            headName,
    ...
    def dimensionRequirements(self, dimName, objtName):
        return self.responseList(
            "call " + dimName + "RequirementNames(:objt)",
            {"objt": objtName},
            "MySQL error getting requirements associated with "
            + dimName
            + " "
            + objtName,
        )

    def exceptionRootObstacles(self, ucId):
        return self.responseList(
            "call hasRootObstacles(:id)",
            {"id": ucId},
            "MySQL error getting root obstacles resulting from exceptions for use case ",
        )[0]
    ...
    def securityPatternsToXml(self):
        return self.responseList(
            "call patternsToXml()", {}, "MySQL error exporting security patterns to XML"
        )[0]

    def checkAssetAssociation(self, envName, headName, tailName):
        if self.responseList(
            "select isClassAssociationPresent(:env,:head,:tail)",
            {"env": envName, "head": headName, "tail": tailName},
            "MySQL error checking asset association",
        )[0]:
            raise DatabaseProxyException(
                "Association between head asset "
                + headName
                + " and tail asset "
                + tailName
    ...
    def checkTrace(self, fromObjt, fromName, toObjt, toName):
        if self.responseList(
            "call isTracePresent(:fromObjt,:fromName,:toObjt,:toName)",
            {
                "fromObjt": fromObjt,
                "fromName": fromName,
                "toObjt": toObjt,
    ...
    def getUserGoals(self, constraintId=-1):
        rows = self.responseList(
            "call getUserGoals(:id)",
            {"id": constraintId},
            "MySQL error getting user goals",
        )
        objts = []
        for row in rows:
            rsId = row[0]
    ...
    def addUserGoal(self, objt):
        ...
        self.updateDatabase(
            "call addUserGoal(:ugId,:refName,:synName,:dimName,:personaName,:gSat)",
            {
                "ugId": ugId,
                "refName": refName,
                "synName": synName,
    ...
    def updateUserGoal(self, objt):
        ...
        self.updateDatabase(
            "call updateUserGoal(:ugId,:refName,:synName,:dimName,:personaName,:gSat)",
            {
                "ugId": ugId,
                "refName": refName,
                "synName": synName,
            ...
            "MySQL error updating user goal",
            session,
        )
        for sysGoal in objt.goals():
            self.addUserSystemGoalLink(synName, sysGoal)
    ...
    def getGoalContributions(self, envName, personaName="", filterElement=""):
        rows = self.responseList(
            "call getGoalContributions(:envName,:persona,:filter)",
            {"envName": envName, "persona": personaName, "filter": filterElement},
            "MySQL error getting goal contributions",
        )
        return rows

    def goalSatisfactionScore(self, goalName, envName):
        return self.responseList(
            "call calculateGoalScore(:goalName,:envName)",
            {"goalName": goalName, "envName": envName},
            "MySQL error getting goal contribution",
        )[0]

    def getGoalContributionsTable(self, sourceId=-1, targetId=-1):
        rows = self.responseList(
            "call getGoalContributionsTable(:source_id,:target_id)",
            {"source_id": sourceId, "target_id": targetId},
            "MySQL error getting goal contribution table",
        )
        objts = []
        for row in rows:
            objts.append(
                GoalContribution(row[0], row[1], row[2], row[3], row[4], row[5])
            )
        return objts

    def deleteGoalContribution(self, src, tgt):
        self.updateDatabase(
            "call deleteGoalContribution(:src,:tgt)",
            {"src": src, "tgt": tgt},
            "MySQL error deleting goal contribution",
        )

    def taskContributions(self, taskName, envName):
        rows = self.responseList(
            "call getTaskContributions(:taskName,:envName)",
            {"taskName": taskName, "envName": envName},
            "MySQL error getting task contributions",
        )
        objts = []
        for row in rows:
    ...
    def conflictingPersonaCharacteristics(self, pName, ugName):
        objts = self.responseList(
            "call conflictingPersonaCharacteristics(:pName,:ugName)",
            {"pName": pName, "ugName": ugName},
            "MySQL error getting persona characteristic user goals",
        )
    ...
    def addUserSystemGoalLink(self, ugName, sgName):
        self.updateDatabase(
            "call addUserSystemGoalLink(:ugName,:sgName)",
            {"ugName": ugName, "sgName": sgName},
            "MySQL error adding user/system goal link",
        )

    def userGoalSystemGoals(self, ugId):
        return self.responseList(
            "call userGoalSystemGoals(:ugId)",
            {"ugId": ugId},
            "MySQL error getting user goal system goals",
        )

    def controlStructure(self, envName, filterElement=""):
        return self.responseList(
            "call controlStructure(:env,:fe)",
            {"env": envName, "fe": filterElement},
            "MySQL error getting control structure",
        )

    def validateForExport(self):
        return self.responseList(
            "call invalidObjectNames()", {}, "MySQL error getting invalid object names"
        )[0]
    ...
    def addUserStory(self, parameters):
        ...
        usId = self.newId()
        self.updateDatabase(
            "call addUserStory(:id,:name,:auth,:role,:desc,:ug)",
            {
                "id": usId,
                "name": usName,
                "auth": usAuth,
    ...
    def addUserStoryAcceptanceCriteria(self, usId, usAc):
        self.updateDatabase(
            "call addUserStoryAcceptanceCriteria(:usId,:usAc)",
            {
                "usId": usId,
                "usAc": usAc,
            },
            "MySQL error adding acceptance criteria to user story",
        )
    ...
    def updateUserStory(self, parameters):
        ...
        tags = parameters.tags()
        session = self.updateDatabase(
            "call deleteUserStoryComponents(:us)",
            {"us": usId},
            "MySQL error deleting user story components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateUserStory(:id,:name,:auth,:role,:desc,:ug)",
            {
                "id": usId,
                "name": usName,
                "auth": usAuth,
    ...
    def getUserStories(self, constraintId=-1):
        usRows = self.responseList(
            "call getUserStories(:id)",
            {"id": constraintId},
            "MySQL error getting user stories",
        )
        uss = []
        for usId, usName, usAuth, roleName, usDesc, ugName in usRows:
            ...
            uss.append(
                UserStory(usId, usName, usAuth, roleName, usDesc, ugName, ac, tags)
            )
        return uss

    def userStoryAcceptanceCriteria(self, usName):
        return self.responseList(
            "call userStoryAcceptanceCriteria(:usName)",
            {"usName": usName},
            "MySQL error getting user story acceptance criteria",
        )

    def storiesToXml(self, includeHeader=True):
        return self.responseList(
            "call storiesToXml(:head)",
            {"head": includeHeader},
            "MySQL error exporting stories to XML",
        )[0]

    def roleUserGoals(self, roleName):
        return self.responseList(
            "call roleUserGoals(:name)",
            {"name": roleName},
            "MySQL error getting user goals for role " + roleName,
        )

    def goalPolicy(self, goalId, environmentId):
        pData = self.responseList(
            "call goalPolicy(:gId,:eId)",
            {"gId": goalId, "eId": environmentId},
            "MySQL error getting goal policy",
        )
        if len(pData) == 1:
            ps = pData[0]
            return {
                "theGoalName": ps[0],
                "theEnvironmentName": ps[1],
                "theSubject": ps[2],
                "theAccessType": ps[3],
                "theResource": ps[4],
                "thePermission": ps[5],
    ...
    def addGoalPolicy(self, goalId, environmentName, subjName, acName, resName, pName):
        self.updateDatabase(
            "call addGoalPolicy(:gId,:eName,:subj,:acc,:res,:perm)",
            {
                "gId": goalId,
            ...
            "MySQL error adding goal policy",
        )
    ...
    def addPolicyStatement(self, parameters):
        ...
        self.updateDatabase(
            "call addPolicyStatement(:id,:goal,:env,:subj,:at,:res,:perm)",
            {
                "id": psId,
                "goal": goalName,
                "env": envName,
            ...
            "MySQL error adding policy statement",
        )
        return psId
    ...
    def updatePolicyStatement(self, parameters):
        ...
        pName = parameters.permission()
        self.updateDatabase(
            "call updatePolicyStatement(:id,:goal,:env,:subj,:at,:res,:perm)",
            {
                "id": psId,
                "goal": goalName,
                "env": envName,
            ...
            "MySQL error updating policy statement",
        )
    ...
    def getPolicyStatements(self, constraintId=-1):
        psRows = self.responseList(
            "call getPolicyStatements(:id)",
            {"id": constraintId},
            "MySQL error getting policy statements",
        )
        objts = []
        for psId, goalName, envName, subjName, atName, resName, pName in psRows:
            objts.append(
                PolicyStatement(
                    psId, goalName, envName, subjName, atName, resName, pName
                )
            )
        return objts
    ...
    def userGoalFilters(self, envName, personaName):
        return self.responseList(
            "call ugm_filterNames(:environment,:persona)",
            {"environment": envName, "persona": personaName},
            "MySQL error getting user goal filters for persona "
            + personaName
            + " in environment "
            + envName,
        )
