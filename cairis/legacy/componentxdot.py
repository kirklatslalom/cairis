...
class ComponentPolygonShape(xdot.PolygonShape):
    ...
    def draw(self, cr, highlight=False, zoom_ratio=-1):
        if (
            ...
            or (self.dim == "risk")
            or (
                zoom_ratio > LOW_ZOOM_RATIO
                and (
                    self.dim == "asset"
                    or self.dim == "threat"
                    or self.dim == "vulnerability"
                )
            )
            or (zoom_ratio > HIGH_ZOOM_RATIO)
        ):
            if self.dim != "role":
                x0, y0 = self.points[-1]
                cr.move_to(x0, y0)
            ...
            if len(self.points) == 4:
                if (self.dim != "role") and (
                    (zoom_ratio == -1) or (zoom_ratio > HIGH_ZOOM_RATIO)
                ):
    ...
    def decorateAssetNode(self, cr, zoom_ratio):
        if (zoom_ratio == -1) or (zoom_ratio > HIGH_ZOOM_RATIO):
            ...
            syProps = asset.securityProperties(
                self.environment.name(),
                self.environment.duplicateProperty(),
                self.environment.overridingEnvironment(),
            )
            cx = xs
            cy = ys + 10
    ...
    def decorateThreatNode(self, cr, zoom_ratio):
        if (zoom_ratio > LOW_ZOOM_RATIO) and (zoom_ratio < HIGH_ZOOM_RATIO):
            ...
            threat = self.dbProxy.dimensionObject(self.objt, self.dim)
            syProps = threat.securityProperties(
                self.environment.name(),
                self.environment.duplicateProperty(),
                self.environment.overridingEnvironment(),
            )
...
class XDotAttrParser(xdot.XDotAttrParser):
    ...
    def handle_ellipse(self, x0, y0, w, h, filled=False):
        if self.dim == "required_interface":
            if filled:
                self.shapes.append(
                    RequiredInterfaceShape(
                        self.pen, x0, y0, w, h, self.objt, filled=True
                    )
                )
            self.shapes.append(
                RequiredInterfaceShape(self.pen, x0, y0, w, h, self.objt)
            )
            self.shapes.append(
                InterfaceLabelShape(self.pen, x0, y0 + 13, 0, w * 2, self.objt)
            )
            ...
            self.shapes.append(
                ProvidedInterfaceShape(self.pen, x0, y0, w, h, self.objt)
            )
            self.shapes.append(
                InterfaceLabelShape(self.pen, x0, y0 + 13, 0, w * 2, self.objt)
            )
    ...
    def handle_polygon(self, points, filled=False):
        if filled:
            self.shapes.append(
                ComponentPolygonShape(
                    self.pen, points, self.dim, self.objt, filled=True
                )
            )
