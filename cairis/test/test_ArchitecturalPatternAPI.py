...
def addAPDependentData(d):
    ...
    for i in d["roles"]:
        b.dbProxy.addRole(
            RoleParameters(
                i["theName"], i["theType"], i["theShortCode"], i["theDescription"], []
            )
        )
    for i in d["access_rights"]:
        b.dbProxy.addValueType(
            ValueTypeParameters(
                i["theName"],
                i["theDescription"],
                "access_right",
    ...
    for i in d["template_assets"]:
        b.dbProxy.addTemplateAsset(
            TemplateAssetParameters(
                i["theName"],
                i["theShortCode"],
                i["theDescription"],
                i["theSignificance"],
    ...
    for i in d["template_goals"]:
        b.dbProxy.addTemplateGoal(
            TemplateGoalParameters(
                i["theName"],
                i["theDefinition"],
                i["theRationale"],
                i["theConcerns"],
                i["theResponsibilities"],
            )
        )
...
class ArchitecturalPatternAPITests(CairisDaemonTestCase):
    def setUp(self):
        # region Class fields
        importModelFile(
            os.environ["CAIRIS_SRC"] + "/test/webinos_presituate.xml", 1, "test"
        )
    ...
    def test_delete(self):
        importComponentViewFile(
            os.environ["CAIRIS_SRC"] + "/test/ContextPolicyManagement.xml", "test"
        )
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            url, content_type="application/json", data=self.prepare_json(ap)
        )
        self.logger.debug("[%s] Response data: %s", method, rv.data)
        json_resp = jsonpickle.decode(rv.data.decode("utf-8"))
    ...
    def test_put(self):
        ...
        ap = d["architectural_patterns"][0]
        rv = self.app.post(
            "/api/architectural_patterns?session_id=test",
            content_type="application/json",
            data=self.prepare_json(ap),
        )
