...
def importModelFile(
    importFile, url, dbName, createDb, overwrite, mFormat, userName, passWd
):
    session = authenticate(url, userName, passWd)
    data = {"session_id": session}
    if createDb:
        newDbResp = requests.post(
            url + "/api/settings/database/" + quote(dbName) + "/create", data=data
        )
        if not newDbResp.ok:
            exceptionTxt = "Cannot create database " + dbName + ": " + newDbResp.text
            raise Exception(exceptionTxt)

    openDbResp = requests.post(
        url + "/api/settings/database/" + quote(dbName) + "/open", data=data
    )
    ...
    importResp = requests.post(
        url + "/api/import/text", data=json.dumps(import_json), headers=hdrs
    )
...
def main(args=None):
    parser = argparse.ArgumentParser(
        description="Computer Aided Integration of Requirements and Information Security - Model Import using CAIRIS API"
    )
    parser.add_argument("modelFile", help="model file to import")
    parser.add_argument("--url", dest="url", help="URL for CAIRIS server")
    parser.add_argument(
        "--database", dest="dbName", default="cairis_default", help="database name"
    )
    ...
    parser.add_argument(
        "--type",
        dest="modelFormat",
        help="model type to import.  One of securitypattern, attackpattern, tvtypes, directory, requirements, riskanalysis, usability, misusability, project, domainvalues, architecturalpattern, associations, synopses, processes, assets, locations, dataflows or all",
    )
