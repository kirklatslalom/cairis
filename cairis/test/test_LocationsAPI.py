...
class LocationsAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        newLocations.append(
            Location(
                -1, iLoc1Name, iLoc1AssetInstances, iLoc1PersonaInstances, iLoc1Links
            )
        )
        ...
        iLoc2AssetInstances = [
            {
                "theName": iLoc2["theAssetInstances"][0]["theName"],
                "theAsset": iLoc2["theAssetInstances"][0]["theAsset"],
            }
        ]
        iLoc2PersonaInstances = [
            {
                "theName": iLoc2["thePersonaInstances"][0]["theName"],
                "thePersona": iLoc2["thePersonaInstances"][0]["thePersona"],
            }
        ]
        iLoc2Links = iLoc2["theLinks"]
        newLocations.append(
            Location(
                -1, iLoc2Name, iLoc2AssetInstances, iLoc2PersonaInstances, iLoc2Links
            )
        )

        self.new_locs = Locations(
            locsId="-1", locsName=newLocsName, locsDiagram=newLocsDia, locs=newLocations
        )
    ...
    def test_put(self):
        ...
        url = "/api/locations/name/%s?session_id=test" % quote(
            self.new_locs_dict["object"].theName
        )
        rv = self.app.put(
            url,
            content_type="application/json",
    ...
    def test_locations_model(self):
        ...
        url = (
            "/api/locations/model/locations/"
            + quote(self.existing_locs_name)
            + "/environment/Day?session_id=test"
        )
        rv = self.app.get(url)
        self.assertIsNotNone(rv.data, "No response")
        if sys.version_info > (3,):
