...
from cairis.tools.ModelDefinitions import (
    ThreatModel,
    ThreatEnvironmentPropertiesModel,
    ThreatModelPropertyModel,
    ThreatModelElementModel,
    ThreatModelModel,
)
...
class ThreatDAO(CairisDAO):
    ...
    def get_threat_model(self, environment_name, pathValues=[]):
        try:
            tm = ThreatModelModel()
            tm.theEntities = self.buildThreatModelElement(
                self.db_proxy.threatenedEntities(environment_name)
            )
            tm.theProcesses = self.buildThreatModelElement(
                self.db_proxy.threatenedProcesses(environment_name)
            )
            tm.theDatastores = self.buildThreatModelElement(
                self.db_proxy.threatenedDatastores(environment_name)
            )
            tm.theDataflows = self.buildThreatModelElement(
                self.db_proxy.threatenedDataflows(environment_name)
            )
            return tm
        except DatabaseProxyException as ex:
            self.close()
    ...
    def convert_props(self, real_props=None, fake_props=None):
        ...
        if real_props is not None:
            if len(real_props) > 0:
                ...
                for real_prop in real_props:
                    ...
                    for idx in range(0, len(real_prop.theProperties)):
                        ...
                        new_attr = SecurityAttribute(
                            attr_name, attr_value, attr_rationale
                        )
            ...
            if len(fake_props) > 0:
                check_required_keys(
                    fake_props[0], ThreatEnvironmentPropertiesModel.required
                )
