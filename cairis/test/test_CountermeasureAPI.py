...
class CountermeasureAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.existing_targets = [
            CountermeasureTarget(
                "Certificate Ubiquity", "High", "Discourages certificate sharing"
            )
        ]
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            countermeasures, list, "The result is not a list as expected"
        )
        self.assertGreater(
            len(countermeasures), 0, "No countermeasures in the dictionary"
        )
        self.logger.info("[%s] Countermeasures found: %d", method, len(countermeasures))
        countermeasure = countermeasures[0]
        self.logger.info(
            "[%s] First countermeasure: %s\n", method, countermeasure["theName"]
        )

    def test_get_by_name(self):
        method = "test_get_by_name"
        url = "/api/countermeasures/name/%s?session_id=test" % quote(
            self.existing_countermeasure_name
        )
        rv = self.app.get(url)
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/countermeasures/name/%s?session_id=test" % quote(
            self.prepare_new_countermeasure().name()
        )
    ...
    def test_post(self):
        ...
        self.app.delete(
            "/api/countermeasures/name/%s?session_id=test"
            % quote(self.prepare_new_countermeasure().name())
        )
        rv = self.app.post(
            url, content_type="application/json", data=new_countermeasure_body
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_put(self):
        ...
        self.assertGreater(
            ...
            -1,
            "The countermeasure was not successfully updated",
        )

        rv = self.app.get(
            "/api/countermeasures/name/%s?session_id=test"
            % quote(countermeasure_to_update.name())
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        rv = self.app.delete(
            "/api/countermeasures/name/%s?session_id=test"
            % quote(countermeasure_to_update.theName)
        )

    def test_generate_asset(self):
        method = "test_generate_asset"
        url = (
            "/api/countermeasures/name/"
            + quote(self.existing_countermeasure_name)
            + "/generate_asset?session_id=test"
        )
        self.logger.info("[%s] URL: %s", method, url)
        ...
        rv = self.app.post(
            url,
            content_type="application/json",
            data=jsonpickle.encode({"session_id": "test"}),
        )
        self.assertIsNotNone(rv.data, "No response")
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        ...
        self.logger.info("[%s] Message: %s\n", method, message)
        self.assertEqual(
            message, "Asset " + self.existing_countermeasure_name + " CM created"
        )
    ...
    def test_generate_asset_from_template(self):
        ...
        url = (
            "/api/countermeasures/name/"
            + quote(self.existing_countermeasure_name)
            + "/template_asset/PFFirewall/generate_asset?session_id=test"
        )
        self.logger.info("[%s] URL: %s", method, url)
    ...
    def test_situate_countermeasure_pattern(self):
        ...
        url = (
            "/api/countermeasures/name/"
            + quote(self.existing_countermeasure_name)
            + "/security_pattern/Demilitarized%20Zone/situate?session_id=test"
        )
        self.logger.info("[%s] URL: %s", method, url)
    ...
    def test_associate_situated_pattern(self):
        method = "test_situate_countermeasure_pattern"
        rv = self.app.post(
            "/api/security_patterns/name/Protection%20Reverse%20Proxy/environment/Psychosis/situate",
            content_type="application/json",
            data=jsonpickle.encode({"session_id": "test"}),
        )
        self.assertIsNotNone(rv.data, "No response")
        ...
        rv = self.app.get(
            "api/countermeasures/name/Location-based%20X.509%20extension/candidate_patterns?session_id=test"
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        url = (
            "/api/countermeasures/name/"
            + quote(self.existing_countermeasure_name)
            + "/security_pattern/Protection%20Reverse%20Proxy/associate_situated?session_id=test"
        )
        self.logger.info("[%s] URL: %s", method, url)
        ...
        rv = self.app.get(
            "api/countermeasures/name/Location-based%20X.509%20extension/patterns?session_id=test"
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        url = (
            "/api/countermeasures/name/"
            + quote(self.existing_countermeasure_name)
            + "/security_pattern/Protection%20Reverse%20Proxy/remove_situated"
        )
        self.logger.info("[%s] URL: %s", method, url)
        rv = self.app.delete(
            url,
