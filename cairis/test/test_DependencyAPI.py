...
class DependencyAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.working_name = (
            "Stroke",
            "Data%20Consumer",
            "Certificate%20Authority",
            "Personal%20certificate",
        )
    ...
    def test_dependency_name_get(self):
        ...
        url = (
            "/api/dependencies/environment/%s/depender/%s/dependee/%s/dependency/%s?session_id=test"
            % self.working_name
        )
    ...
    def test_dependency_post(self):
        ...
        json_dict = {"session_id": "test", "object": new_dep}
        json_body = jsonpickle.encode(json_dict)

        new_name = (
            new_dep.theEnvironmentName,
            new_dep.theDepender,
            new_dep.theDependee,
            new_dep.theDependency,
        )
        delete_url = (
            "/api/dependencies/environment/%s/depender/%s/dependee/%s/dependency/%s?session_id=test"
            % new_name
    ...
    def test_dependency_name_put(self):
        ...
        json_dict = {"session_id": "test", "object": upd_dep}
        json_body = jsonpickle.encode(json_dict)
        upd_url = (
            "/api/dependencies/environment/%s/depender/%s/dependee/%s/dependency/%s?session_id=test"
            % new_name
        ...
        delete_name = (
            upd_dep.theEnvironmentName,
            upd_dep.theDepender,
            upd_dep.theDependee,
            upd_dep.theDependency,
        )
        del_get_url = (
            "/api/dependencies/environment/%s/depender/%s/dependee/%s/dependency/%s?session_id=test"
            % delete_name
