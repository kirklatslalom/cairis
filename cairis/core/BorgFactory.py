...
def parseConfigFile():
    ...
    try:
        ...
        raise ARMException(
            "CAIRIS_CFG environment variable has not been set.  Please set this to the correct location of your CAIRIS configuration file, e.g. export CAIRIS_CFG=/home/cairisuser/cairis.cnf"
        )

    if not os.path.exists(cfgFileName):
        raise ARMException(
            "Unable to locate configuration file at the following location:"
            + cfgFileName
        )
    cfgDict = {}
...
def dInitialise(withTest=True):
    ...
    for key, path in list(paths.items()):
        if not os.path.exists(path):
            err_msg = "The {0} directory of CAIRIS is inaccessible or not existing.{1}Path: {2}".format(
                key, os.linesep, path
            )
