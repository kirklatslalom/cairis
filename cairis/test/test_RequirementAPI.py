...
class RequirementAPITests(CairisDaemonTestCase):
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            requirements, list, "The result is not a dictionary as expected"
        )
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            url, content_type="application/json", data=self.new_requirement_body
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_get_asset_name(self):
        ...
        url = "/api/requirements/asset/%s?session_id=test" % quote(
        ...
        self.assertIsNotNone(requirements, "No results after deserialization")
        self.assertGreater(
            len(requirements), 0, "No requirements found for this environment"
        )
        self.logger.info(
            "[%s] Requirement: %s [%d]\n", method, requirements[0]["theName"]
        )
    ...
    def test_get_just_asset_name(self):
        method = "test_asset_get_name"
        url = "/api/requirements/asset/%s/names?session_id=test" % quote(
            self.existing_asset_name
        )
        rv = self.app.get(url)
        if sys.version_info > (3,):
    ...
    def test_get_environment_name(self):
        ...
        url = "/api/requirements/environment/%s?session_id=test" % quote(
    ...
    def test_get_just_environment_name(self):
        method = "test_asset_get_name"
        url = "/api/requirements/environment/%s/names?session_id=test" % quote(
            self.existing_environment_name
        )
        rv = self.app.get(url)
        if sys.version_info > (3,):
    ...
    def test_x_put(self):
        ...
        rv = self.app.put(
            "/api/requirements/name/Analysis%20data",
            content_type="application/json",
            data=upd_requirement_body,
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_concept_map_model(self):
        ...
        rv = self.app.post(
            url, content_type="application/json", data=json.dumps(reqBody1)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        rv = self.app.post(
            url, content_type="application/json", data=json.dumps(reqBody2)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
