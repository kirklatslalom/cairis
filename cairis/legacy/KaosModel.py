...
class KaosModel:
    ...
    def layout(self, renderer=""):
        if renderer == "":
            if (
                (self.theKaosModel == "goal")
                or (self.theKaosModel == "template_goal")
                or (self.theKaosModel == "obstacle")
            ):
                renderer = "dot"
    ...
    def buildGoalModel(self, isComponent=False):
        ...
        for association in self.theAssociations:
            ...
            if (
                self.theGoalName != "" or isComponent == True
            ) and goalName not in self.nodeNameSet:
                self.buildNode(goalDimName, goalName)
            if (
                self.theGoalName != "" or isComponent == True
            ) and subGoalName not in self.nodeNameSet:
            ...
            if (associationType == "obstruct") or (associationType == "resolve"):
                ...
                if (subGoalName, goalName) not in edgeSet:
                    ...
                    edge1 = pydot.Edge(
                        goalName, objtUrl, dir="forward", arrowhead="vee", weight="1"
                    )
                    self.theGraph.add_edge(edge1)
                    edge2 = pydot.Edge(
                        objtUrl, subGoalName, dir="forward", arrowhead="vee", weight="1"
                    )
                ...
                if (subGoalName, refNodeName) not in edgeSet:
                    ...
                    if alternativeFlag == 1:
                        refNodeName = (
                            goalName + "#" + subGoalName + "#" + associationType
                        )
                    ...
                    if (subGoalName, refNodeName) not in edgeSet:
                        self.theGraph.add_edge(
                            pydot.Edge(subGoalName, refNodeName, dir="none", weight="1")
                        )
    ...
    def buildTaskModel(self):
        ...
        for association in self.theAssociations:
            ...
            if assocType in (
                ...
                assocType == "rolepersona_association"
                or assocType == "roleattacker_association"
            ):
                arrowHead = "empty"
                assocDir = "forward"
    ...
    def graph(self):
        try:
            ...
            if self.theKaosModel == "goal" and self.theGoalName == "":
                ...
                elements = self.dbProxy.responsibilityModelElements(
                    self.theEnvironmentName
                )
            ...
            if (
                (self.theKaosModel == "goal")
                or (self.theKaosModel == "responsibility")
                or (self.theKaosModel == "obstacle")
            ):
                self.buildGoalModel()
