...
class GUIDatabaseProxy(MySQLDatabaseProxy):
    ...
    def associateGrid(self, gridObjt):
        self.theGrid = gridObjt

    def riskEnvironmentNames(self, riskName):
        return self.responseList(
            "call riskEnvironmentNames(:risk)",
            {"risk": riskName},
    ...
    def threatVulnerabilityEnvironmentNames(self, threatName, vulName):
        return self.responseList(
            "call threatVulnerabilityEnvironmentNames(:threat,:vuln)",
            {"threat": threatName, "vuln": vulName},
    ...
    def threatTypes(self, envName=""):
        rows = self.responseList(
            "call threatTypes(:env)",
            {"env": envName},
            "MySQL error getting threat types",
        )
        stats = {}
    ...
    def inheritedAssetProperties(self, assetId, environmentName):
        ...
        return AssetEnvironmentProperties(
    ...
    def inheritedAttackerProperties(self, attackerId, environmentName):
        ...
        return AttackerEnvironmentProperties(
    ...
    def inheritedThreatProperties(self, threatId, environmentName):
        ...
        return ThreatEnvironmentProperties(
            environmentName, likelihood, assets, attackers, syProperties, pRationale
        )
    ...
    def inheritedTaskProperties(self, taskId, environmentName):
        ...
        return TaskEnvironmentProperties(
            environmentName, dependencies, personas, assets, concs, narrative
        )
    ...
    def inheritedUseCaseProperties(self, ucId, environmentName):
        ...
        return UseCaseEnvironmentProperties(
    ...
    def inheritedGoalProperties(self, goalId, environmentName):
        ...
        return GoalEnvironmentProperties(
            environmentName,
            "",
            goalDef,
            goalType,
    ...
    def inheritedObstacleProperties(self, obsId, environmentName):
        ...
        return ObstacleEnvironmentProperties(
            environmentName, "", obsDef, obsType, goalRefinements, subGoalRefinements
        )

    def reassociateAsset(self, assetName, envName, reqId):
        self.updateDatabase(
            "call reassociateAsset(:ass,:env,:req)",
            {"ass": assetName, "env": envName, "req": reqId},
            "MySQL error reassociating asset",
        )

    def getEnvironmentGoals(self, goalName, envName):
        goalRows = self.responseList(
            "call getEnvironmentGoals(:goal,:env)",
        ...
        for goalId, goalName, goalOrig in goalRows:
            ...
            parameters = GoalParameters(
                goalName, goalOrig, [], self.goalEnvironmentProperties(goalId)
            )
    ...
    def updateEnvironmentGoal(self, g, envName):
        ...
        self.updateDatabase(
            "call updateEnvironmentGoal(:id,:env,:name,:orig,:def,:cat,:pri,:fc,:issue)",
            {
                "id": g.id(),
                "env": envName,
                "name": g.name(),
            ...
            "MySQL error updating environment goal",
        )

    def getTaskSpecificCharacteristics(self, tName):
        rows = self.responseList(
            "call taskSpecificCharacteristics(:task)",
            {"task": tName},
            "MySQL error getting task specific characteristics",
        )
        tChars = {}
        tcSumm = []
    ...
    def getImpliedProcesses(self, constraintId=-1):
        ipRows = self.responseList(
            "call getImpliedProcesses(:const)",
            {"const": constraintId},
            "MySQL error getting implied processes",
        )
        ips = {}
        for ipId, ipName, ipDesc, pName, ipSpec in ipRows:
            ...
            parameters = ImpliedProcessParameters(
                ipName, ipDesc, pName, ipNet, ipSpec, chs
            )
    ...
    def updateImpliedProcess(self, parameters):
        ...
        session = self.updateDatabase(
            "call deleteImpliedProcessComponents(:id)",
            {"id": ipId},
            "MySQL error deleting implied process components",
            None,
            False,
        )
        self.updateDatabase(
            "call updateImpliedProcess(:id,:name,:desc,:proc,:spec)",
            {
                "id": ipId,
                "name": ipName,
                "desc": ipDesc.encode("utf-8"),
                "proc": pName,
                "spec": ipSpec.encode("utf-8"),
            },
            "MySQL error updating implied process",
            session,
        )
    ...
    def getSubGoalNames(self, goalName, envName):
        rows = [""]
        rows += self.responseList(
            "call subGoalNames(:goal,:env)",
            {"goal": goalName, "env": envName},
            "MySQL error getting sub goal names",
        )
        return rows
    ...
    def getSubObstacleNames(self, obsName, envName):
        rows = [""]
        rows += self.responseList(
            "call subObstacleNames(:obs,:env)",
            {"obs": obsName, "env": envName},
            "MySQL error getting sub obstacle names",
        )
        return rows
    ...
    def goalEnvironments(self, goalName):
        rows = [""]
        rows += self.responseList(
            "call goalEnvironments(:goal)",
            {"goal": goalName},
            "MySQL error getting goal environments",
        )
        return rows
    ...
    def obstacleEnvironments(self, obsName):
        rows = [""]
        rows += self.responseList(
            "call obstacleEnvironments(:obs)",
            {"obs": obsName},
            "MySQL error getting obstacle environments",
        )
        return rows

    def getEnvironmentObstacles(self, obsName, envName):
        obsRows = self.responseList(
            "call getEnvironmentObstacles(:obs,:env)",
            {"obs": obsName, "env": envName},
        ...
        for obsId, obsName, obsOrig in obsRows:
            ...
            parameters = ObstacleParameters(
                obsName, obsOrig, self.obstacleEnvironmentProperties(obsId)
            )
    ...
    def updateEnvironmentObstacle(self, o, envName):
        ...
        self.updateDatabase(
            "call updateEnvironmentObstacle(:id,:env,:name,:orig,:def,:cat)",
            {
                "id": o.id(),
                "env": envName,
                "name": o.name(),
            ...
            "MySQL error updating environment obstacle",
        )

    def impliedProcessChannels(self, procName):
        return self.responseList(
            "call impliedProcessChannels(:proc)",
            {"proc": procName},
            "MySQL error getting implied process channels",
        )

    def artifactText(self, artType, artName):
        if artType == "internal_document":
            return self.responseList(
                "call artifactText(:type,:name)",
                {"type": artType, "name": artName},
                "MySQL error getting artifact text",
            )[0]
        else:
            return ""
    ...
    def impliedCharacteristicElements(self, pName, fromCode, toCode, rtName, isLhs):
        return self.responseList(
            "call impliedCharacteristicElements(:pName,:fCode,:tCode,:rt,:lhs)",
            {
                "pName": pName,
                "fCode": fromCode,
                "tCode": toCode,
            ...
            "MySQL error getting implied characteristic elements",
        )

    def initialiseImpliedCharacteristic(self, pName, fromCode, toCode, rtName):
        self.updateDatabase(
            "call initialiseImpliedCharacteristic(pName,fCode,tCode,rt)",
            {"pName": pName, "fCode": fromCode, "tCode": toCode, "rt": rtName},
            "MySQL error initialising implied characteristic",
        )
    ...
    def exposedCountermeasure(self, envName, assetName):
        rows = self.responseList(
            "call exposedCountermeasure(:env,:ass)",
            {"env": envName, "ass": assetName},
            "MySQL error checking countermeasures exposed by " + assetName,
        )
        expCMs = []
        for r1, r2 in rows:
    ...
    def getExternalDocumentReferences(self, docName=""):
        drRows = self.responseList(
            "call getDocumentReferencesByExternalDocument(:doc)",
            {"doc": docName},
            "MySQL error getting document references for external document " + docName,
        )
        dRefs = {}
    ...
    def getPersonaBehaviouralCharacteristics(self, pName, bvName):
        rows = self.responseList(
            "call personaBehaviouralCharacteristics(:pName,:bvName)",
            {"pName": pName, "bvName": bvName},
            "MySQL error getting persona behavioural characteristics",
        )
        pChars = {}
        pcSumm = []
    ...
    def personaImpactRationale(self, cvName, personaName, envName):
        rows = self.responseList(
            "call personaImpactRationale(:cv,:pers,:env)",
            {"cv": cvName, "pers": personaName, "env": envName},
            "MySQL error getting persona impact rationale",
        )
        piRationale = {}
