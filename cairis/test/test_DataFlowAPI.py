...
class DataFlowAPITests(CairisDaemonTestCase):
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            dataflows, list, "The result is not a dictionary as expected"
        )
    ...
    def test_get_by_name(self):
        ...
        url = (
            "/api/dataflows/name/"
            + quote(self.existing_dataflow_name)
            + "/from_name/"
            + self.existing_from_name
            + "/from_type/"
            + self.existing_from_type
            + "/to_name/"
            + self.existing_to_name
            + "/to_type/"
            + self.existing_to_type
            + "/environment/"
    ...
    def test_post(self):
        ...
        rv = self.app.delete(
            "/api/dataflows/name/acknowledge/from_name/Authenticate Researcher/from_type/process/to_name/Authorised Researcher/to_type/entity/environment/Psychosis?session_id=test"
        )
    ...
    def test_put(self):
        ...
        rv = self.app.put(
            "/api/dataflows/name/acknowledge/from_name/Authenticate%20Researcher/from_type/process/to_name/Authorised%20Researcher/to_type/entity/environment/Psychosis?session_id=test",
            data=upd_env_body,
        ...
        rv = self.app.get(
            "/api/dataflows/name/Edited%20test%20dataflow/from_name/"
            + dataflow_to_update.fromName()
            + "/from_type/"
            + dataflow_to_update.fromType()
            + "/to_name/"
            + dataflow_to_update.toName()
            + "/to_type/"
            + dataflow_to_update.toType()
            + "/environment/Psychosis?session_id=test"
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        self.assertEqual(
            upd_dataflow["theEnvironmentName"], dataflow_to_update.environment()
        )
