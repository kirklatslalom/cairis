...
class TaskTest(unittest.TestCase):
    def setUp(self):
        ...
        iep2 = EnvironmentParameters(
            iEnvironments[1]["theName"],
        ...
        ipp = PersonaParameters(
            ...
            [
                ...
                PersonaEnvironmentProperties(
                    ...
                    (
                        iPersonas[0]["theEnvironmentProperties"][1]["theDirectFlag"]
                        == "True"
                    ),
                    iPersonas[0]["theEnvironmentProperties"][1]["theNarrative"],
    ...
    def testTask(self):
        ...
        iteps = [
            TaskEnvironmentProperties(
                envName,
                deps,
                [(taskPersona, taskDuration, taskFreq, taskDemands, taskGoalConflict)],
                taskAssets,
                taskConcerns,
                taskNarrative,
                taskConsequences,
        ...
        self.assertEqual(
            iTask.environmentProperties()[0].personas(),
            oTask.personas(envName, "", envName),
        )
        self.assertEqual(
            iTask.environmentProperties()[0].assets(), oTask.assets(envName, "")
        )
        self.assertEqual(
            iTask.environmentProperties()[0].narrative(), oTask.narrative(envName, "")
        )
        self.assertEqual(
            iTask.environmentProperties()[0].consequences(),
            oTask.environmentProperties()[0].consequences(),
        )
        self.assertEqual(
            iTask.environmentProperties()[0].benefits(),
            oTask.environmentProperties()[0].benefits(),
        )
        self.assertEqual(
            iTask.environmentProperties()[0].dependencies(),
        ...
        self.assertEqual(
            iTask.environmentProperties()[0].assets(),
            oTask.environmentProperties()[0].assets(),
        )
        self.assertEqual(
            iTask.environmentProperties()[0].assets(), oTask.assets("Psychosis", "")
        )
        self.assertEqual(
            iTask.environmentProperties()[0].assets(),
        ...
        self.assertEqual(
            iTask.environmentProperties()[0].narrative(),
