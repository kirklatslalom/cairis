...
def importSecurityPatterns(taps, spps, vts, session_id=None):
    ...
    for tap in taps:
        if db_proxy.nameExists(tap.name(), "asset"):
            raise ARMException(
                "Cannot import template asset with name "
                + tap.name()
                + " as asset with same name already exists."
            )
...
def importAttackPattern(importFile, session_id=None):
    try:
        ...
        risk = handler.risk()
        raTxt = importRiskAnalysis(
            [], assets, [vulnerability], attackers, [threat], [risk], [], [], session_id
        )
        obsTxt = importRequirements([], [], handler.obstacles(), [], [], [], session_id)
        assocTxt = importAssociations(
            [], handler.obstacleAssociations(), [], session_id
        )
...
def importRiskAnalysis(
    roleParameterSet,
    assetParameterSet,
    vulParameterSet,
    attackerParameterSet,
    threatParameterSet,
    riskParameterSet,
    responseParameterSet,
    assocParameterSet,
    session_id,
):
    b = Borg()
...
def importUsabilityFile(importFile, session_id=None):
    try:
        ...
        parser.setEntityResolver(handler)
        parser.parse(importFile)
        return importUsability(
            handler.personas(),
            handler.externalDocuments(),
...
def importMisusabilityFile(importFile, session_id=None):
    try:
        ...
        parser.setEntityResolver(handler)
        parser.parse(importFile)
        return importMisusability(
            handler.conceptReferences(),
            handler.taskCharacteristics(),
            session_id=session_id,
        )
    except xml.sax.SAXException as e:
        raise ARMException("Error parsing" + importFile + ": " + e.getMessage())
...
def importProjectFile(importFile, session_id=None):
    try:
        ...
        return importProjectData(
            pSettings, envParameterSet, ceParameterSet, session_id=session_id
        )
    except xml.sax.SAXException as e:
        raise ARMException("Error parsing" + importFile + ": " + e.getMessage())
...
def importSynopses(
    charSyns, refSyns, stepSyns, refConts, ucConts, taskConts, session_id
):
    b = Borg()
    db_proxy = b.get_dbproxy(session_id)
    for cs in charSyns:
...
def importDomainValuesFile(importFile, session_id=None):
    try:
        ...
        parser.setEntityResolver(handler)
        parser.parse(importFile)
        tvValues, rvValues, cvValues, svValues, lvValues, capValues, motValues = list(
            handler.values()
        )
        return importDomainValues(
            tvValues,
            rvValues,
            cvValues,
            svValues,
...
def importDomainValues(
    tvValues, rvValues, cvValues, svValues, lvValues, capValues, motValues, session_id
):
...
def importModelFile(importFile, isOverwrite=1, session_id=None):
    try:
        ...
        if isOverwrite == 1:
            ...
            modelTxt += (
                importTVTypeFile(importFile, isOverwrite, session_id=session_id) + "  "
            )
...
def importDiagramsNetDFD(importFile, envName, session_id):
    ...
    for objt in objts:
        ...
        if db_proxy.existingObject(objt["name"], objtType) == -1:
            cairis.core.DefaultParametersFactory.build(
                objt["name"], envName, objtType, session_id, assetType
            )
            if objtType == "usecase":
                newUCCount += 1
            else:
        ...
        try:
            ...
            for dfAsset in dfAssets:
                if db_proxy.existingObject(dfAsset, "asset") == -1:
                    cairis.core.DefaultParametersFactory.build(
                        dfAsset, envName, "asset", session_id, "Information"
                    )
                    newAssetCount += 1
            db_proxy.addDataFlow(
                DataFlowParameters(
                    dfName,
                    "Information",
                    envName,
                ...
                AssetParameters(
                    ...
                    [
                        AssetEnvironmentProperties(
                            envName, asset["properties"], asset["rationale"]
                        )
                    ],
                )
            )
...
def importPersonaCharacteristicsWorkbook(wbFile, session_id):
        ...
        if drName != "" and drName != None:
            ...
            if drDoc not in edNames:
                if db_proxy.existingObject(drDoc, "external_document") == -1:
                    raise ARMException(
                        "Cannot import document reference "
                        + drName
                        + ". "
                        + drDoc
    ...
    for row in pcSheet.iter_rows(min_row=2):
        ...
        if grounds == None:
            ...
            grounds = list(
                map(lambda x: (x.strip(), "", "document"), grounds.strip().split(","))
            )
        ...
        if warrant == None:
            ...
            warrant = list(
                map(lambda x: (x.strip(), "", "document"), warrant.strip().split(","))
            )
        ...
        if rebuttal == None:
            ...
            rebuttal = list(
                map(lambda x: (x.strip(), "", "document"), rebuttal.strip().split(","))
            )
        ...
        if pcName != "":
            if db_proxy.existingObject(pName, "persona") == -1:
                raise ARMException(
                    "Cannot import persona characteristic "
                    + pcName
                    + ". Persona "
                    + pName
                    + " does not exist."
            ...
            if bvName not in [
                ...
                raise ARMException(
                    ...
                    + ". "
                    + bvName
                    + " is an invalid behavioural variable name."
                )
            ...
            for g in grounds:
                if (g[0] not in drNames) and (
                    db_proxy.existingObject(g[0], "document_reference") == -1
                ):
                    raise ARMException(
                        "Cannot import persona characteristic "
                        + pcName
                        + ". Document reference corresponding with grounds "
                        + g[0]
                        + " does not exist."
                    )
            for w in warrant:
                if (w[0] not in drNames) and (
                    db_proxy.existingObject(w[0], "document_reference") == -1
                ):
                    raise ARMException(
                        "Cannot import persona characteristic "
                        + pcName
                        + ". Document reference corresponding with warrant "
                        + w[0]
                        + " does not exist."
                    )
            for r in rebuttal:
                if (r[0] not in drNames) and (
                    db_proxy.existingObject(r[0], "document_reference") == -1
                ):
                    raise ARMException(
                        "Cannot import persona characteristic "
                        + pcName
                        + ". Document reference corresponding with rebuttal "
                        + r[0]
                        + " does not exist."
                    )

            pcs.append(
                PersonaCharacteristicParameters(
                    pName, modQual, bvName, pcName, grounds, warrant, [], rebuttal
                )
            )
    ...
    for pcParameters in pcs:
        objtId = db_proxy.existingObject(
            pcParameters.characteristic(), "persona_characteristic"
        )
        if objtId == -1:
            db_proxy.addPersonaCharacteristic(pcParameters)
        else:
