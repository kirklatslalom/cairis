...
from cairis.tools.ModelDefinitions import (
    CountermeasureModel,
    CountermeasureEnvironmentPropertiesModel,
    CountermeasureTask,
)
...
class CountermeasureDAO(CairisDAO):
    ...
    def from_json(self, request):
        ...
        json_dict["__python_obj__"] = (
            Countermeasure.__module__ + "." + Countermeasure.__name__
        )

        countermeasure_props = self.convert_props(
        ...
        if real_props is not None:
            if len(real_props) > 0:
                for real_prop in real_props:
                    ...
                    for ctc in real_prop.personas():
                        ctList.append(
                            CountermeasureTaskCharacteristics(
                                ctc[0], ctc[1], ctc[2], ctc[3], ctc[4], ctc[5]
                            )
                        )
            ...
            if len(fake_props) > 0:
                for fake_prop in fake_props:
                    check_required_keys(
                        fake_prop, CountermeasureEnvironmentPropertiesModel.required
                    )
                    ctList = []
