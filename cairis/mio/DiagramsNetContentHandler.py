...
def attrsToDict(str):
    g = {}
    for val in list(map(lambda x: x.split("="), str.rstrip(";").split(";"))):
        g[val[0]] = val[1]
    return g
...
class DiagramsNetContentHandler(ContentHandler, EntityResolver):
    ...
    def objects(self):
        return list(self.theObjects.values())

    def flows(self):
        return self.theUpdatedFlows

    def trustBoundaries(self):
        return list(self.theUpdatedTrustBoundaries.values())

    def associations(self):
        return self.theUpdatedAssociations
    ...
    def startElement(self, name, attrs):
        if name == "object":
            ...
            if self.theModelType == "dataflow":
                if "type" not in attrs:
                    if "assets" not in attrs and self.theObjectId != "0":
                        self.theFlows[self.theObjectId] = {
                            "assets": ["Unknown information"]
                        }
                    elif "assets" in attrs and self.theObjectId != "0":
                        self.theFlows[self.theObjectId] = {
                            "name": "Undefined flow",
                            "assets": list(
                                map(lambda v: v.strip(), attrs["assets"].split(","))
                            ),
                        }
                    self.inFlow = 1
                else:
                    ...
                    if objtType in [
                        ...
                        self.theTrustBoundaries[self.theObjectId] = {
                            "name": sanitise(attrs["name"])
                        }
                        ...
                        if objtType not in ["process", "entity", "datastore"]:
                            raise ARMException(
                                objtType
                                + " is not a valid type for DFD object "
                                + objtName
                            )
                        self.theObjects[self.theObjectId] = {
                            "name": objtName,
                            "type": objtType,
                        }
                        self.inObject = 1
                ...
                if assetType not in [
                    ...
                    raise ARMException(
                        attrs["type"]
                        + " is an invalid type for asset "
                        + assetName
                        + ".  Valid types are Hardware, Information, People, Software, and Systems."
                    )
                ...
                self.theObjects[self.theObjectId] = {
                    "name": assetName,
                    "short_code": assetSC,
                    "type": assetType.capitalize(),
                    "description": assetDesc,
                    "significance": assetSig,
            ...
            name == "mxCell"
            and self.theModelType == "asset"
            and "source" in attrs
            and "target" in attrs
        ):
            if "style" not in attrs:
                raise ARMException(
                    "Missing style attribute in mxCell id " + attrs["id"]
                )
            ...
            if (
                ("startArrow" not in d)
                and ("endArrow" not in d)
                and ("edgeStyle" in d)
                and (d["edgeStyle"] == "orthogonalEdgeStyle")
            ):
                tailNav = 1
            else:
            ...
            if self.inObject:
                ...
                self.theObjects[self.theObjectId]["maxX"] = float(attrs["x"]) + float(
                    attrs["width"]
                )
                self.theObjects[self.theObjectId]["minY"] = float(attrs["y"])
                self.theObjects[self.theObjectId]["maxY"] = float(attrs["y"]) + float(
                    attrs["height"]
                )
            elif self.inTrustBoundary:
                self.theTrustBoundaries[self.theObjectId]["minX"] = float(attrs["x"])
                self.theTrustBoundaries[self.theObjectId]["maxX"] = float(
                    attrs["x"]
                ) + float(attrs["width"])
                self.theTrustBoundaries[self.theObjectId]["minY"] = float(attrs["y"])
                self.theTrustBoundaries[self.theObjectId]["maxY"] = float(
                    attrs["y"]
                ) + float(attrs["height"])
        ...
        for objtKey in self.theFlows:
            ...
            if (fromType, toType) not in validFlowTypes:
                raise ARMException(
                    ...
                    + " to "
                    + toType
                    + " flows are not permissible."
                )
            else:
                self.theUpdatedFlows.append(
                    {
                        "name": dfName,
                        "from_name": fromName,
                        "from_type": fromType,
            ...
            for objtKey in self.theObjects:
                ...
                if (
                    ...
                    if tbKey not in self.theUpdatedTrustBoundaries:
                        self.theUpdatedTrustBoundaries[tbKey] = {
                            "name": tbName,
                            "components": [],
                        }
                    ...
                    if compType == "entity":
                        raise ARMException(
                            ...
                            + " to trust boundary "
                            + tbName
                            + ". Entities are invalid trust boundary components."
                        )
                    else:
                        self.theUpdatedTrustBoundaries[tbKey]["components"].append(
                            {"name": objtName, "type": compType}
                        )

    def updateAssociations(self):
        for assoc in self.theAssociations:
            self.theUpdatedAssociations.append(
                {
                    "head": self.theObjects[assoc["head"]]["name"],
                    "headType": assoc["headType"],
                    "headNav": assoc["headNav"],
                    "tail": self.theObjects[assoc["tail"]]["name"],
                    "tailType": assoc["tailType"],
                    "tailNav": assoc["tailNav"],
                }
            )
