...
class ObstacleAPITests(CairisDaemonTestCase):
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            obstacles, dict, "The result is not a dictionary as expected"
        )
    ...
    def test_get_by_name(self):
        ...
        url = "/api/obstacles/name/%s?session_id=test" % quote(
            self.existing_obstacle_name
        )
        rv = self.app.get(url)
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/obstacles/name/%s?session_id=test" % quote(
            self.prepare_new_obstacle().theName
        )
        ...
        self.app.post(
            "/api/obstacles", content_type="application/json", data=new_obstacle_body
        )
    ...
    def test_post(self):
        ...
        rv = self.app.delete(
            "/api/obstacles/name/%s?session_id=test"
            % quote(self.prepare_new_obstacle().theName)
        )
    ...
    def test_put(self):
        ...
        self.assertGreater(
            message.find("updated"), -1, "The obstacle was not successfully updated"
        )

        rv = self.app.get(
            "/api/obstacles/name/%s?session_id=test" % quote(obstacle_to_update.theName)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
    ...
    def test_generate_vulnerability(self):
        ...
        url = (
            "/api/obstacles/name/"
            + quote(self.existing_obstacle_name)
            + "/generate_vulnerability?session_id=test"
        )
        self.logger.info("[%s] URL: %s", method, url)
        ...
        self.logger.info("[%s] Message: %s\n", method, message)
        self.assertGreater(
            message.find("successfully generated"), -1, "Vulnerability not generated"
        )
        ...
        return Obstacle(
            ...
            [
                ...
                ObstacleEnvironmentProperties(
                    "Stroke",
                    "New Obstacle",
                    "This is a test definition for env2",
                    "Integrity Threat",
                    [],
                    [],
                    [],
