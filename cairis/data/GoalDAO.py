...
class GoalDAO(CairisDAO):
        ...
        try:
            if not self.check_existing_goal(goal.theName):
                ...
                raise CairisHTTPError(
                    ...
                    message="An object with the name "
                    + goal.theName
                    + " already exists.",
                )
        except DatabaseProxyException as ex:
    ...
    def get_responsibility_model(self, environment_name, role_name, pathValues=[]):
        ...
        if role_name == "all":
            role_name = ""
        try:
            associationDictionary = self.db_proxy.responsibilityModel(
                environment_name, role_name
            )
            associations = KaosModel(
                list(associationDictionary.values()),
                environment_name,
    ...
    def add_goal_type(self, goal_type, environment_name=""):
        ...
        type_exists = self.check_existing_goal_type(
            goal_type.theName, environment_name=environment_name
        )
        ...
        if real_props is not None:
            for real_prop in real_props:
                ...
                for sgr in real_prop.theSubGoalRefinements:
                    new_subgoal_refinements.append(
                        RefinementModel(sgr[0], sgr[1], sgr[2], sgr[3], sgr[4])
                    )
                ...
                if gp != None:
                    real_prop.thePolicy = PolicyStatementModel(
                        goalName,
                        gp["theEnvironmentName"],
                        gp["theSubject"],
                        gp["theAccessType"],
                        gp["theResource"],
                        gp["thePermission"],
            ...
            for fake_prop in fake_props:
                ...
                for concern_assoc in fake_prop["theConcernAssociations"]:
                    new_concern_assocs.append(
                        [
                            concern_assoc["theSource"],
                            concern_assoc["theSourceNry"],
                            concern_assoc["theLinkVerb"],
                            concern_assoc["theTarget"],
                            concern_assoc["theTargetNry"],
                        ]
                    )
                ...
                if fgp != None:
                    gp = {
                        "theGoalName": goalName,
                        "theEnvironmentName": fake_prop["theEnvironmentName"],
                        "theSubject": fgp["theSubject"],
                        "theAccessType": fgp["theAccessType"],
                        "theResource": fgp["theResource"],
                        "thePermission": fgp["thePermission"],
                    }
                else:
                    gp = None
    ...
    def from_json(self, request):
        ...
        real_props = self.convert_properties(
            json_dict["theName"], fake_props=props_list
        )
    ...
    def simplify(self, goal):
        goal.theEnvironmentProperties = self.convert_properties(
            goal.theName, real_props=goal.theEnvironmentProperties
        )
