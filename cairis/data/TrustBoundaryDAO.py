...
class TrustBoundaryDAO(CairisDAO):
    ...
    def update_object(self, tb, old_trust_boundary_name):
        try:
            tbId = self.db_proxy.getDimensionId(
                old_trust_boundary_name, "trust_boundary"
            )
            tb.theId = tbId
    ...
    def from_json(self, request):
        ...
        json_dict["__python_obj__"] = (
            TrustBoundary.__module__ + "." + TrustBoundary.__name__
        )
        comps, pls = self.convert_props(
    ...
    def simplify(self, tb):
        ...
        del tb.theId
        tb.theEnvironmentProperties = self.convert_props(
            real_props=(tb.theComponents, tb.thePrivilegeLevels)
        )
    ...
    def convert_props(self, real_props=None, fake_props=None):
        if real_props is not None:
            ...
            for envName in real_comps:
                ...
                new_props.append(
                    TrustBoundaryEnvironmentModel(envName, compList, pLevel)
                )
            return new_props
