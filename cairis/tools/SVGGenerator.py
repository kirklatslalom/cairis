...
def embedImage(line):
    if line.find("<image") == 0:
        ...
        with open(imageName, "rb") as image_file:
            ...
            line = line.replace(
                "/assets/" + linkName, "data:image/png;base64," + str(encoded_string)
            )
    return line
    ...
    while href_exists > -1:
        ...
        if is_valid:
            ...
            if (model_type == "goal" or model_type == "risk") and type == "requirement":
                ...
                new_link = "/api/{0}s/name/{1}/from_name/{2}/from_type/{3}/to_name/{4}/to_type/{5}/environment/{6}".format(
                    type,
                    object,
                    quote(fromName),
                    quote(fromType),
                ...
                new_link = "/api/{0}s/name/{1}/environment/{2}".format(
                    type, object, quote(environment)
                )
            elif type == "goalassociation":
                assocParams = parts[1].split("/")
                new_link = (
                    "/api/goalassociations/environment/"
                    + quote(assocParams[0])
                    + "/goal/"
                    + quote(assocParams[1])
                    + "/subgoal/"
