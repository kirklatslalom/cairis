...
class PersonaCharacteristicAPITests(CairisDaemonTestCase):
    ...
    def setUp(self):
        ...
        self.new_pc = {
            ...
            "theGrounds": [
                {
                    "theReferenceName": "Line manager site authorisation",
                    "theDimensionName": "document",
                    "theCharacteristicType": "grounds",
                    "theReferenceDescription": "Can only access sites they have been authorised to; permission for authorisation changes need to be sought from the line manager.",
                    "theReferenceSynopsis": {
                        "theActor": "Rick",
                        "theActorType": "persona",
                        "theSynopsis": "Holds authorisation",
                        "theDimension": "goal",
                    },
            ...
            "theWarrant": [
                {
                    "theReferenceDescription": "Work reports are filed and sent to ACME monthly.",
                    "theDimensionName": "document",
                    "theCharacteristicType": "warrant",
                    "theReferenceName": "Work reports are filed",
                    "theReferenceSynopsis": {
                        "theActor": "Rick",
                        "theActorType": "persona",
                        "theSynopsis": "Reports filed",
                        "theDimension": "goal",
                    },
                    "theReferenceContribution": {
                        "theMeansEnd": "means",
            ...
            "theRebuttal": [
                {
                    "theReferenceDescription": "Everything that happens is logged.",
                    "theDimensionName": "document",
                    "theCharacteristicType": "rebuttal",
                    "theReferenceName": "Everything is logged",
                    "theReferenceSynopsis": {
                        "theActor": "Risk",
                        "theActorType": "persona",
                        "theSynopsis": "Event logged",
                        "theDimension": "goal",
                    },
                    "theReferenceContribution": {
                        "theMeansEnd": "means",
            ...
            "theCharacteristicSynopsis": {
                "theActor": "Rick",
                "theActorType": "persona",
                "theSynopsis": "Characteristic tested",
                "theDimension": "goal",
            },
        }
        self.new_pc_dict = {"session_id": "test", "object": self.new_pc}
    ...
    def test_get_all_summary(self):
        ...
        self.logger.info("[%s] Persona characteristics found: %d", method, len(pcs))
        self.logger.info(
            "[%s] First persona characteristic summary: %s", method, pcs[0]["theName"]
        )

    def test_get_by_name(self):
        method = "test_get_by_name"
        url = "/api/persona_characteristics/name/%s?session_id=test" % quote(
            self.existing_pc_name
        )
        rv = self.app.get(url)
    ...
    def test_put(self):
        ...
        rv = self.app.put(
            url, content_type="application/json", data=json.dumps(self.new_pc_dict)
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
