...
class CAIRISDatabaseConfigurationForm(np.ActionForm):
    def create(self):
        ...
        self.theRootPassword = self.add(
            np.TitlePassword, name="Database root password:", value=""
        )
        ...
        self.theRootDir = self.add(
            np.TitleText, name="Root directory:", value=pathName + "cairis"
        )
        self.theFileName = self.add(
            np.TitleText,
            name="CAIRIS configuration file name:",
            value=os.environ.get("HOME") + "/cairis.cnf",
        )
        ...
        self.theStaticDir = self.add(
            np.TitleText, name="Static directory:", value=pathName + "cairis/dist"
        )
        self.theAssetDir = self.add(
            np.TitleText, name="Asset directory:", value=pathName + "cairis/dist"
        )

        self.theUsername = self.add(np.TitleText, name="Initial Email:", value="")
        self.thePassword = self.add(
            np.TitlePassword, name="Initial Password:", value=""
        )
    ...
    def on_ok(self):
        try:
            quick_setup(
                self.theHost.value,
                int(self.thePort.value),
                self.theRootPassword.value,
                self.theTmpDir.value,
                self.theRootDir.value,
                self.theFileName.value,
                int(self.theWebPort.value),
                self.theLogLevel.value,
                self.theStaticDir.value,
                self.theAssetDir.value,
                self.theUsername.value,
                self.thePassword.value,
                self.theMailServer.value,
                self.theMailPort.value,
                self.theMailUser.value,
                self.theMailPasswd.value,
            )
...
def main(args=None):
    if args is None:
        ...
        try:
            ...
            print(
                "The terminal window is too small to display the configuration form, please resize it and try again."
            )
