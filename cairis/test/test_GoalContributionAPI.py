...
class GoalContributionAPITests(CairisDaemonTestCase):
    ...
    def setUpClass(cls):
        pkgStr = open(
            os.environ["CAIRIS_SRC"] + "/../examples/exemplars/webinos.cairis", "rb"
        ).read()
    ...
    def setUp(self):
        ...
        self.new_gc_dict = {"session_id": "test", "object": self.new_gc}
    ...
    def test_get_by_name(self):
        ...
        rv = self.app.post(
            ...
            data=jsonpickle.encode(self.new_gc_dict),
        )

        url = (
            "/api/goal_contributions/source/"
            + quote(self.existing_src_name)
            + "/target/"
            + quote(self.existing_tgt_name)
            + "?session_id=test"
        )
        rv = self.app.get(url)
        responseData = rv.data.decode("utf-8")
        ...
        self.logger.info(
            "[%s] First goal contribution: %s\n",
            method,
            gc["theSource"] + " / " + gc["theDestination"],
        )
