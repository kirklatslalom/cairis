...
class CImportTests(CairisDaemonTestCase):
    ...
    def test_cimport_package_post(self):
        ...
        package = BytesIO(
            open(os.environ["CAIRIS_SRC"] + "/test/test.cairis", "rb").read()
        )
        rv = self.app.post(
            "/api/import/package?session_id=test",
            data={
        ...
        self.assertIsInstance(
            json_dict, dict, "The response is not a valid JSON dictionary"
        )
    ...
    def test_cimport_ugwb_post(self):
        ...
        rv = self.app.post(
            "/api/import/file/user_goals?session_id=test",
            data={
                "file": (wb, "ngug.xlsx"),
            },
        )
        ...
        self.assertEqual(
            message,
            "Imported 5 characteristic synopses, 7 reference synopses, and 4 reference contributions.",
        )
    ...
    def test_cimport_pcwb_post(self):
        ...
        rv = self.app.post(
            "/api/import/file/persona_characteristics?session_id=test",
            data={
                "file": (wb, "ngpc.xlsx"),
            },
        )
        ...
        self.assertEqual(
            message,
            "Imported 2 external documents, 5 document references, and 2 persona characteristics.",
        )
    ...
    def test_cimport_diagramsnet_dfd_post(self):
        ...
        self.assertEqual(
            message,
            "Imported 4 assets, 1 use case, 2 data flows, and 1 trust boundary.",
        )
    ...
    def test_cimport_file_post(self):
        ...
        data = {"file": (buf, "import.xml"), "session_id": "test"}
    ...
    def defaultEnvBody(self):
        defaultEnv = Environment(
            -1, "Default", "DEFAULT", "Default environment", [], "", "", []
        )
