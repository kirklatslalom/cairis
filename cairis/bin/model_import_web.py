...
def importModel(url, dbName, modelFile, session):
    ...
    importResp = requests.post(
        url + "/api/import/package?session_id=" + session,
        data=data,
        files=dict(file=open(modelFile, "rb")),
    )
...
def main(args=None):
    parser = argparse.ArgumentParser(
        description="Computer Aided Integration of Requirements and Information Security - CAIRIS model import (using APIs)"
    )
