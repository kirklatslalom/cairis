...
class DomainPropertyAPITests(CairisDaemonTestCase):
    ...
    def test_get_all(self):
        ...
        self.assertIsInstance(
            domainproperties, list, "The result is a list as expected"
        )
        self.assertGreater(
            len(domainproperties), 0, "No domainproperties in the dictionary"
        )
        self.logger.info(
            "[%s] DomainProperties found: %d", method, len(domainproperties)
        )
        domainproperty = domainproperties[0]
        self.logger.info(
            "[%s] First domainproperty: %s\n", method, domainproperty["theName"]
        )

    def test_get_by_name(self):
        method = "test_get_by_name"
        url = "/api/domainproperties/name/%s?session_id=test" % quote(
            self.existing_domainproperty_name
        )
        rv = self.app.get(url)
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
    ...
    def test_delete(self):
        method = "test_delete"
        url = "/api/domainproperties/name/%s?session_id=test" % quote(
            self.prepare_new_domainproperty().name()
        )
    ...
    def test_post(self):
        ...
        rv = self.app.post(
            url, content_type="application/json", data=new_domainproperty_body
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        self.assertGreater(
            message.find("already exists"),
            -1,
            "The domainproperty post should have failed",
        )

        rv = self.app.delete(
            "/api/domainproperties/name/%s?session_id=test"
            % quote(self.prepare_new_domainproperty().name())
        )
    ...
    def test_put(self):
        ...
        self.assertGreater(
            ...
            -1,
            "The domainproperty was not successfully updated",
        )

        rv = self.app.get(
            "/api/domainproperties/name/%s?session_id=test"
            % quote(domainproperty_to_update.name())
        )
        if sys.version_info > (3,):
            responseData = rv.data.decode("utf-8")
        else:
        ...
        self.assertGreater(
            message.find("already exists"),
            -1,
            "The domainproperty update should have failed",
        )

        rv = self.app.delete(
            "/api/domainproperties/name/%s?session_id=test"
            % quote(domainproperty_to_update.theName)
        )
